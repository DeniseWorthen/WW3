...
CONTAINS
  ...
  SUBROUTINE W3CPRT ( IMOD )
    ...
    ! Formats
    !
9000 FORMAT (' TEST W3CPRT : DIMP, DIMXP, TMPSIZ    :',I2,2I6) ! W3_T
...
9062 FORMAT (' TEST W3CPRT : FINAL STORAGE SIZE :',I6)
  ...
  SUBROUTINE W3IOSF ( NDSPT, IMOD )
    ...
    USE W3GDATMD, ONLY: NSEAL                ! W3_MPI
    ...
    USE W3ODATMD, ONLY: ICPRT, DTPRT, IT0PRT ! W3_MPI
    USE W3ODATMD, ONLY: NDST                 ! W3_T
    !
    IMPLICIT NONE
    !
    ...
    IF ( IAPROC.NE.NAPPRT .AND. IAPROC.LE.NAPROC ) THEN
...
#ifdef W3_MPI
       IT     = IT0PRT + IAPROC - 1
       CALL MPI_SEND ( ICPRT, ICSIZ, MPI_REAL, NAPPRT-1, IT,  MPI_COMM_WAVE, IERR_MPI )
       ...
       IF ( DTSIZ .GT. 0 ) CALL MPI_SEND ( DTPRT, 6*DTSIZ, MPI_REAL, NAPPRT-1, &
