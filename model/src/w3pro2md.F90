...
CONTAINS
  ...
  SUBROUTINE W3MAP2
    ...
    USE W3ADATMD , ONLY : NMXY, MAPX2, MAPY2, MAPAXY, MAPXY
...
9011 FORMAT (2X,60I2)
  ...
  SUBROUTINE W3XYP2 ( ISP, DTG, MAPSTA, MAPFS, VQ, VGX, VGY )
    ...
    USE W3GDATMD , ONLY : NSEA, SX, SY, MAPSF, ICLOSE, FLCX, FLCY
    USE W3GDATMD , ONLY : ICLOSE_NONE, ICLOSE_SMPL, ICLOSE_TRPL
    USE W3GDATMD , ONLY : DTCFL, CLATS, DTME, CLATMN, FLAGLL
    USE W3GDATMD , ONLY : HPFAC, HQFAC, DPDX, DPDY, DQDX, DQDY, GSQRT
    ...
    REAL          :: VFDIFX(1-NY:NX*NY), VFDIFY(NX*NY)
    REAL          :: VFDIFC(1-NY:NX*NY), VDXX((NX+1)*NY)
    REAL          :: VDYY(NX*NY), VDXY((NX+1)*NY)

    REAL          :: CXTOT((NX+1)*NY), CYTOT(NX*NY)
    REAL          :: VQ_OLD(1-NY:NY*(NX+2))
    !CURV2: BEGIN -----------------------------------------------------------------
    REAL          :: VFDIFX_FAC(1-NY:NX*NY)
    REAL          :: VFDIFY_FAC(1-NY:NX*NY)
    REAL          :: VFDIFC_FAC(1-NY:NX*NY)
    !CURV2: END -------------------------------------------------------------------
    !/
    !/ ------------------------------------------------------------------- /
    !/
