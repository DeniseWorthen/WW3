...
CONTAINS
  ...
  SUBROUTINE W3PSMC ( ISP, DTG, VQ )
    ...
    USE W3ODATMD, ONLY: ISBPI, BBPI0, BBPIN
    !
    ...
    !
    REAL, Dimension(-9:NCel) ::  FCNt, AFCN, BCNt, UCFL, VCFL, CQ, CQA, CXTOT, CYTOT
    ...
    ! Formats
    !
9001 FORMAT (' TEST W3PSMC : ISP, ITH, IK, COS-SIN :',I8,2I4,2F7.3) ! W3_T
9003 FORMAT (' TEST W3PSMC : NO DISPERSION CORRECTION ')
    !
9010 FORMAT (' TEST W3PSMC : INITIALIZE ARRAYS') ! W3_T
    !
9020 FORMAT (' TEST W3PSMC : CALCULATING LCFLX/Y AND DSS/NN (NSEA=', & ! W3_T
         I6,')')
9021 FORMAT (1X,I6,2I5,E12.4,2f7.3) ! W3_T1
9022 FORMAT (' TEST W3PSMC : CORRECTING FOR CURRENT') ! W3_T
    !
9040 FORMAT (' TEST W3PSMC : FIELD AFTER PROP. (NSEA=',I6,')') ! W3_T
9041 FORMAT (1X,I6,2I5,E12.4) ! W3_T2
    !/
    !/ End of W3PSMC ----------------------------------------------------- /
    !/
  ...
  SUBROUTINE W3KRTN ( ISEA, FACTH, FACK, CTHG0, CG, WN, DEPTH,    &
    ...
    USE W3GDATMD, ONLY: EC2, ESC, ES2, FLCTH, FLCK, CTMAX, DTH
    ...
    REAL, INTENT(IN)    :: WN(0:NK+1), DEPTH, DDDX, DDDY
    REAL, INTENT(IN)    :: ALFLMT(NTH), CX, CY, DCXDX, DCXDY
    REAL, INTENT(IN)    :: DCYDX, DCYDY, DCDX(0:NK+1), DCDY(0:NK+1) 
  ...
  SUBROUTINE SMCDHXY
    ...
    USE W3GDATMD, ONLY: CLATS, NTH, DTH, ESIN, ECOS, Refran, DMIN
    USE W3GDATMD, ONLY: NGLO, ANGARC, ARCTC
    ...
    DO n=1,NSEA
       ...
       IF ( CNST4 .GT. 1.0E-5 ) THEN
          ...
          if (w3_t_flag) then
             IF( MOD(n, 1000) .EQ. 0 ) WRITE(NDST,'(i8,18F5.1)' ) n, (DHLMT(i,n), i=1,18)
          end if
       ELSE
          DHLMT(:,n) = 0.0
       ENDIF
...
999 PRINT*, ' Sub SMCDHXY ended.'
  ...
  SUBROUTINE SMCDCXY
    ...
    if (w3_debugdcxdx_flag) then
       ! PROBLEM (MV) IAPROC was never defined here - defining it above
