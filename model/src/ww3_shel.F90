...
PROGRAM W3SHEL
  ...
  USE W3OACPMD, ONLY: CPL_OASIS_DEFINE, CPL_OASIS_FINALIZE
  ...
  REAL                :: HS(NHMAX,-7:10)
  REAL                :: CLKFIN, CLKFEL
  REAL, ALLOCATABLE   :: X(:), Y(:), XXX(:,:), DATA0(:,:), DATA1(:,:), DATA2(:,:)
  !
  ...
  CHARACTER(LEN=30)   :: OFILE ! W3_COU
  character(len=30)   :: jchar
  !
  ...
  LOGICAL             :: FLHYBR = .FALSE. ! W3_MPI
  INTEGER             :: THRLEV ! W3_OMPH
  LOGICAL             :: L_MASTER ! W3_OASIS
  !
  !/
  !/ ------------------------------------------------------------------- /
  !/
  ...
  CALL W3SETI ( 1, 6, 6 )
  !
  call print_memcheck(740+IAPROC, 'memcheck_____:'//' WW3_SHEL SECTION 1')
  !
  ...
  !
  if (w3_nco_flag) then
     ! IF ( IAPROC .EQ. 1 ) CALL W3TAGB('WAVEFCST',1998,0007,0050,'NP21   ')
  end if
  ...
  call print_memcheck(740+IAPROC, 'memcheck_____:'//' WW3_SHEL SECTION 2a')
  !
  !--- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ! 2.  Define input fields
  !
  ...
  IF (FLGNML) THEN
     ...
     IF ( INFLAGS1(10) .AND. IAPROC.EQ.NAPOUT ) then
  ...
  IF (.NOT. FLGNML) THEN
     ...
     call print_memcheck(740+IAPROC, 'memcheck_____:'//' WW3_SHEL SECTION 2b')
     ...
     DO J = 1, NOTYPE
        ...
        ELSE
           ...
           !
           write(jchar, '(i0)') j
           call print_memcheck(740+IAPROC, 'memcheck_____:'//'WW3_SHEL NOTTYPE'//trim(jchar)) ! check
           !
           ! 2.5 Output types
           !
           IF ( ODAT(5*(J-1)+3) .NE. 0 ) THEN
  ...
  IF ( FLFLG ) THEN
     IF ( IAPROC .EQ. NAPOUT ) WRITE (NDSO,951)'Preparing input files ...'
     !
     DO J=JFIRST, 6
        if (w3_debuginit_flag) then
     ...
     DO J=7, 9
        IF ( INFLAGS1(J) .AND. .NOT. FLAGSC(J)) THEN
           CALL W3FLDO ('READ', IDSTR(J), NDSF(J), NDST, NDSEN, &
                RCLD(J), NY, NODATA(J), IERR, FPRE=TRIM(FNMPRE) )
           IF ( IERR .NE. 0 ) GOTO 2222
           IF ( IAPROC .EQ. NAPOUT ) WRITE (NDSO,956) IDFLDS(J), RCLD(J), NODATA(J)
        ELSE
           IF ( IAPROC .EQ. NAPOUT ) WRITE (NDSO,954) IDFLDS(J)
        END IF
     END DO
     !
  END IF ! FLFLG
  call print_memcheck(740+IAPROC, 'memcheck_____:'//' WW3_SHEL SECTION 4')
  ...
  END IF
  !
  call print_memcheck(740+IAPROC, 'memcheck_____:'//' WW3_SHEL SECTION 5')
  !
  !--- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ! 5.  Initializations
  !
  ...
  call print_memcheck(740+IAPROC, 'memcheck_____:'//' WW3_SHEL SECTION 6')
...
#ifdef W3_OASIS
     ! PROBLEM (MV) - optional arguments should always be called with keyword/value pairs
     ! The following has been modified to do this
     CALL W3WAVE ( 1, ODAT, TIMEN, STAMP=.TRUE., NO_OUT=.FALSE., ID_LCOMM=MPI_COMM, TIMEN=TIMEN)
#else
...
700 CONTINUE
  ...
  !
  call print_memcheck(740+IAPROC, 'memcheck_____:'//' WW3_SHEL SECTION 7')

  DO J=JFIRST,10
     !
     write(jchar, '(i0)') j
     call print_memcheck(740+IAPROC, 'memcheck_____:'//' WW3_SHEL update '//trim(jchar))
        ...
        ELSE
           ...
           IF ( DTOUT(7).NE.0 ) THEN
              ...
              ELSE
                 ...
                 IF ( NINT(MOD(DSEC21(TIME00,TIME), DTOUT(7))) .EQ. 0 .AND. DSEC21 (TFN(:,J), TIMEEND) .GT. 0.0 ) then
                    DTTST=0.
                 end IF
              ENDIF
           ENDIF
              ...
              ELSE
                 if (w3_oasicm_flag) then
                    ...
                    CALL W3FLDG ('READ', IDSTR(J), NDSF(J),         &
                         ...
                         IERR, FLAGSC=FLAGSC(J), COUPL_COMM=MPI_COMM)
                 else
                    CALL W3FLDG ('READ', IDSTR(J), NDSF(J),         &
                         NDST, NDSEN, NX, NY, NX, NY, TIME0, TIMEN, &
                         TTT, XXX, XXX, XXX, TI1, XXX, XXX, ICEP1,  &
                         IERR, FLAGSC=FLAGSC(J))
                 end if
              END IF ! IF (FLH(J)
  ...
  !
  call print_memcheck(740+IAPROC, 'memcheck_____:'//' WW3_SHEL SECTION 8')
  ...
  ! PROBLEM (MV) - optional arguments should always be called with keyword/value pairs
  ! The following has been modified to do this
  CALL W3WAVE ( 1, ODAT, TIME0, STAMP=.TRUE., NO_OUT=.FALSE., ID_LCOMM=MPI_COMM, TIMEN=TIMEN)
#else
  CALL W3WAVE ( 1, ODAT, TIME0)
#endif

  call print_memcheck(740+IAPROC, 'memcheck_____:'//' WW3_SHEL SECTION 9')
  !
  ...
  ! 7.e Check times
  !
  call print_memcheck(740+IAPROC, 'memcheck_____:'//' WW3_SHEL SECTION 10')
