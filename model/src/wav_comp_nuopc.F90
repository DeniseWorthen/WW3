...
contains
  ...
  subroutine InitializeRealize(gcomp, importState, exportState, clock, rc)
    ...
    !INGRID = (ITOUT.GT.0)
    ! returns indices of points used in interp, weights
    ...
    do jsea = 1,nseal_cpl
      ...
      do ipt = 1,10
        ...
        if (abs(xtin-xavg) .gt. 180.0.d0) then
          xtinmod = xtin-sign(360.0d0, (xtin-xavg))
        else
          xtinmod=xtin
        end if

        ! want a is_in_ungrid for single point instead of global
        ...
        if ((s1*sg1.ge.0.0d0).and.(s2*sg2.ge.0.0d0).and.(s3*sg3.ge.0.0.d0)) then
          !itout=itri
          !nbfound=nbfound+1
          !found = .true.
          lcnt = lcnt + 1
          ...
          rw(1,lcnt)=s1/sg1
          rw(2,lcnt)=s2/sg2
          rw(3,lcnt)=1.d0-rw(1)-rw(2)  !s3/sg3
          rw(4,lcnt)=0.d0
          write(1000+iaproc,'(2i8,2f10.2,i8,3i8,3g14.7)')ipt,gind,xtin,ytin,lcnt,is(lcnt,1:3),rw(lcnt,1:3)
