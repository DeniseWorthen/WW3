...
100 CONTINUE
  ...
  PURE SUBROUTINE WAVNU_LOCAL (SIG,DW,WNL,CGL)
    ...
    !/ ------------------------------------------------------------------- /
    !/
    ! PROBLEM(MV) - cannot call to STRACE here since this is a pure procedure
    ! removed the call
    !/
    !/ End of WAVNU_LOCAL------------------------------------------------- /
    !/
  ...
  SUBROUTINE LIU_FORWARD_DISPERSION (H_ICE,VISC,H_WDEPTH,SIGMA &
    ...
    !
    if (w3_t38_flag) then
       WRITE(*,*)'FORWARD OUT: K_SOLUTION,CG,ALPHA = ', K_SOLUTION,CG,ALPHA
       IF (H_ICE==1.0)THEN
          WRITE(*,*)FWANTED,ALPHA
       ENDIF
    end if
...
800 FORMAT (/' *** WAVEWATCH III ERROR IN '           &
  ...
  SUBROUTINE LIU_REVERSE_DISPERSION (H_ICE,VISC,H_WDEPTH,KWN &
    ...
    REAL              :: B          ! quantifies effect of bending of ice
    REAL              :: M          ! quantifies effect of inertia of ice
