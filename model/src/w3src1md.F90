...
CONTAINS
  ...
  SUBROUTINE W3SPR1 (A, CG, WN, EMEAN, FMEAN, WNMEAN, AMAX)
    ...
    ! Formats
    !
9000 FORMAT (' TEST W3SPR1 : E,F,WN MEAN ',3E10.3) ! W3_T
    !/
    !/ End of W3SPR1 ----------------------------------------------------- /
    !/
  ...
  SUBROUTINE W3SIN1 (A, K, USTAR, USDIR, S, D)
    ...
    INTEGER                 :: IS
    INTEGER, SAVE           :: IENT = 0 ! W3_S
    INTEGER                 :: IK, ITH ! W3_T0
    REAL                    :: COSU, SINU
    REAL                    :: DOUT(NK,NTH) ! W3_T0
    !/
    !/ ------------------------------------------------------------------- /
    !/
    if (w3_s_flag) then
    ...
    !
    if (w3_t0_flag) then
       ! PROBLEM (MV) ERROR in calling PRT2DS If the actual argument
       ! is scalar, the dummy argument shall be scalar unless the
       ! actual argument is of type character or is an element of an
       ! array that is not assumed shape, pointer, or polymorphic. [FR]
       ! This is now being caught because the ifdefs are not on
       ! Commenting the call for now
    ...
    ! Formats
    !
9000 FORMAT (' TEST W3SIN1 : COMMON FACT.: ',3E10.3) ! W3_T
    !/
    !/ End of W3SIN1 ----------------------------------------------------- /
    !/
  ...
  SUBROUTINE W3SDS1 (A, K, EMEAN, FMEAN, WNMEAN, S, D)
    ...
    ! Formats
    !
9000 FORMAT (' TEST W3SDS1 : COMMON FACT.: ',5E10.3) ! W3_T
    !/
    !/ End of W3SDS1 ----------------------------------------------------- /
    !/
