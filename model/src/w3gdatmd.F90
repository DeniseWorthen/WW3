...
MODULE W3GDATMD
  ...
  !
  INTEGER, PARAMETER      :: NIC4=10  ! only for W3_IC4
  ...
  !
  INTEGER,    PARAMETER   :: NKHS=2000, NKD=1300 ! only for W3_ST4
  INTEGER,    PARAMETER   :: NDTAB=2000          ! only for W3_ST4
  !/
  !/ Data structures
  !/
  !/ Grid type
  TYPE GRID  ! this is the geographical grid with all associated parameters
     ...
     INTEGER                    :: NITERSEC1                                          ! only if W3_SEC1
     INTEGER          , POINTER :: MAPSTA(:,:), MAPST2(:,:), MAPFS(:,:), MAPSF(:,:)
     !!Li     Cell and face arrays for SMC grid.
     INTEGER                    :: NCel, NUFc, NVFc, NRLv, MRFct                      ! only if W3_SMC
     INTEGER                    :: NGLO, NARC, NBGL, NBAC, NBSMC                      ! only if W3_SMC
     ...
     REAL                       :: PoLat, PoLon                                       ! Rotated N-Pole lat/lon - only if W3_RTD
     REAL             , POINTER :: AnglD(:)                                           ! Angle in degree - on if W3_RTD
     LOGICAL                    :: FLAGUNR                                            ! only if W3_RTD
     ...
     REAL             , POINTER :: CTRNX(:), CTRNY(:), CLATF(:)                       ! only if W3_SMC
     ...
     LOGICAL                    :: GUGINIT
     REAL             , POINTER :: REFLC(:,:)                                         ! reflection coefficient - only if W3_REF1
     INTEGER          , POINTER :: REFLD(:,:)                                         ! reflection direction - only if W3_REF1
     ...
     REAL             , POINTER :: SED_D50(:), SED_PSIC(:)                            ! only if W3_BT4
     LOGICAL          , POINTER :: RREF(:)                                            ! only if W3_REF1
     REAL,              POINTER :: REFPARS(:)                                         ! only if W3_REF1
     REAL,              POINTER :: IGPARS(:)                                          ! only if W3_IG1
     REAL,              POINTER :: IC2PARS(:)                                         ! only if W3_IC2
     REAL,              POINTER :: IC3PARS(:)                                         ! only if W3_IC3
     INTEGER,           POINTER :: IC4PARS(:)                                         ! only if W3_IC4
     REAL,              POINTER :: IC4_KI(:)                                          ! only if W3_IC4
     REAL,              POINTER :: IC4_FC(:)                                          ! only if W3_IC4
     REAL,              POINTER :: IC5PARS(:)                                         ! only if W3_IC5
     REAL,              POINTER :: IS2PARS(:)                                         ! only if W3_IC2
     ! unstructured data
     INTEGER                    :: NTRI
     INTEGER,           POINTER :: TRIGP(:,:)
     INTEGER                    :: NBND_MAP                                           ! only if W3_PDLIB
     INTEGER,           POINTER :: INDEX_MAP(:)                                       ! only if W3_PDLIB
     INTEGER,           POINTER :: MAPSTA_LOC(:)                                      ! only if W3_PDLIB
     INTEGER*1,         POINTER :: IOBPD_LOC(:,:)                                     ! only if W3_PDLIB
     INTEGER*2,         POINTER :: IOBP_LOC(:)                                        ! only if W3_PDLIB
     INTEGER*1,         POINTER :: IOBDP_LOC(:)                                       ! only if W3_PDLIB
     INTEGER*1,         POINTER :: IOBPA_LOC(:)                                       ! only if W3_PDLIB
     ...
     CHARACTER(LEN=256)         :: UOSTFILELOCAL, UOSTFILESHADOW                      ! only for W3_UOST
     LOGICAL, ALLOCATABLE       :: UOST_LCL_OBSTRUCTED(:,:), UOST_SHD_OBSTRUCTED(:,:) ! only for W3_UOST
     INTEGER*1, ALLOCATABLE     :: UOSTLOCALALPHA(:,:,:,:), UOSTLOCALBETA(:,:,:,:)    ! only for W3_UOST
     INTEGER*1, ALLOCATABLE     :: UOSTSHADOWALPHA(:,:,:,:), UOSTSHADOWBETA(:,:,:,:)  ! only for W3_UOST
     REAL*4, ALLOCATABLE        :: UOSTCELLSIZE(:,:,:)                                ! only for W3_UOST
     REAL                       :: UOSTABMULTFACTOR = 100                             ! only for W3_UOST
     REAL                       :: UOSTCELLSIZEFACTOR = 1000                          ! only for W3_UOST
     REAL                       :: UOSTLOCALFACTOR = 1                                ! only for W3_UOST
     REAL                       :: UOSTSHADOWFACTOR = 1                               ! only for W3_UOST
     LOGICAL                    :: UOSTENABLED = .true.                               ! only for W3_UOST
  ...
  TYPE NPAR
     ...
     REAL :: RWINDC   ! only for W3_RWND
     REAL :: WWCOR(2) ! only for W3_WCOR
  END TYPE NPAR
  !
  TYPE PROP
     REAL    :: DUMMY                      ! W3_PR0, W3_PR1
     REAL    :: DTME, CLATMN               ! W3_PR2
     REAL    :: WDCG, WDTH                 ! W3_PR3
     REAL    :: DTMS, Refran               ! W3_SMC
     LOGICAL :: FUNO3, FVERG, FSWND, ARCTC ! W3_SMC
  END TYPE PROP
  !
  TYPE FLDP
     REAL    :: DUMMY
     INTEGER :: Tail_ID                          ! W3_FLD1, W3_FLD2
     REAL    :: Tail_Lev, TAIL_TRAN1, TAIL_TRAN2 ! W3_FLD1, W3_FLD2
  END TYPE FLDP

  TYPE SFLP
     REAL    :: DUMMY  ! W3_FLX0, W3_FLX1
     INTEGER :: NITTIN ! W3_FLX2, W3_FLX3
     REAL    :: CINXSI ! W3_FLX2, W3_FLX3
     INTEGER :: CAP_ID ! W3_FLX3
     REAL    :: CD_MAX ! W3_FLX3
     REAL    :: FLX4A0 ! W3_FLX4
  END TYPE SFLP
  !
  TYPE SLNP
     REAL :: DUMMY             ! W3_SEED, W3_LN0
     REAL :: SLNC1, FSPM, FSHF ! W3_LN1
  END TYPE SLNP
  !
  TYPE SRCP
     REAL              :: WWNMEANPTAIL, SSTXFTFTAIL
     REAL              :: SINC1, SDSC1                                    ! W3_ST1
     REAL              :: ZWIND, FSWELL, SHSTAB                           ! W3_ST2
     REAL              :: OFSTAB, CCNG, CCPS, FFNG, FFPS                  ! W3_ST2
     REAL              :: CDSA0, CDSA1, CDSA2, SDSALN                     ! W3_ST2
     REAL              :: CDSB0, CDSB1, CDSB2, CDSB3, FPIMIN              ! W3_ST2
     REAL              :: XFH, XF1, XF2                                   ! W3_ST2
                                                                          !
     REAL              :: SSDSC1, SSDSC2, SSDSC3                          ! W3_ST3
     REAL              :: SSDSC4, SSDSC5, SSDSC6, DDELTA1                 ! W3_ST3
     REAL              :: DDELTA2                                         ! W3_ST3
     ...
     REAL              :: SSDSC(1:21)                                     ! W3_ST4
     REAL              :: FFXFA                                           ! W3_ST4
     REAL              :: SSDSBRF1, SSDSBRF2, SSDSBINT,SSDSBCK            ! W3_ST4
     REAL              :: SSDSHCK, SSDSABK, SSDSPBK, SSINBR               ! W3_ST4
     REAL              :: SSDSCOS, SSDSDTH, SSDSBT, SSDSBM(0:4)           ! W3_ST4
                                                                          !
     REAL              :: SIN6A0, SDS6A1, SDS6A2, SWL6B1                  ! W3_ST6
     REAL              :: SIN6WS, SIN6FC                                  ! W3_ST6
     INTEGER           :: SDS6P1, SDS6P2                                  ! W3_ST6
     LOGICAL           :: SDS6ET, SWL6S6, SWL6CSTB1                       ! W3_ST6
  END TYPE SRCP
  !
  TYPE SNLP
     REAL                  :: DUMMY                          ! W3_NL0
     REAL                  :: SNLC1, LAM, KDCON, KDMN        ! W3_NL1
     REAL                  :: SNLS1, SNLS2, SNLS3            ! W3_NL1
     INTEGER               :: IQTPE, NDPTHS                  ! W3_NL2
     REAL                  :: NLTAIL                         ! W3_NL2
     REAL, POINTER         :: DPTHNL(:)                      ! W3_NL2
     INTEGER               :: NFRMIN, NFRMAX, NFRCUT, NTHMAX ! W3_NL3
     INTEGER               :: NTHEXP, NSPMIN, NSPMAX, NSPMX2 ! W3_NL3
     INTEGER               :: NQA, SNLNQ                     ! W3_NL3
     INTEGER, POINTER      :: QST1(:,:,:), QST4(:,:,:)       ! W3_NL3
     REAL                  :: SNLMSC, SNLNSC, SNLSFD, SNLSFS ! W3_NL3
     REAL, POINTER         :: FRQ(:), XSI(:)                 ! W3_NL3
     REAL, POINTER         :: QST2(:,:,:), QST3(:,:,:)       ! W3_NL3
     REAL, POINTER         :: QST5(:,:,:), QST6(:,:,:)       ! W3_NL3
     REAL, POINTER         :: SNLL(:), SNLM(:), SNLT(:)      ! W3_NL3
     REAL, POINTER         :: SNLCD(:), SNLCS(:)             ! W3_NL3
     INTEGER               :: ITSA, IALT                     ! W3_NL4
     REAL                  :: QR5DPT, QR5OML                 ! W3_NL5
     INTEGER               :: QI5DIS, QI5KEV, QI5IPL, QI5PMX ! W3_NL5
     INTEGER(KIND=8)       :: QI5NNZ                         ! W3_NL5
     INTEGER               :: NTHX, NFRX, NSPL, NSPH         ! W3_NLS
     REAL                  :: CNLSA, CNLSC, CNLSFM           ! W3_NLS
     REAL                  :: CNLSC1, CNLSC2, CNLSC3         ! W3_NLS
     REAL, POINTER         :: SNSST(:,:)                     ! W3_NLS
  END TYPE SNLP
  !
  TYPE SBTP
     REAL :: DUMMY     ! W3_BT0, W3_BT8, W3_BT9
     REAL :: SBTC1     ! W3_BT1
     REAL :: SBTCX(10) ! W3_BT4
  END TYPE SBTP
  !
  TYPE SDBP
     REAL    :: DUMMY        ! W3_DB0
     REAL    :: SDBC1, SDBC2 ! W3_DB1
     LOGICAL :: FDONLY       ! W3_DB1
     REAL    :: SDBSC        ! W3_DB1
  END TYPE SDBP

  TYPE UOSTP
     CHARACTER(LEN=256) :: UOSTFILELOCAL, UOSTFILESHADOW     ! W3_UOST
     REAL               :: UOSTFACTORLOCAL, UOSTFACTORSHADOW ! W3_UOST
  END TYPE UOSTP
  !
  TYPE STRP
     REAL :: DUMMY ! W3_TR0, W3_TR1
  END TYPE STRP
  !
  TYPE SBSP
     REAL :: DUMMY ! W3_BS0, W3_BS1
  END TYPE SBSP
  !
  TYPE SICP
     REAL :: DUMMY ! W3_IS0
     REAL :: IS1C1, IS1C2 ! W3_IS1, W3_IS2
     REAL :: IS2C1, IS2C2
  END TYPE SICP
  ...
  REAL,    POINTER :: REFLC(:,:) !  W3_REF1
  INTEGER, POINTER :: REFLD(:,:) !  W3_REF1
  ...
  INTEGER,           POINTER :: TRIGP(:,:)
  INTEGER,           POINTER :: NBND_MAP       ! W3_PDLIB
  INTEGER,           POINTER :: INDEX_MAP(:)   ! W3_PDLIB
  INTEGER,           POINTER :: MAPSTA_LOC(:)  ! W3_PDLIB
  INTEGER*1,         POINTER :: IOBPD_LOC(:,:) ! W3_PDLIB
  INTEGER*2,         POINTER :: IOBP_LOC(:)    ! W3_PDLIB
  INTEGER*1,         POINTER :: IOBDP_LOC(:)   ! W3_PDLIB
  INTEGER*1,         POINTER :: IOBPA_LOC(:)   ! W3_PDLIB
  ...
  LOGICAL,           POINTER ::  RREF(:)       ! W3_REF1
  REAL,              POINTER :: REFPARS(:)     ! W3_REF1
  REAL,              POINTER :: IGPARS(:)      ! W3_IG1
  REAL,              POINTER :: IC2PARS(:)     ! W3_IC2
  REAL,              POINTER :: IC3PARS(:)     ! W3_IC3
  INTEGER,           POINTER :: IC4PARS(:)     ! W3_IC4
  REAL,              POINTER :: IC4_KI(:)      ! W3_IC4
  REAL,              POINTER :: IC4_FC(:)      ! W3_IC4
  REAL,              POINTER :: IC5PARS(:)     ! W3_IC5
  REAL,              POINTER :: IS2PARS(:)     ! W3_IS2
  INTEGER,           POINTER :: MAPSTA(:,:), MAPST2(:,:)
  INTEGER,           POINTER :: MAPFS(:,:), MAPSF(:,:)
                                               !
  INTEGER,           POINTER :: NCel, NUFc, NVFc, NRLv, MRFct         ! W3_SMC
  INTEGER,           POINTER :: NGLO, NARC, NBGL, NBAC, NBSMC         ! W3_SMC
  ...
  REAL,              POINTER :: PoLat, PoLon ! W3_RTD
  REAL,              POINTER :: AnglD(:)     ! W3_RTD
  LOGICAL,           POINTER :: FLAGUNR      ! W3_RTD
  REAL   ,           POINTER :: ZB(:)
  REAL   ,           POINTER :: CLATS(:)
  REAL   ,           POINTER :: CLATIS(:) ! INVERSE OF COS(LAT) DEFINED ON ISEA
  REAL   ,           POINTER :: CTHG0S(:) ! TAN(Y)/R, DEFINED ON ISEA
  ...
  REAL   ,           POINTER :: TRNX(:,:), TRNY(:,:)         ! TRANSPARENCY INFORMATION ON IX,IY
  REAL,              POINTER :: CTRNX(:), CTRNY(:), CLATF(:) ! W3_SMC
  REAL   ,           POINTER :: SPCBAC(:,:), ANGARC(:)
  DOUBLE PRECISION,  POINTER :: XGRD(:,:), YGRD(:,:)         ! X AND Y DEFINED ON IX,IY
  REAL   ,           POINTER :: DXDP(:,:), DXDQ(:,:)         ! DX/DP & DX/DQ DEFINED ON IX,IY
  REAL   ,           POINTER :: DYDP(:,:), DYDQ(:,:)         ! DY/DP & DY/DQ DEFINED ON IX,IY
  ...
  REAL   ,           POINTER :: HPFAC(:,:)                   ! H_P = SQRT(G_PP) DEFINED ON IX,IY
  REAL   ,           POINTER :: HQFAC(:,:)                   ! H_Q = SQRT(G_QQ) DEFINED ON IX,IY
  REAL,              POINTER :: SED_D50(:), SED_PSIC(:)      ! W3_BT4
  ...
  REAL, POINTER           :: RWINDC   ! W3_RWND
  REAL, POINTER           :: WWCOR(:) ! W3_WCOR
  !/
  !/ Data aliasses for structure PROP(S)
  !/
  REAL, POINTER           :: DTME, CLATMN               ! W3_PR2
  REAL, POINTER           :: WDCG, WDTH                 ! W3_PR3
  REAL, POINTER           :: DTMS, Refran               ! W3_SMC
  LOGICAL, POINTER        :: FUNO3, FVERG, FSWND, ARCTC ! W3_SMC
  !/
  !/ Data aliasses for structure FLDP(S)
  !/
  INTEGER, POINTER         :: TAIL_ID                          ! W3_FLD1, W3_FLD2
  REAL, POINTER            :: TAIL_LEV, TAIL_TRAN1, TAIL_TRAN2 ! W3_FLD1, W3_FLD2
  !/
  !/ Data aliasses for structure SFLP(S)
  !/
  INTEGER, POINTER        :: NITTIN ! W3_FLX2, W3_FLX3
  REAL, POINTER           :: CINXSI ! W3_FLX2, W3_FLX3
  INTEGER, POINTER        :: CAP_ID ! W3_FLX3
  REAL, POINTER           :: CD_MAX ! W3_FLX3
  REAL, POINTER           :: FLX4A0 ! W3_FLX4
  !/
  ...
  !/
  REAL,      POINTER :: SINC1, SDSC1                                    ! W3_ST1
  !
  REAL,      POINTER :: ZWIND, FSWELL, SHSTAB                           ! W3_ST2
  REAL,      POINTER :: OFSTAB, CCNG, CCPS, FFNG, FFPS                  ! W3_ST2
  REAL,      POINTER :: CDSA0, CDSA1, CDSA2, SDSALN                     ! W3_ST2
  REAL,      POINTER :: CDSB0, CDSB1, CDSB2, CDSB3, FPIMIN              ! W3_ST2
  REAL,      POINTER :: XFH, XF1, XF2                                   ! W3_ST2
  !
  REAL,      POINTER :: SSDSC1, SSDSC2, SSDSC3                          ! W3_ST3
  REAL,      POINTER :: SSDSC4, SSDSC5, SSDSC6                          ! W3_ST3
  REAL,      POINTER :: DDELTA1, DDELTA2                                ! W3_ST3
  !
  REAL,      POINTER :: ZZWND, AALPHA, BBETA, ZZ0MAX, ZZ0RAT            ! W3_ST3, W3_ST4
  REAL,      POINTER :: SSWELLF(:), SSINTHP,  TTAUWSHELTER              ! W3_ST3, W3_ST4
  REAL,      POINTER :: ZZALP                                           ! W3_ST3, W3_ST4
  REAL,      POINTER :: FFXFM, FFXPM                                    ! W3_ST3, W3_ST4
  REAL,      POINTER :: SSDSCOS, SSDSDTH, SSDSBM(:)                     ! W3_ST3, W3_ST4
  REAL,      POINTER :: SSDSBT, SSDSBR                                  ! W3_ST3, W3_ST4
  REAL,      POINTER :: SSDSP, WWNMEANP, SSTXFTF, SSTXFTWN              ! W3_ST3, W3_ST4
  !
  INTEGER,   POINTER :: SSWELLFPAR, SSDSISO, SSDSBRFDF                  ! W3_ST4
  INTEGER,   POINTER :: IKTAB(:,:), SATINDICES(:,:),SSDSDIK             ! W3_ST4
  REAL,      POINTER :: DCKI(:,:), SATWEIGHTS(:,:),CUMULW(:,:),QBI(:,:) ! W3_ST4
  REAL,      POINTER :: FFXFA                                           ! W3_ST4
  REAL,      POINTER :: SSDSBRF1, SSDSBRF2                              ! W3_ST4
  REAL,      POINTER :: SSDSBINT, SSDSBCK, SSDSHCK, SSDSABK             ! W3_ST4
  REAL,      POINTER :: SSDSPBK, SSINBR                                 ! W3_ST4
  REAL,      POINTER :: SSDSC(:)                                        ! W3_ST4
  !
  REAL,      POINTER :: SIN6A0, SDS6A1, SDS6A2, SWL6B1                  ! W3_ST6
  REAL,      POINTER :: SIN6WS, SIN6FC                                  ! W3_ST6
  INTEGER,   POINTER :: SDS6P1, SDS6P2                                  ! W3_ST6
  LOGICAL,   POINTER :: SDS6ET, SWL6S6, SWL6CSTB1                       ! W3_ST6
  !
  REAL     , POINTER :: WWNMEANPTAIL, SSTXFTFTAIL
  !/
  !/ Data aliasses for structure SNLP(S)
  !/
  REAL,            POINTER :: SNLC1, LAM, KDCON, KDMN            ! W3_NL1
  REAL,            POINTER :: SNLS1, SNLS2, SNLS3                ! W3_NL1
  INTEGER,         POINTER :: IQTPE, NDPTHS                      ! W3_NL2
  REAL,            POINTER :: NLTAIL                             ! W3_NL2
  REAL,            POINTER :: DPTHNL(:)                          ! W3_NL2
  INTEGER,         POINTER :: NFRMIN, NFRMAX, NFRCUT, NTHMAX     ! W3_NL3
  INTEGER,         POINTER :: NTHEXP, NSPMIN, NSPMAX, NSPMX2     ! W3_NL3
  INTEGER,         POINTER :: NQA, SNLNQ                         ! W3_NL3
  INTEGER,         POINTER :: QST1(:,:,:), QST4(:,:,:)           ! W3_NL3
  REAL,            POINTER :: SNLMSC, SNLNSC, SNLSFD, SNLSFS     ! W3_NL3
  REAL,            POINTER :: FRQ(:), XSI(:)                     ! W3_NL3
  REAL,            POINTER :: QST2(:,:,:), QST3(:,:,:)           ! W3_NL3
  REAL,            POINTER :: QST5(:,:,:), QST6(:,:,:)           ! W3_NL3
  REAL,            POINTER :: SNLL(:), SNLM(:), SNLT(:)          ! W3_NL3
  REAL,            POINTER :: SNLCD(:), SNLCS(:)                 ! W3_NL3
  INTEGER,         POINTER :: ITSA, IALT                         ! W3_NL4
  REAL,            POINTER :: QR5DPT, QR5OML                     ! W3_NL5
  INTEGER,         POINTER :: QI5DIS, QI5KEV, QI5IPL, QI5PMX     ! W3_NL5
  INTEGER(KIND=8), POINTER :: QI5NNZ                             ! W3_NL5
  INTEGER,         POINTER :: NTHX, NFRX, NSPL, NSPH             ! W3_NLS
  REAL,            POINTER :: CNLSA, CNLSC, CNLSFM               ! W3_NLS
  REAL,            POINTER :: CNLSC1, CNLSC2, CNLSC3, SNSST(:,:) ! W3_NLS
  !/
  !/ Data aliasses for structure SBTP(S)
  !/
  REAL, POINTER :: SBTC1    ! W3_BT1
  REAL, POINTER :: SBTCX(:) ! W3_BT4
  !/
  !/ Data aliasses for structure SDBP(S)
  !/
  REAL,    POINTER :: SDBC1, SDBC2 ! W3_DB1
  LOGICAL, POINTER :: FDONLY       ! W3_DB1
  REAL,    POINTER :: SDBSC        ! W3_DB1
  !/
  !/ Data aliases for structure UOSTP(S)
  CHARACTER(LEN=:), POINTER   :: UOSTFILELOCAL, UOSTFILESHADOW ! W3_UOST
  REAL, POINTER           :: UOSTFACTORLOCAL, UOSTFACTORSHADOW ! W3_UOST
  !/
  ...
  REAL, POINTER           :: IS1C1, IS1C2 ! W3_IS1
  !/
CONTAINS
  !/ ------------------------------------------------------------------- /
...
9000 FORMAT (' TEST W3NMOD : SETTING UP FOR GRIDS ',I3,           &
  ...
  SUBROUTINE W3DIMX  ( IMOD, MX, MY, MSEA, NDSE, NDST, &
    ...
    !/ Local parameters
    !/
    INTEGER                 :: IARC, IBAC, IBSMC ! W3_SMC
    INTEGER, SAVE           :: IENT = 0  ! W3_S
    !/
    if (w3_s_flag) then
    ...
    IF (GTYPE .NE. UNGTYPE) THEN
       ALLOCATE ( GRIDS(IMOD)%ZB(MSEA) ,STAT=ISTAT)
       CHECK_ALLOC_STATUS ( ISTAT )
       ALLOCATE ( GRIDS(IMOD)%XGRD(MY  ,MX) , STAT=ISTAT)
       CHECK_ALLOC_STATUS ( ISTAT )
       ALLOCATE ( GRIDS(IMOD)%YGRD(MY  ,MX) , STAT=ISTAT)
       CHECK_ALLOC_STATUS ( ISTAT )
    ENDIF

    ALLOCATE ( GRIDS(IMOD)%MAPSTA(MY,MX), STAT=ISTAT)
    CHECK_ALLOC_STATUS ( ISTAT )
    ALLOCATE ( GRIDS(IMOD)%MAPST2(MY,MX), STAT=ISTAT)
    CHECK_ALLOC_STATUS ( ISTAT )
    ALLOCATE ( GRIDS(IMOD)%MAPFS(MY,MX), STAT=ISTAT)
    CHECK_ALLOC_STATUS ( ISTAT )
    ALLOCATE ( GRIDS(IMOD)%MAPSF(MSEA,3), STAT=ISTAT)
    CHECK_ALLOC_STATUS ( ISTAT )
    ALLOCATE ( GRIDS(IMOD)%FLAGST(MSEA), STAT=ISTAT)
    if (w3_rtd_flag) then
       allocate(GRIDS(IMOD)%AnglD(MSEA))
       CHECK_ALLOC_STATUS ( ISTAT )
    end if
    ALLOCATE ( GRIDS(IMOD)%CLATS(0:MSEA),  STAT=ISTAT)
    CHECK_ALLOC_STATUS ( ISTAT )
    ALLOCATE ( GRIDS(IMOD)%CLATIS(0:MSEA), STAT=ISTAT)
    CHECK_ALLOC_STATUS ( ISTAT )
    ALLOCATE ( GRIDS(IMOD)%CTHG0S(0:MSEA), STAT=ISTAT)
    CHECK_ALLOC_STATUS ( ISTAT )
    ALLOCATE ( GRIDS(IMOD)%TRNX(MY,MX),    STAT=ISTAT)
    CHECK_ALLOC_STATUS ( ISTAT )
    ALLOCATE ( GRIDS(IMOD)%TRNY(MY,MX),    STAT=ISTAT)
    CHECK_ALLOC_STATUS ( ISTAT )
    ALLOCATE ( GRIDS(IMOD)%DXDP(MY,MX),    STAT=ISTAT)
    CHECK_ALLOC_STATUS ( ISTAT )
    ALLOCATE ( GRIDS(IMOD)%DXDQ(MY,MX),    STAT=ISTAT)
    CHECK_ALLOC_STATUS ( ISTAT )
    ALLOCATE ( GRIDS(IMOD)%DYDP(MY,MX),    STAT=ISTAT)
    CHECK_ALLOC_STATUS ( ISTAT )
    ALLOCATE ( GRIDS(IMOD)%DYDQ(MY,MX),    STAT=ISTAT)
    CHECK_ALLOC_STATUS ( ISTAT )
    ALLOCATE ( GRIDS(IMOD)%DPDX(MY,MX),    STAT=ISTAT)
    CHECK_ALLOC_STATUS ( ISTAT )
    ALLOCATE ( GRIDS(IMOD)%DPDY(MY,MX),    STAT=ISTAT)
    CHECK_ALLOC_STATUS ( ISTAT )
    ALLOCATE ( GRIDS(IMOD)%DQDX(MY,MX),    STAT=ISTAT)
    CHECK_ALLOC_STATUS ( ISTAT )
    ALLOCATE ( GRIDS(IMOD)%DQDY(MY,MX),    STAT=ISTAT)
    CHECK_ALLOC_STATUS ( ISTAT )
    ALLOCATE ( GRIDS(IMOD)%GSQRT(MY,MX),   STAT=ISTAT)
    CHECK_ALLOC_STATUS ( ISTAT )
    ALLOCATE ( GRIDS(IMOD)%HPFAC(MY,MX),   STAT=ISTAT)
    CHECK_ALLOC_STATUS ( ISTAT )
    ALLOCATE ( GRIDS(IMOD)%HQFAC(MY,MX),   STAT=ISTAT)
    CHECK_ALLOC_STATUS ( ISTAT )

    !!/DEBUGINIT  WRITE(740+IAPROC,*) 'After alocation of MAPST2, MY=', MY, ' MX=', MX
    !!/DEBUGINIT FLUSH(740+IAPROC)
    if (w3_bt4_flag) then
       ALLOCATE ( GRIDS(IMOD)%SED_D50(0:MSEA), STAT=ISTAT)
       CHECK_ALLOC_STATUS ( ISTAT )
       ALLOCATE ( GRIDS(IMOD)%SED_PSIC(0:MSEA), STAT=ISTAT)
       CHECK_ALLOC_STATUS ( ISTAT )
    end if
    !
    if (w3_smc_flag) then
    ...
    if (w3_ref1_flag) then
       ALLOCATE ( GRIDS(IMOD)%RREF(4),     STAT=ISTAT)
       CHECK_ALLOC_STATUS ( ISTAT )
       ALLOCATE ( GRIDS(IMOD)%REFPARS(10), STAT=ISTAT)
       CHECK_ALLOC_STATUS ( ISTAT )
       ...
       ALLOCATE ( GRIDS(IMOD)%REFLC(4,0:NSEA), STAT=ISTAT)
       CHECK_ALLOC_STATUS ( ISTAT )
       ALLOCATE ( GRIDS(IMOD)%REFLD(6,0:NSEA), STAT=ISTAT)
       CHECK_ALLOC_STATUS ( ISTAT )
    end if
...
9003 FORMAT (' TEST W3DIMX : POINTERS RESET')
  ...
  SUBROUTINE W3DIMS  ( IMOD, MK, MTH, NDSE, NDST )
    ...
    INTEGER                 :: SDSNTH ! only for W3_ST4
       ...
       DO IY=2, NY-1
          DO IX=2, NX-1
             ...
             IF (MAPSTA(IY,IX).GT.0) THEN
                ...
                END IF  ! IF (RREF(1)) THEN
             END IF  ! IF (MAPSTA(IY,IX).GT.0) THEN
          END DO  ! DO IX=2, NX-1
       END DO  ! DO IY=2, NY-1
    end if
    !
