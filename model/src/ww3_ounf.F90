...
PROGRAM W3OUNF
  ...
  USE W3WDATMD     , ONLY : UST, USTDIR, RHOAIR
#ifdef W3_SETUP
  USE W3WDATMD     , ONLY : ZETA_SETUP
#endif
  USE W3ADATMD     , ONLY : DW, UA, UD, AS, CX, CY, HS, WLM, T0M1, THM
  USE W3ADATMD     , ONLY : THS, FP0, THP0, DTDYN, FCUT
  USE W3ADATMD     , ONLY : ABA, ABD, UBA, UBD, SXX, SYY, SXY, USERO
  USE W3ADATMD     , ONLY : PHS, PTP, PLP, PDIR, PSI, PWS, PWST, PNR
  USE W3ADATMD     , ONLY : PTM1, PT1, PT2, PEP, TAUOCX, TAUOCY
  USE W3ADATMD     , ONLY : PTHP0, PQP, PSW, PPE, PGW, QP
  USE W3ADATMD     , ONLY : TAUOX, TAUOY, TAUWIX
  USE W3ADATMD     , ONLY : TAUWIY, PHIAW, PHIOC, TUSX, TUSY, PRMS, TPMS
  USE W3ADATMD     , ONLY : USSX, USSY, MSSX, MSSY, MSSD, MSCX, MSCY
  USE W3ADATMD     , ONLY : MSCD, CHARN, TWS, TAUA, TAUADIR
  USE W3ADATMD     , ONLY : TAUWNX, TAUWNY, BHD, T02, HSIG, CGE
  USE W3ADATMD     , ONLY : T01, BEDFORMS, WHITECAP, TAUBBL, PHIBBL
  USE W3ADATMD     , ONLY : CFLTHMAX, CFLXYMAX, CFLKMAX, TAUICE, PHICE
  USE W3ADATMD     , ONLY : STMAXE, STMAXD, HMAXE, HCMAXE, HMAXD, HCMAXD
  USE W3ADATMD     , ONLY : P2SMS, EF, US3D, TH1M, STH1M, TH2M, STH2M
  USE W3ADATMD     , ONLY : WN, USSP, WBT, WNMEAN
  USE W3ODATMD     , ONLY : NDSO, NDSE, SCREEN, NOGRP, NGRPP, IDOUT
  USE W3ODATMD     , ONLY : UNDEF, FLOGRD, FNMPRE, NOSWLL, NOGE
  !
  USE W3NMLOUNFMD
  !
  USE W3OUNFMETAMD , ONLY : INIT_META, TEARDOWN_META, GETMETA
  USE W3OUNFMETAMD , ONLY : WRITE_META, WRITE_GLOBAL_META
  USE W3OUNFMETAMD , ONLY : WRITE_FREEFORM_META_LIST
  USE W3OUNFMETAMD , ONLY : META_T, NCVARTYPE, CRS_META, CRS_NAME
  USE W3OUNFMETAMD , ONLY : FL_DEFAULT_GBL_META, COORDS_ATTR
  !
  USE NETCDF

#ifdef W3_SMC
  USE W3SMCOMD     , ONLY : SMCNOVAL=>NOVAL
#endif
...
CONTAINS
  ...
  SUBROUTINE W3EXNC ( NX, NY, IX1, IXN, IY1, IYN, NSEA,             &
    ...
    USE W3GDATMD , ONLY : XGRD, YGRD, SX, SY, X0, Y0, TRIGP, USSP_WN
#ifdef W3_RTD
