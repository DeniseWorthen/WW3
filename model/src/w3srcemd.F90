...
CONTAINS
  ...
  SUBROUTINE W3SRCE ( srce_call, IT, ISEA, JSEA, IX, IY, IMOD,    &
    ...
    USE W3FLD1MD  , ONLY : W3FLD1                                      ! W3_FLD1
    USE W3GDATMD  , ONLY : AALPHA                                      ! W3_FLD1
    USE W3FLD2MD  , ONLY : W3FLD2                                      ! W3_FLD2
    USE W3GDATMD  , ONLY : AALPHA                                      ! W3_FLD2
    USE W3FLX1MD                                                       ! W3_FLX1
    USE W3FLX2MD                                                       ! W3_FLX2
    USE W3FLX3MD                                                       ! W3_FLX3
    USE W3FLX4MD                                                       ! W3_FLX4
    USE W3FLX5MD                                                       ! W3_FLX5
    USE W3SLN1MD                                                       ! W3_LN1
    USE W3SRC0MD                                                       ! W3_ST0
    USE W3GDATMD  , ONLY : ZWIND                                       ! W3_ST2
    USE W3GDATMD  , ONLY : ZZWND, FFXFM, FFXPM                         ! W3_ST3
    USE W3GDATMD  , ONLY : ZZWND, FFXFM, FFXPM, FFXFA                  ! W3_ST4
    USE W3GDATMD  , ONLY : SWL6S6                                      ! W3_ST6
    USE W3TIMEMD  , ONLY : TICK21
    USE W3GDATMD  , ONLY : IS2PARS                                     ! W3_IS2
#ifdef W3_ST1
    USE W3SRC1MD ! W3_ST1 (TODO: add this to compilation - removed ifdefs)
#endif
#ifdef W3_ST2
    USE W3SRC2MD
#endif
    ...
    REAL               :: EMEAN, FMEAN, AMAX, CD, Z0, SCAT, SMOOTH_ICEDISP
    ...
    REAL               :: MWXFINISH, MWYFINISH, A1BAND, B1BAND,  COSI(2)
    ...
    REAL               :: VSIR(NSPEC), VDIR(NSPEC)              ! W3_IS1
    REAL               :: VSIR(NSPEC), VDIR(NSPEC),VDIR2(NSPEC) ! W3_IS2
    REAL               :: VSUO(NSPEC), VDUO(NSPEC)              ! W3_UOST
    REAL               :: VS(NSPEC), VD(NSPEC), EB(NK)
    LOGICAL            :: LLWS(NSPEC)                           ! W3_ST3
    LOGICAL            :: LLWS(NSPEC)                           ! W3_ST4
    ...
    CHARACTER(LEN=17), SAVE :: FNAME = 'test_data_nnn.ww3' ! W3_NNT
    !/
    !/ ------------------------------------------------------------------- /
    !/
    if (w3_s_flag) then
...
#endif
#ifdef W3_FLX2
    CALL W3FLX2 ( ZWND, DEPTH, FP, U10ABS, U10DIR, USTAR, USTDIR, Z0, CD )
#endif
#ifdef W3_FLX3
    CALL W3FLX3 ( ZWND, DEPTH, FP, U10ABS, U10DIR, USTAR, USTDIR, Z0, CD )
#endif
#ifdef W3_FLX4
    CALL W3FLX4 ( ZWND, U10ABS, U10DIR, USTAR, USTDIR, Z0, CD )
#endif
#ifdef W3_FLX5
    CALL W3FLX5 ( ZWND, U10ABS, U10DIR, TAUA, TAUADIR, DAIR, USTAR, USTDIR, Z0, CD, CHARN )
#endif
    !
    ...
    if (w3_nnt_flag) then
       IF ( IT .EQ. 0 ) THEN
          ...
          OPEN (NDSD2,FILE=FNMPRE(:J)//'time.ww3', FORM='FORMATTED',ERR=800,IOSTAT=IERR)
       END IF
    end if
    !
    ...
    DO
...
#endif
#ifdef W3_ST2
       CALL W3SIN2 ( SPEC, CG1, WN2, U10ABS, U10DIR, CD, Z0, FPI, VSIN, VDIN )
#endif
#ifdef W3_ST3
       CALL W3SIN3 ( SPEC, CG1, WN2, U10ABS, USTAR, DRAT, AS, &
            U10DIR, Z0, CD, TAUWX, TAUWY, TAUWAX, TAUWAY,     &
       ...
       CALL W3SIN6 ( SPEC, CG1, WN2, U10ABS, USTAR, USTDIR, CD, &
            DAIR, TAUWX, TAUWY, TAUWAX, TAUWAY, VSIN, VDIN )
#endif
       !
       ...
       !
#ifdef W3_TR1
       if ((w3_pdlib_flag .and. (.NOT. FSSOURCE .or. LSLOC)) .or. (.not. w3_pdlib_flag)) then
...
! End of ifdef PDLIB
       ...
       if (w3_st1_flag) then
          ...
          NKH    = MAX ( 2 , MIN ( NKH1 , INT ( FACTI2 + FACTI1*LOG(MAX(1.E-7,FHIGH)) ) ) )
       end if
       !
       if (w3_st1_flag) then
