...
CONTAINS
  ...
  SUBROUTINE W3SRCE ( srce_call, IT, ISEA, JSEA, IX, IY, IMOD,          &
    ...
    !
    !/ ------------------------------------------------------------------- /
    ! PROBLEM(MV) DEBUG_NODE is used below but was never included from constants - fixing this
    ! PROBLEM(MV) file_endian is used below but was not included from constants - fixing this
    USE CONSTANTS , ONLY: DEBUG_NODE, file_endian
    USE CONSTANTS , ONLY: DWAT, srce_imp_post, srce_imp_pre
    USE CONSTANTS , ONLY: srce_direct, GRAV, TPI, TPIINV, LPDLIB, RADE
    ...
    USE W3GDATMD  , ONLY: FTE, FTF, FHMAX, ECOS, ESIN, IICEDISP, ICESCALES, IICESMOOTH
    USE W3GDATMD  , ONLY: FSSOURCE, optionCall
    USE W3GDATMD  , ONLY: B_JGS_NLEVEL, B_JGS_SOURCE_NONLINEAR
    USE W3GDATMD  , ONLY: IOBP, IOBPD, IOBDP, GTYPE, UNGTYPE, REFPARS ! W3_REF1
    USE W3GDATMD  , ONLY: AALPHA                                      ! W3_FLD1, W3_FLD2
    USE W3GDATMD  , ONLY: ZWIND                                       ! W3_ST2
    USE W3GDATMD  , ONLY: IS2PARS                                     ! W3_IS2
    USE W3GDATMD  , ONLY: IGPARS                                      ! W3_IG1
    USE W3GDATMD  , ONLY: IOBP_LOC, IOBPD_LOC, IOBPA_LOC, IOBDP_LOC   ! W3_PDLIB
    USE W3ODATMD  , ONLY: NDSE, NDST, IAPROC
    USE W3ODATMD  , ONLY: IAPROC, SCREEN, FNMPRE                      ! W3_NNT
    USE W3PARALL  , ONLY: ONESIXTH, ZERO, THR, IMEM, LSLOC            ! W3_PDLIB
    USE W3IDATMD  , ONLY: INFLAGS2, ICEP2
    USE W3TIMEMD  , ONLY: TICK21
    USE W3WDATMD  , ONLY: TIME
    USE W3WDATMD  , ONLY: VA                                          ! W3_PDLIB
    ! PROBLEM(MV) EXTCDE is used below but was never included form w3servmd - fixing this
    ...
    INTEGER            :: IENT = 0                     ! W3_S
    INTEGER            :: NDSD = 89, NDSD2 = 88, J     ! W3_NNT
    INTEGER            :: QI5TSTART(2)                 ! W3_NL5
    REAL               :: QR5KURT
    ...
    REAL               :: FH1, FH2                     ! W3_ST1, W3_ST3, W3_ST4
    REAL               :: FHTRAN, DFH, FACDIA, FACPAR  ! W3_ST2
    REAL               :: FMEANS                       ! W3_ST3, W3_ST4
    REAL               :: FAGE, DLWMEAN                ! W3_ST4
    REAL               :: QCERR  = 0.                  !/XNL2 and !/NNT
    REAL               :: UC, SLEV                     ! W3_SEED
    REAL               :: HM, EM                       ! W3_MLIM
    REAL               :: FACNN                        ! W3_NNT
    REAL               :: DTRAW                        ! W3_T
    ...
    REAL               :: VSWL(NSPEC), VDWL(NSPEC)     ! W3_ST6
    REAL               :: VSBT(NSPEC), VDBT(NSPEC)
    REAL               :: VSIC(NSPEC), VDIC(NSPEC)     ! W3_IC1, W3_IC2, W3_IC3, W3_IC4, W3_IC5
    REAL               :: VSDB(NSPEC), VDDB(NSPEC)     ! W3_DB1
    REAL               :: VSTR(NSPEC), VDTR(NSPEC)     ! W3_TR1
    REAL               :: VSBS(NSPEC), VDBS(NSPEC)     ! W3_BS1
    REAL               :: VREF(NSPEC)                  ! W3_REF1
    REAL               :: VSIR(NSPEC), VDIR(NSPEC)     ! W3_IS1, W3_IS2
    REAL               :: VDIR2(NSPEC)                 ! W3_IS2
    REAL               :: VSUO(NSPEC), VDUO(NSPEC)     ! W3_UOST
    REAL               :: VS(NSPEC), VD(NSPEC), EB(NK) ! W3_UOST
    LOGICAL            :: LLWS(NSPEC)                  ! W3_ST3, W3_ST4
    REAL               :: BRLAMBDA(NSPEC)              ! W3_ST4
    DOUBLE PRECISION   :: SCATSPEC(NTH)                ! W3_IS2
    REAL               :: FOUT(NK,NTH), SOUT(NK,NTH), DOUT(NK,NTH)
    !
    REAL, SAVE         :: TAUNUX, TAUNUY
    !$omp threadprivate( TAUNUX, TAUNUY) !??? why is this here ??? 
    !
    LOGICAL          :: FLTEST = .FALSE., FLAGNN = .TRUE.
    !$omp threadprivate( FLTEST, FLAGNN ) !??? why is this here ??? 
    !
    LOGICAL          :: SHAVE
    LOGICAL          :: LBREAK
    !
    LOGICAL          :: FIRST = .TRUE.
    !$omp threadprivate( FIRST )  !??? why is this here ??? 
    !
    ...
    REAL             :: PreVS, FAK, DVS, SIDT, FAKS, MAXDAC ! W3_PDLIB
    CHARACTER(LEN=17) :: FNAME = 'test_data_nnn.ww3' ! W3_NNT
    !/
    !/ ------------------------------------------------------------------- /
    !/
    if (w3_s_flag) then
    ...
    IF ( IT .eq. 0 ) THEN
       ...
       USTDIR=0.
    ELSE
       CALL W3SPR4 (SPEC, CG1, WN1, EMEAN, FMEAN, FMEAN1, WNMEAN, AMAX, U10ABS, U10DIR, &
#ifdef W3_FLX5
                    TAUA, TAUADIR, DAIR, &
#endif
                    USTAR, USTDIR,                                  &
    ...
    if (w3_nnt_flag) then
       IF ( IT .EQ. 0 ) THEN
          ...
          OPEN (NDSD,FILE=FNMPRE(:J)//FNAME,form='UNFORMATTED', convert=file_endian, ERR=800,IOSTAT=IERR)
    ...
    DO
       ...
       if (w3_pdlib_flag) then
          IF (.NOT. FSSOURCE .or. LSLOC) THEN
#ifdef W3_DB1
             CALL W3SDB1 ( IX, SPEC, DEPTH, EMEAN, FMEAN, WNMEAN, CG1, LBREAK, VSDB, VDDB )
#endif
          end IF
       else
#ifdef W3_DB1
...
#endif
#ifdef W3_BT4
       CALL W3SBT4 ( SPEC, CG1, WN1, DEPTH, D50, PSIC, TAUBBL, BEDFORM, VSBT, VDBT, IX, IY )
#endif
#ifdef W3_BT8
       ...
       !
#ifdef W3_BS1
       CALL W3SBS1 ( SPEC, CG1, WN1, DEPTH, CX, CY,TAUSCX, TAUSCY, VSBS, VDBS )
#endif
       !
       ...
       CALL UOST_SRCTRMCOMPUTE(IX, IY, SPEC, CG1, DT, U10ABS, U10DIR, VSUO, VDUO)
#endif
       !
...
8888      FORMAT (1X,I8.8,1X,I6.6,F8.1,L2,F8.2)
       ...
       DO IS=IS1, NSPECH
          ...
          if (w3_nl5_flag) then
             IF (NL5_SELECT .EQ. 1)  THEN
                DT = MIN ( DT , AFAC / ( MAX ( 1.E-10, 1. + NL5_OFFSET*AFAC*MIN(0.,VD(IS)) ) ) )
             ELSE
                DT = MIN ( DT , AFAC / ( MAX ( 1.E-10, 1. + OFFSET*AFAC*MIN(0.,VD(IS)) ) ) )
             ENDIF
          else
             DT  = MIN ( DT , AFAC / ( MAX ( 1.E-10, 1. + OFFSET*AFAC*MIN(0.,VD(IS)) ) ) )
          end if
       ...
       if (w3_debugsrc_flag) then
          IF (IX == DEBUG_NODE) then
             WRITE(*,'(A20,2I10,5F20.10,L20)') 'TIMINGS 2', IDT, NSTEPS, DT, DTMIN, DTDYN, HDT, DTTOT, SHAVE
       ...
       !
       if (w3_st1_flag) then
          IF ( FLTEST ) WRITE (NDST,9060) FH1*TPIINV, FH2*TPIINV, FHIGH*TPIINV, NKH
       end if
       !
       if (w3_st2_flag) then
          FHTRAN = XFT*FPI
       ...
       if (w3_st3_flag) then
          ...
          NKH    = MAX ( 2 , MIN ( NKH1 ,                           &
                   INT ( FACTI2 + FACTI1*LOG(MAX(1.E-7,FHIGH)) ) ) )
          IF ( FLTEST ) WRITE (NDST,9062) FH1*TPIINV, FH2*TPIINV, FHIGH*TPIINV, NKH
       end if
       !
       if (w3_st4_flag) then
          ! Introduces a Long & Resio (JGR2007) type dependance on wave age
          ...
          DO ITH=1, NTH
             if (w3_st2_flag) then
                SPEC(ITH+(IK-1)*NTH) = SPEC(ITH+(IK-2)*NTH) * FACHFA &
                     * FACDIA + FACPAR * SPEC(ITH+(IK-1)*NTH) + 0.
             else
       ...
       DO IK=1,NK
          ...
          IF (IS2PARS(2).GE.0) THEN
             IF (IS2PARS(20).GT.0.5) THEN
                ...
                SPEC(1+(IK-1)*NTH:NTH+(IK-1)*NTH) = ISO + (SPEC(1+(IK-1)*NTH:NTH+(IK-1)*NTH)-ISO)*SCAT
             ELSE
                !
                ! General solution with matrix exponentials: same
                ! as bottom scattering, see Ardhuin & Herbers (JFM 2002)
                !
...
8001 FORMAT (/ ' *** ERROR W3SRCE : ERROR IN WRITING TO FILE ***'/    &
               '                    IOSTAT = ',I10/)
    !
9000 FORMAT (  ' TEST W3SRCE : COUNTERS   : NO LONGER AVAILABLE') ! W3_T
...
9006 FORMAT (  ' TEST W3SRCE : FHIGH (3X) : ',3F8.4/                   &
               ...
               ' ------------- ')
    !
9020 FORMAT (  ' TEST W3SRCE : NSTEP : ',I4,'    DTTOT :',F6.1) ! W3_T
9021 FORMAT (  ' TEST W3SRCE : NKH (3X)   : ',2I3,I6)
    !
9040 FORMAT (  ' TEST W3SRCE : DTRAW, DT, SHAVE :',2F6.1,2X,L1) ! W3_T
    !
! W3_ST1
