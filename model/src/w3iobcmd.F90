...
CONTAINS
  ...
  SUBROUTINE W3IOBC ( INXOUT, NDSB, TIME1, TIME2, IOTST, IMOD )
    ...
    USE W3GDATMD, ONLY: SIG ! W3_T1
    !!   Use rotated N-Pole lat/lon and conversion sub.  JGLi12Jun2012
    USE W3GDATMD, ONLY: PoLat, PoLon, AnglD ! W3_RTD
    USE W3SERVMD, ONLY: W3LLTOEQ, W3EQTOLL, W3ACTURN ! W3_RTD
    ...
    INTEGER           :: IK, ITH ! W3_T1
    INTEGER, SAVE     :: IENT = 0 ! W3_S
    REAL              :: HS, HS0 ! W3_T1
    !!    Declare rotation angle and rotated lat/lon variables for
    !!    boundary points.   JGLi12Jun2012
    REAL, ALLOCATABLE :: Anglbdy(:), ELatbdy(:), ELonbdy(:)  ! W3_RTD
    REAL              :: Spectr(NK*NTH) ! W3_RTD
    REAL              :: XRLIM, YRLIM !  W3_RTD
             ...
             if (w3_t0_flag) then
                ...
                DO I=NBO(IFILE-1)+1, NBO(IFILE)
                   WRITE (NDST,9004) I-NBO(IFILE-1), XBPO(I),      &
                        YBPO(I), (IPBPO(I,J),J=1,4), (RDBPO(I,J),J=1,4)
                END DO
             end if
             !
          END DO
    ...
    IF ( INXOUT.EQ.'DUMP' .AND. FILED ) THEN
       IF ( IAPROC .EQ. NAPBPT ) THEN
          ...
          if (w3_t0_flag) then
             WRITE (NDST,9003)
             DO I=1, NBI
                WRITE (NDST,9004) I, XBPI(I), YBPI(I), (IPBPI(I,J),J=1,4), (RDBPI(I,J),J=1,4)
             END DO
          end if
          !
       END IF
    ...
    IF ( INXOUT.EQ.'READ' .AND. FILER ) THEN
       !
       READ (NDSB,ERR=803,IOSTAT=IERR) IDTST, VERTST, NKI, NTHI, XFRI, FR1I, TH1I, NBI
       !
       if (w3_t_flag) then
       ...
       if (w3_rtd_flag) then
          ...
          IF ( Polat < 90. ) THEN
             ...
             CALL W3LLTOEQ ( YBPI, XBPI, ELatbdy, ELonbdy, Anglbdy, PoLat, PoLon, NBI )
             XBPI = ELonbdy
             YBPI = ELatbdy
       ...
       ELSE IF( GTYPE .EQ. SMCTYPE ) THEN
          ...
          IF ( IAPROC .EQ. NAPERR ) WRITE (NDSE,920) ( ISBPI(I), XBPI(I), YBPI(I), I=1,NBI ) 
#endif
       ELSE 
          DO I=1, NBI
       ...
       if (w3_t0_flag) then
          WRITE (NDST,9003)
          DO I=1, NBI
             WRITE (NDST,9005) I, ISBPI(I), XBPI(I), YBPI(I), (IPBPI(I,J),J=1,4), (RDBPI(I,J),J=1,4)
          END DO
       end if
       !
       ...
       DO IFILE=1, NFBPO
          DO ISOUT=NBO2(IFILE-1)+1, NBO2(IFILE)
             ...
             if (w3_shrd_flag) then
                DO IS=1, NSPEC
                   ABPOS(IS,ISOUT) = VA(IS,ISEA) * SIG2(IS) / CG(1+(IS-1)/NTH,ISEA)
                END DO
             end if
             !
             ...
             if (w3_dist_flag) then
                DO IS=1, NSPEC
                   ABPOS(IS,ISOUT) = ABPOS(IS,ISOUT) * SIG2(IS) / CG(1+(IS-1)/NTH,ISEA)
                END DO
             end if
             !
             if (w3_rtd_flag) then
...
1010 FORMAT (/' *** WAVEWATCH III ERROR IN W3IOBC : '/               &
    ...
    !
9000 FORMAT (' TEST W3IOBC : INXOUT  : ',A5/                      & ! W3_T
         '               FLAGS   : ',3L2/                     &
         '               UNIT    : ',I4)
...
9005 FORMAT ('             ',I3,I4,2E10.3,2X,4I4,2X,4F5.2)
    !
9010 FORMAT (' TEST W3IOBC : OUTPUT FILE ',I1,' UNIT',I3,' TIME', & ! W3_T
...
9012 FORMAT (' TEST W3IOBC : INPUT FILE UNIT',I3,' TIME',         &
    ...
    !
9020 FORMAT (' TEST W3IOBC : SAVING OLD DATA') ! W3_T
9021 FORMAT (' TEST W3IOBC : SAVING FIRST DATA')
9022 FORMAT (' TEST W3IOBC : EOF REACHED')
    !
9040 FORMAT (' TEST W3IOBC : UNIT, ISOUT, ISEA, HS(NO TAIL) ') ! W3_T1
