...
CONTAINS
  ...
  SUBROUTINE WMGLOW ( FLRBPI )
    ...
    !/ Local parameters
    !/
    INTEGER              :: I, IBI, IX, IY, JS, J, JTOT, I1, J1, I2, J2
    INTEGER              :: NXYG, IERR_MPI     ! W3_MPI
    INTEGER              :: IENT = 0           ! W3_S
    INTEGER, ALLOCATABLE :: TSTORE(:,:)
    LOGICAL              :: FLBARR             ! W3_MPI
    REAL                 :: XA, YA
    REAL                 :: FACTOR
    ...
    REAL                 :: DX_MIN_GRIDI,DY_MIN_GRIDI,DX_MAX_GRIDI, DY_MAX_GRIDI
    REAL                 :: DX_MIN_GRIDJ,DY_MIN_GRIDJ,DX_MAX_GRIDJ, DY_MAX_GRIDJ
    INTEGER              :: ITRI, IM1, IM2, IT, JT, ISFIRST, ITOUT, NBRELEVANT
    REAL                 :: DIST_MIN, DIST_MAX, EDIST
    LOGICAL              :: RESOL_CHECK
    CHARACTER(LEN=1), ALLOCATABLE :: TMAP(:,:)  ! W3_T9
    !/ ------------------------------------------------------------------- /
    !/
    if (w3_s_flag) then
    ...
    if (w3_smc_flag) then
       ...
       IF( IMPROC.EQ.NMPERR )  WRITE(MDSE,*) " GTYPES in WMGLOW:",( GRIDS(I)%GTYPE, I=1, NRGRD )
    end if
    !
    ! -------------------------------------------------------------------- /
    ! 2.  Process grids
...
2001 FORMAT (/ ' *** WAVEWATCH-III WARNING : BOUNDARY POINT'/  &
               ...
               '     POINT SKIPPED '/)
    !
9010 FORMAT (  ' TEST WMGLOW : ALL GRIDS INITIALIZED') ! W3_T
    !
9020 FORMAT (  ' TEST WMGLOW : STARTING LOOP OVER GRIDS') ! W3_T
9021 FORMAT (  ' TEST WMGLOW : I, RANK, NBI :',2I4,I6)
9022 FORMAT (  '               ',A)
9023 FORMAT (  ' TEST WMGLOW : POINT DATA ') ! W3_T1
9024 FORMAT (I5,I8,2F6.1,4I5,4F5.2,I3,4I8)
9025 FORMAT (  ' TEST WMGLOW : NBI2S ') ! W3_T2
9026 FORMAT (  '           ',2I4,2X,I8)
9027 FORMAT (  ' TEST WMGLOW : NBI, NBI2, RFILE, NBI2G ') ! W3_T
9028 FORMAT (  '               ',2I5,L2,' : ',20I5)
! W3_T9
9029 FORMAT (  ' TEST WMGLOW : SOURCE MAP GRID',I3,'   PART',I3)
    !
9030 FORMAT (  ' TEST WMGLOW : GRDLOW DIMENSIONED AT ',I2) ! W3_T
9031 FORMAT (  ' TEST WMGLOW : GRDLOW :')
9032 FORMAT (  '                 ',2i4,' : ',20I3)
    !
9040 FORMAT (  ' TEST WMGLOW : GRDHGH DIMENSIONED AT ',I2) ! W3_T
...
9042 FORMAT (  '                 ',2i4,' : ',20I3)
  ...
  SUBROUTINE WMGHGH
    ...
    INTEGER                       ::  NLMAX_SCRIP                   ! W3_SCRIP
    INTEGER                       :: LTAG0                          ! W3_DIST
    ...
    INTEGER, ALLOCATABLE          :: NX_SIZE(:), IRQ(:), MSTAT(:,:) ! W3_MPIBDI
    INTEGER                       :: IM, NX_REM, TAG, NRQ           ! W3_MPI
    ...
    REAL                          :: WX, WY, WTOT
    LOGICAL                       :: FLGREC
    LOGICAL, ALLOCATABLE          :: GRIDOK(:), STMASK(:,:), MASKI(:,:), TMPLOG(:)
...
#ifdef W3_SCRIP
    TYPE ALLWGT
       TYPE(WEIGHT_DATA), POINTER :: WGTDATA(:)                     ! W3_SCRIP
    END TYPE ALLWGT
    ...
    INTEGER                       :: IMPROC_ASSIGN                  ! W3_SCRIPNC
    CHARACTER(LEN=80)             :: interp_file1, interp_file_test ! W3_SCRIPNC
    CHARACTER(LEN=3)              :: cdst, csrc                     ! W3_SCRIPNC
    LOGICAL, ALLOCATABLE          :: LGRDREAD(:,:)                  ! W3_SCRIPNC
    LOGICAL, ALLOCATABLE          :: LGRDWRITE(:,:)                 ! W3_SCRIPNC
    INTEGER                       :: NGRDRANK(2)                    ! W3_SCRIPNC
    ...
    CHARACTER(LEN=1), ALLOCATABLE :: MAPST(:,:)                           ! W3_T
    CHARACTER (LEN=10)            :: CDATE_TIME(3)                        ! W3_T38
    INTEGER                       :: DATE_TIME(8)                         ! W3_T38
    INTEGER                       :: ELAPSED_TIME, BEG_TIME(10), END_TIME ! W3_T38
    INTEGER                       :: NMYOUT=42                            ! W3_T38
    CHARACTER (LEN=14)            :: CMYOUT="myout00000.lis"              ! W3_T38
    CHARACTER (LEN=5)             :: CRANK                                ! W3_T38
    !/ ------------------------------------------------------------------- /
...
1021 FORMAT (/ ' *** WAVEWATCH III ERROR IN WMGHGH : *** '/           &
    ...
    !
9010 FORMAT (  ' TEST WMGHGH : INITIALIZE BOUNDARY DISTANCE MAPS') ! W3_T
...
9014 FORMAT (2x,65I2)
    !/
9020 FORMAT (  ' TEST WMGHGH : GRID',I3,' HAS',I3,' DATA SOURCES') ! W3_T
...
9027 FORMAT (2X,65A2)
    !
9028 FORMAT (  ' TEST WMGHGH : COUNTERS ',A) ! W3_T
9029 FORMAT (2x,20I6)
    !
9030 FORMAT (  ' TEST WMGHG : FROM GRID',I3,', NO DATA TO RECEIVE') ! W3_T3
...
9042 FORMAT ( 12X,I10,4I6)
  ...
  SUBROUTINE WMGEQL
    ...
    INTEGER                  :: IA  ! W3_T7
    INTEGER                  :: IENT = 0 ! W3_S
    INTEGER, ALLOCATABLE     :: MAP3D(:,:,:), NREC(:), NSND(:),  NTPP(:), MAPOUT(:,:)
    ...
    LOGICAL, ALLOCATABLE     :: SHRANK(:,:), DOGRID(:), MASKA(:,:), MASKI(:,:)
    ...
    if (w3_smc_flag) then
       !!  Check GTYPE for all grids.
       IF( IMPROC.EQ.NMPERR )  WRITE (MDSE,*) " WMGEQL GTYPE:",( GRIDS(I)%GTYPE, I=1, NRGRD )
    end if
    !
    ! -------------------------------------------------------------------- /
          ...
          DO JJ=1, STORES(I,J)%NTOT
             ...
             IF ( IX.EQ.1 .OR. IX.EQ.NX .OR. IY.EQ.1 .OR. IY.EQ.NY ) THEN
                ...
                IF ( ABS(MAPSTA(IY,IX)).EQ.2 .AND.                    &
                   ...
                   MAPODI(IY,IX) = 0.
                   if (w3_o13_flag) then
                      IF ( IMPROC.EQ.NMPERR ) WRITE (MDSE,1020) I, IX, 1
                   end if
                END IF
             ELSE
       ...
       DO J=1, NRGRD
          ...
          DO JJ=1, STORES(I,J)%NTOT
             ...
             IF ( MAP3D(IY,IX,-2) .NE. 0 ) THEN
                ...
                if (w3_mpi_flag) then
                   MAP3D(IY,IX,-4) = MAP3D(IY,IX,-2) - (MAP3D(IY,IX,-3)-1)*NAPROC + CROOT - 1
                end if
             END IF
...
1051 FORMAT (/' *** WAVEWATCH III ERROR IN WMGEQL : *** '/           &
         ...
         '         GRID',I4,' IN GROUP',I4/)
    !
9010 FORMAT ( ' TEST WMGEQL : STARTING LOOP OVER GRIDS') ! W3_T
9011 FORMAT ( ' TEST WMGEQL : I, RANK :',2I4)
9012 FORMAT ( '               GRID ',I3,' HAS SAME RANK')
9013 FORMAT ( '               ',A)
    !
9020 FORMAT ( ' TEST WMGEQL : GENERATING DISTANCE MAP GRID ',I3) ! W3_T
9024 FORMAT ( ' TEST WMGEQL : FINAL MAP FOR X RANGE ',2I6)
9025 FORMAT (2X,65I2)
    !
9030 FORMAT ( ' TEST WMGEQL : DEPENDENCE INFORMATION GRID ',I3) ! W3_T
...
9037 FORMAT (2X,65I2)
    !
9040 FORMAT ( ' TEST WMGEQL : GRID ',I2,'-',I2,' CLEAR STORAGE') ! W3_T
...
9045 FORMAT ( '               SEND ',I2,'-',I2,' STORAGE SIZE',I6)
    !
9050 FORMAT ( ' TEST WMGEQL : GRDEQL DIMENSIONED AT ',I2) ! W3_T
...
9334 FORMAT (64X,2I6,I4,F6.2,I6)
  ...
  SUBROUTINE WMRSPC
    ...
    ! Formats
    !
9000 FORMAT ( 'TEST WMRSPC : MAP RESPEC FILLED ') ! W3_T
9001 FORMAT ( '              ',I4,' : ',20L2)
    !/
    !/ End of WMRSPC ----------------------------------------------------- /
    !/
  ...
  SUBROUTINE WMSMCEQL
    ...
    !
    USE W3SERVMD, ONLY: EXTCDE
    USE W3PSMCMD, ONLY: W3SMCGMP, W3SMCELL  ! W3_SMC
    USE W3SERVMD, ONLY: STRACE ! W3_S
    !
    IMPLICIT NONE
    !
    ...
    if ( w3_smc_flag) then
#ifdef W3_MPI
       IF( IMPROC .EQ. NMPERR ) WRITE(MDSE,*) " *** WMSMCEQL completed from PE ", IMPROC
#else
...
9013 FORMAT ( '               ',A)
    !
9020 FORMAT ( ' TEST WMSMCEQL : GENERATING DISTANCE MAP GRID ',I3) ! W3_T
9024 FORMAT ( ' TEST WMSMCEQL : FINAL MAP FOR X RANGE ',2I6)
9025 FORMAT (2X,65I2)
    !
9030 FORMAT ( ' TEST WMSMCEQL : DEPENDENCE INFORMATION GRID ',I3) ! W3_T
9031 FORMAT ( '               CHECKING GRID ',I3)
9032 FORMAT ( '               POINTS USED/AVAIL :',2I6)
9033 FORMAT ( '               TOTAL/GRIDS/OUT   :',3I6)
9034 FORMAT ( '               LOCAL PER GRID    :',15I6)
...
9037 FORMAT (2X,65I2)
    !
9040 FORMAT ( ' TEST WMSMCEQL : GRID ',I2,'-',I2,' CLEAR STORAGE') ! W3_T
...
9045 FORMAT ( '               SEND ',I2,'-',I2,' STORAGE SIZE',I6)
    !
9050 FORMAT ( ' TEST WMSMCEQL : GRDEQL DIMENSIONED AT ',I2) ! W3_T
