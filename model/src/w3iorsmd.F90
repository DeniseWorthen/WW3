...
CONTAINS
  ...
  SUBROUTINE W3IORS ( INXOUT, NDSR, DUMFPI, IMOD, FLRSTRT )
    ...
    USE W3IDATMD, ONLY: WXN, WYN, W3SETI ! W3_WRST
    USE W3IDATMD, ONLY: WXNwrst, WYNwrst ! W3_WRST
    USE W3ODATMD, ONLY: NDSE, NDST, IAPROC, NAPROC, NAPERR, NAPRST, &
                        IFILE => IFILE4, FNMPRE, NTPROC, IOSTYP,    &
                        FLOGRR, NOGRP, NGRPP, SCREEN
    USE W3ODATMD, ONLY: NRQRS, NBLKRS, RSBLKS, IRQRS, IRQRSS, VAAUX ! W3_MPI
    USE W3ADATMD, ONLY: MPI_COMM_WCMP ! W3_MPI
    !/
    ...
    INTEGER                     :: IERR_MPI, IH, IB, ISEA0, ISEAN, NRQ, NSEAL_MIN ! W3_MPI
    INTEGER(KIND=8)             :: RPOS
    INTEGER, ALLOCATABLE        :: STAT1(:,:), STAT2(:,:) ! W3_MPI
    REAL, ALLOCATABLE           :: VGBUFF(:), VLBUFF(:) ! W3_MPI
    REAL(KIND=LRB), ALLOCATABLE :: WRITEBUFF(:), TMP(:), TMP2(:)
    ! PROBLEM (MV) should not use WRITE as a variable name
    LOGICAL                     :: write_flag, IOSFLG
...
!CHECKPOINT RESTART FILE
       ...
       IF(ITMP.NE.1)THEN ! FNAME is not set above, so do it here
          ...
          ELSE
             ...
             IF ( write_flag .AND. IAPROC.EQ.NAPRST ) then
    ...
    ELSE
       ...
       IF (.NOT. write_flag .AND. OARST .AND. IAPROC .EQ. NAPROC) THEN
          DO I=1, NOGRP
             DO J=1, NGRPP
...
#endif
             !
          END IF ! IF ( .NOT.IOSFLG .OR. (NAPROC.EQ.1.AND.NAPRST.EQ.1) )
          !
       END IF ! IF ( TYPE.EQ.'WIND' .OR. TYPE.EQ.'CALM' )
          ...
          ELSE
             ...
             ELSE
                ...
                DO JSEA=1, NSEAL
                   ...
                   NREC   = ISEA + 2
                   RPOS   = 1_8 + LRECL*(NREC-1_8)
                   READ (NDSR,POS=RPOS,ERR=802,IOSTAT=IERR) (VA(I,JSEA),I=1,NSPEC)
                ENDDO
#ifdef W3_MPI
             END IF
#endif
...
1000 FORMAT (/' *** WAVEWATCH III WARNING IN W3IORS : '/             &
    ...
    !
    !
9000 FORMAT (' TEST W3IORS : TEST PARAMETERS :'/                  & ! W3_T
         '      INXOUT : ',A,/                                &
         '       WRITE : ',L10/                               &
         '      NTPROC : ',I10/                               &
         '      NAPROC : ',I10/                               &
...
9008 FORMAT (' TEST W3IORS : WATER LEVELS ETC. PROCESSED (DUMMY)')
    !
9020 FORMAT (' TEST W3IORS : RSTYPE = ',A,', PERFORMED BY W3INIT') ! W3_T
    !/
    !/ End of W3IORS ----------------------------------------------------- /
    !/
