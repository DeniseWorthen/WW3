...
CONTAINS
  ...
  SUBROUTINE W3UCUR ( FLFRST )
    ...
    USE W3GDATMD, ONLY: NARC, NGLO, ANGARC ! W3_SMC
    USE W3GDATMD, ONLY: FSWND, ARCTC ! W3_SMC
    USE W3WDATMD, ONLY: TIME
    USE W3ADATMD, ONLY: CX, CY, CA0, CAI, CD0, CDI
    USE W3IDATMD, ONLY: TC0, CX0, CY0, TCN, CXN, CYN
    USE W3GDATMD, ONLY: YGRD ! W3_TIDE
    USE W3TIMEMD
    ...
    !
    if (w3_crt0_flag) then
       RD     = 0.
    end if
    if (w3_crt1_flag) then
       RD     = DT0T / MAX ( 1.E-7 , DT0N )
    end if
    if (w3_crt2_flag) then
       RD     = DT0T / MAX ( 1.E-7 , DT0N )
       RD2    = 1. - RD
    end if
    ...
    DO ISEA=1, NSEA
       ...
       ! could move IF test outside of ISEA loop ...
       if (w3_tide_flag .and. FLCURTIDE) THEN  ! could move IF test outside of ISEA loop ...
       ...
       else if ((w3_tide_flag .and. .not. FLCURTIDE) .or. .not. w3_tide_flag) then

          CABS = CA0(ISEA) + RD * CAI(ISEA)
          if (w3_crt2_flag) then
             CI2 = SQRT ( RD2 * CA0(ISEA)**2 + RD  *(CA0(ISEA)+CAI(ISEA))**2 )
    ...
    END DO ! DO ISEA=1, NSEA  ! TODO: check with Denise
    !
    RETURN
    !
    ! Formats
    !
9000 FORMAT (' TEST W3UCUR : DT0N, DT0T, RD :',2F8.1,F6.3) ! W3_T
    !/
    !/ End of W3UCUR ----------------------------------------------------- /
    !/
  ...
  SUBROUTINE W3UWND ( FLFRST, VGX, VGY )
    ...
    !/ ------------------------------------------------------------------- /
    USE W3GDATMD, ONLY: NX, NY, NSEA, MAPSF
    USE W3GDATMD, ONLY: WWCOR                                         ! W3_WCOR
    USE W3GDATMD, ONLY: RWINDC                                        ! W3_RWND
    USE W3GDATMD, ONLY: ZWIND, OFSTAB, FFNG, FFPS, CCNG, CCPS, SHSTAB ! W3_ST2
    USE W3GDATMD, ONLY: NARC, NGLO, ANGARC, ARCTC, FSWND              ! W3_SMC
    USE W3WDATMD, ONLY: TIME, ASF
    USE W3ADATMD, ONLY: DW, CX, CY, UA, UD, U10, U10D, AS
    USE W3ADATMD, ONLY: UA0, UAI, UD0, UDI, AS0, ASI
    ...
    INTEGER :: ISEA, IX, IY
    INTEGER :: IENT = 0                                ! W3_S
    REAL    :: D0, DN, DD, DT0N, DT0T, RD, UI2, UXR, UYR
    REAL    :: RD2                                     ! W3_WNT2
    REAL    :: STAB0, STAB, THARG1, THARG2, COR1, COR2 ! W3_STAB2
    REAL    :: UDARC
    !/
    !/ ------------------------------------------------------------------- /
    !/
    ...
    !
    if (w3_wnt0_flag) then
       RD     = 0.
    end if
    if (w3_wnt1_flag) then
       RD     = DT0T / MAX ( 1.E-7 , DT0N )
    end if
    if (w3_wnt2_flag) then
       RD     = DT0T / MAX ( 1.E-7 , DT0N )
       RD2    =  1. - RD
    end if
    ...
    ! Formats
    !
9000 FORMAT (' TEST W3UWND : DT0N, DT0T, RD :',2F8.1,F6.3) ! W3_T
    !/
    !/ End of W3UWND ----------------------------------------------------- /
    !/
  ...
  SUBROUTINE W3UTAU ( FLFRST )
    ...
    INTEGER :: ISEA, IX, IY
    INTEGER :: IENT = 0 ! W3_S
    REAL    :: D0, DN, DD, DT0N, DT0T, RD, MI2, MXR, MYR
    REAL    :: RD2      ! W3_WNT2
    REAL    :: MDARC
    !/
    !/ ------------------------------------------------------------------- /
    ...
    ! Formats
    !
9000 FORMAT (' TEST W3UTAU : DT0N, DT0T, RD :',2F8.1,F6.3) ! W3_T
    !/
    !/ End of W3UTAU ----------------------------------------------------- /
    !/
  ...
  SUBROUTINE W3UINI ( A )
    ...
    INTEGER :: IENT = 0                ! W3_S
    INTEGER :: IX0, IXN, MAPOUT(NX,NY) ! W3_T
    INTEGER :: NXP = 60
    REAL    :: ALFA(NSEAL), FP(NSEAL), YLN(NSEAL),  AA, BB, CC
    REAL    :: XGR, U10C, U10DIR, XSTAR, FSTAR, GAMMA, FR, D1(NTH), D1INT, F1, F2
    REAL    :: ETOT, E1I
    REAL    :: U10MIN =  1.
    REAL    :: U10MAX = 20.
    REAL    :: HSIG(NX,NY)             ! W3_T
    !/
    !/ ------------------------------------------------------------------- /
    !/
    if (w3_s_flag) then
    ...
    ! Formats
    !
9000 FORMAT (' TEST W3UINI : XGR = ',E10.3) ! W3_T
    !
9010 FORMAT (' TEST W3UINI :  ISEA, U10C, XSTAR, ALPHA, FP, GAMMA') ! W3_T1
9011 FORMAT ('   ',I6,F8.2,F10.1,2F6.3,F6.2)
  ...
  SUBROUTINE W3UBPT
    ...
    USE W3GDATMD, ONLY: NK, NTH, NSPEC, AnglD, PoLat ! W3_RTD
    USE W3SERVMD, ONLY: W3ACTURN ! W3_RTD
    USE W3ADATMD, ONLY: CG
    USE W3ODATMD, ONLY: NBI, ABPI0, ABPIN, ISBPI, IPBPI, RDBPI, BBPI0, BBPIN
    !/
    IMPLICIT NONE
    !/
    !/ ------------------------------------------------------------------- /
    ...
    REAL    :: Spectr(NSPEC), AnglBP ! W3_RTD
    !/
    !/ ------------------------------------------------------------------- /
    !/
    if (w3_s_flag) then
...
9001 FORMAT ( '         ',2I8,2X,2F8.2)
  ...
  SUBROUTINE W3UIC1( FLFRST )
    ...
    RETURN
    !
9010 FORMAT ( ' TEST W3UIC1 : TIME     :',I9.8,I7.6/ & ! W3_T
              '               OLD TICE :',I9.8,I7.6/ &
              '               NEW TICE :',I9.8,I7.6)
    !/
    !/ End of W3UIC1 ----------------------------------------------------- /
  ...
  SUBROUTINE W3UIC5( FLFRST )
    ...
    !
    RETURN
9010 FORMAT ( ' TEST W3UIC5 : TIME     :',I9.8,I7.6/ & ! W3_T
              '               OLD TICE :',I9.8,I7.6/ &
              '               NEW TICE :',I9.8,I7.6)
    !/
    !/
  ...
  SUBROUTINE W3UICE ( VA )
    ...
    ! Formats
    !
9000 FORMAT ( ' TEST W3UICE : FICEN    :',F9.3) ! W3_T
...
9021 FORMAT ( '           ',I8,3I4,F6.2,2X,A)
  ...
  SUBROUTINE W3ULEV ( A, VA )
    ...
    USE W3GDATMD, ONLY : ZB, DMIN, NK, NTH, NSPEC, SIG, DSIP
    USE W3GDATMD, ONLY : MAPWN, MAPTH, FACHFA, GTYPE, UNGTYPE, W3SETREF
    ...
    USE W3WDATMD, ONLY : ZETA_SETUP    ! W3_SETUP
    USE W3GDATMD, ONLY : DO_CHANGE_WLV ! W3_SETUP
    USE W3ARRYMD, ONLY : PRT2DS        ! W3_T3
    USE W3TIMEMD
    USE W3TIDEMD
#ifdef W3_PDLIB
    ...
    !/ Local variables
    !/
    INTEGER         :: ISEA, JSEA, IX, IY, IK, I1, I2, ISPEC, IK0, ITH
    INTEGER         :: IENT = 0 ! W3_S
    ...
    DO ISEA = 1, NSEA
       ...
       DWO(ISEA) = DW(ISEA)
       !
       if (w3_tide_flag .and. FLLEVTIDE) then
          !
       ...
       IF ( KDCHCK .LT. KDMAX ) THEN
          ...
          IF ( WLV(ISEA)-ZB(ISEA) .LE.0. ) THEN
             IF ( MAPDRY(IY,IX) .EQ. 0 ) THEN
                ...
                MAPSTA(IY,IX) = -ABS(MAPSTA(IY,IX))
                if (w3_t2_flag) then
                   WRITE (NDST,9021) ISEA, WLV(ISEA)-ZB(ISEA),  0., 0., '  (NEW DRY)'
                end if
             ELSE
                if (w3_t2_flag) then
                   WRITE (NDST,9021) ISEA, WLV(ISEA)-ZB(ISEA),  0., 0., '  (DRY)'
                end if
             ENDIF
             !
             CYCLE
          ...
          IF (WLV(ISEA)-ZB(ISEA).GT.0. .AND. MAPDRY(IY,IX).EQ.1) THEN
             ...
             IF ( MAPST2(IY,IX) .EQ. 0 ) THEN
                MAPSTA(IY,IX) = ABS(MAPSTA(IY,IX))
                if (w3_t2_flag) then
                   WRITE (NDST,9021) ISEA, WLV(ISEA)-ZB(ISEA), 0., 0., '  (NEW WET)'
                end if
             ELSE
                if (w3_t2_flag) then
                   WRITE (NDST,9021) ISEA, WLV(ISEA)-ZB(ISEA), 0., 0., '  (NEW WET INACTIVE)'
                end if
             END IF
             !
             CYCLE
...
1000 FORMAT (/ ' *** ERROR W3ULEV *** '/               &
    ...
    !
9000 FORMAT (  ' TEST W3ULEV : KDMAX    :',F6.1/       & ! W3_T
               '               RDKMIN   :',F8.3)
    !
9010 FORMAT (  ' TEST W3ULEV : TIME     :',I9.8,I7.6/  & ! W3_T
               '               OLD TLEV :',I9.8,I7.6)
...
9021 FORMAT (  '             ',I6,F8.2,F6.2,F7.3,A)
  ...
  SUBROUTINE W3URHO ( FLFRST )
    ...
    !/ ------------------------------------------------------------------- /
    USE W3GDATMD, ONLY: NSEA, MAPSF
    USE W3GDATMD, ONLY: FSWND ! W3_SMC
    ...
    ! Formats
    !
9000 FORMAT (' TEST W3URHO : DT0N, DT0T, RD :',2F8.1,F6.3) ! W3_T
    !/
    !/ End of W3URHO ----------------------------------------------------- /
    !/
  ...
  SUBROUTINE W3UTRN ( TRNX, TRNY )
    ...
    USE W3GDATMD, ONLY: TRFLAG, FICE0, FICEN, FICEL
    USE W3GDATMD, ONLY: RLGTYPE, CLGTYPE, GTYPE, FLAGLL
    USE W3GDATMD, ONLY: HPFAC, HQFAC, FFACBERG
    ...
    INTEGER :: IENT = 0 ! W3_S
    INTEGER :: ILEV, NLEV ! W3_T
    REAL    :: TRIX(NY*NX), TRIY(NY*NX), DX, DY,LICE0, LICEN
    REAL    :: LEVS(0:10) ! W3_T
    !/
    !/ ------------------------------------------------------------------- /
    !/
    ...
    ! Formats
    !
9000 FORMAT ( ' TEST W3UTRN : TRFLAG = ',I3) ! W3_T
...
9011 FORMAT ( '             ',I4,F8.5)
  ...
  SUBROUTINE W3DZXY( ZZ, ZUNIT, DZZDX, DZZDY )
    ...
    USE W3GDATMD, ONLY: DPDX, DPDY, DQDX, DQDY, FLAGLL, ICLOSE
    USE W3GDATMD, ONLY: ICLOSE_NONE, ICLOSE_SMPL, ICLOSE_TRPL
    ...
    ! Formats
    !
9000 FORMAT (' TEST W3DZXY : DX0I, DY0I : ',2E12.5) ! W3_T
