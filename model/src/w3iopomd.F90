...
CONTAINS
  ...
  SUBROUTINE W3IOPP ( NPT, XPT, YPT, PNAMES, IMOD )
    ...
    USE W3GDATMD, ONLY: RLGTYPE, CLGTYPE, UNGTYPE, GTYPE, FLAGLL
    USE W3GDATMD, ONLY: ICLOSE,ICLOSE_NONE,ICLOSE_SMPL,ICLOSE_TRPL
    USE W3GDATMD, ONLY: MAPSTA, MAPFS, FILEXT, ZB, TRNX, TRNY
    USE W3GDATMD, ONLY: TRIGP,MAXX, MAXY, DXYMAX
    !!  Use rotated N-Pole lat/lon and conversion sub.  JGLi12Jun2012
    USE W3GDATMD, ONLY: PoLat, PoLon, FLAGUNR ! W3_RTD
    USE W3SERVMD, ONLY: W3LLTOEQ  ! W3_RTD
    USE W3ODATMD, ONLY: W3DMO2
    USE W3ODATMD, ONLY: NDSE, NDST, IAPROC, NAPERR, NAPOUT, SCREEN
    USE W3ODATMD, ONLY: NOPTS, PTLOC, PTNME, GRDID, IPTINT, PTIFAC
    ...
    INTEGER          :: IX0, IXN, IY0, IYN, NNX, KX, KY, JX, IIX, IX2, IY2, IS1 ! W3_O7a
    INTEGER          :: IX(4), IY(4)                                            ! Indices of points used in interp.
    REAL             :: RD(4)                                                   ! Interpolation coefficient
    REAL             :: RD1, RD2, RDTOT, ZBOX(4), DEPTH                         ! W3_O7a
    REAL, PARAMETER  :: ACC = 0.05
    REAL             :: FACTOR
    INTEGER          :: ITOUT                                                   ! Triangle index in unstructured grids
    CHARACTER(LEN=1) :: SEA(5), LND(5), OUT(5)                                  ! W3_O7a
    CHARACTER(LEN=9) :: PARTS                                                   ! W3_O7a
    CHARACTER(LEN=1), ALLOCATABLE :: STRING(:), LINE1(:), LINE2(:)              ! W3_O7a
    !
    DATA SEA / ' ', 's', 'e', 'a', ' ' /  !  W3_O7a
    DATA LND / ' ', 'l', 'n', 'd', ' ' /  !  W3_O7a
    DATA OUT / ' ', 'x', 'x', 'x', ' ' /  !  W3_O7a
    !/
    !!  Declare a few temporary variables for rotated grid.  JGLi12Jun2012  
    REAL, ALLOCATABLE :: EquLon(:),EquLat(:),StdLon(:),StdLat(:),AnglPT(:)  ! W3_RTD
    !/
    !/ ------------------------------------------------------------------- /
    !/
    if (w3_s_flag) then
    ...
    if (w3_rtd_flag) then
       ...
       ALLOCATE( EquLon(NPT), EquLat(NPT), StdLon(NPT), StdLat(NPT), AnglPT(NPT) )

       StdLon = XPT
       StdLat = YPT

       CALL W3LLTOEQ ( StdLat, StdLon, EquLat, EquLon, AnglPT, PoLat, PoLon, NPT )
    ...
    if (w3_o7a_flag) then
       IF ( IAPROC .EQ. NAPOUT ) THEN
          ...
          WRITE (SCREEN,*)
          WRITE (SCREEN,*)
       END IF ! IF ( IAPROC .EQ. NAPOUT )
    end if ! if (w3_o7a_flag)
    !
    RETURN
    !
    ! Formats
...
1003 FORMAT (/' *** WAVEWATCH-III WARNING :'/                   &
         ...
         '     POINT SKIPPPED '/)
    !
9010 FORMAT (' TEST W3IOPP : INPUT  : ',I4,2F12.2,2X,A) ! W3_T
...
9021 FORMAT (' TEST W3IOPP : PREPROCESSED DATA',I4,2X,A,2X,2F12.2, &
  ...
  SUBROUTINE W3IOPE ( A )
    ...
    USE W3GDATMD, ONLY: MAPSTA, MAPFS
    !!   Use spectral rotation sub and angle.  JGLi12Jun2012
    USE W3GDATMD, ONLY: NSPEC, AnglD, FLAGUNR ! W3_RTD
    USE W3SERVMD, ONLY: W3ACTURN              ! W3_RTD
    USE W3WDATMD, ONLY: ICE, ICEH, ICEF
    USE W3WDATMD, ONLY: RHOAIR                ! W3_FLX5
    USE W3ADATMD, ONLY: CG, DW, UA, UD, AS, CX, CY, SP => SPPNT
    USE W3ADATMD, ONLY: TAUA, TAUADIR         ! W3_FLX5
    USE W3ODATMD, ONLY: NDST, NOPTS, IPTINT, PTIFAC, IL, IW, II
    USE W3ODATMD, ONLY: DPO, WAO, WDO, ASO, CAO, CDO, ICEO, ICEHO
    USE W3ODATMD, ONLY: ICEFO, SPCO, NAPROC
    USE W3ODATMD, ONLY: TAUAO, TAUDO, DAIRO   ! W3_FLX5
    USE W3WDATMD, ONLY: ZETA_SETUP            ! W3_SETUP
    USE W3ODATMD, ONLY: ZET_SETO              ! W3_SETUP
    ...
    !/ Local parameters
    !/
    INTEGER       :: I, IX1, IY1, IX(4), IY(4), J, IS(4), IM(4), IK, ITH, ISP
#ifdef W3_MPI
    ...
    REAL          :: TAUX, TAUY             ! W3_FLX5
    INTEGER       :: JSEA, ISEA
    REAL          :: SPTEST(NK,NTH)         ! W3_T
    REAL          :: Spectr(NSPEC), AnglDIS ! W3_RTD
    INTEGER       :: IROT                   ! W3_RTD
    !/
    !/ ------------------------------------------------------------------- /
    !/
    ...
    ! Formats
    !
9000 FORMAT (' TEST W3IOPE : POINT NR.:',I3) ! W3_T
...
9004 FORMAT (' TEST W3IOPE :',F8.1,2(F7.2,F7.1))
  ...
  SUBROUTINE W3IOPO ( INXOUT, NDSOP, IOTST, IMOD )
    ...
    USE W3ODATMD, ONLY: NDST, NDSE, NOPTS, IPTINT
    USE W3ODATMD, ONLY: IPASS => IPASS2 
    USE W3ODATMD, ONLY: IL, IW, II, PTLOC, PTIFAC, DPO, WAO, WDO
    USE W3ODATMD, ONLY: ASO, CAO, CDO, SPCO, PTNME, O2INIT, FNMPRE
    USE W3ODATMD, ONLY: GRDID, ICEO, ICEHO, ICEFO
    USE W3ODATMD, ONLY: TAUAO, TAUDO, DAIRO ! W3_FLX5
    ...
    INTEGER, SAVE           :: IENT = 0 ! W3_S
    ! PROBLEM (MV) should not have a variable called WRITE - call it write_flag
    LOGICAL,SAVE            :: write_flag
    ...
    ELSE
       IF ( write_flag .AND. INXOUT.EQ.'READ' ) THEN
    ...
    DO I=1, NOPTS
       ...
       IF ( write_flag ) THEN
          ...
          II(I) = 0
          IL(I) = 0
          WRITE (NDSOP) IW(I), II(I), IL(I), DPO(I), WAO(I), WDO(I), &
#ifdef W3_FLX5
...
1002 FORMAT (/' *** WAVEWATCH III ERROR IN W3IOPO : '/               &
         ...
         '     IOSTAT =',I5/)
    !
9000 FORMAT (' TEST W3IOPO : IPASS =',I4,'    INXOUT = ',A,       & ! W3_T
         ' WRITE = ',L1,' UNIT =',I3/                         &
         '               IGRD =',I3,' FEXT = ',A)

9001 FORMAT (' TEST W3IOPO : OPENING NEW FILE [',A,']') ! W3_T
...
9004 FORMAT (3X,I4,2F10.2,2X,A)
    !
9010 FORMAT (' TEST W3IOPO : TIME  :',I9.8,I7.6) ! W3_T
9011 FORMAT (' TEST W3IOPO : END OF FILE REACHED')
    !
9020 FORMAT (' TEST W3IOPO : POINT NR.:',I5) ! W3_T
