...
1008 FORMAT (/' *** WAVEWATCH III WARNING  : '/                       &
  ...
  SUBROUTINE W3OUTG ( A, FLPART, FLOUTG, FLOUTG2 )
    ...
    ! USSX, USSY   : surface Stokes drift (SD) (W3_CESMCOUPLED)
    ! USSXH, USSYH : surface layer (SL) averaged SD (W3_CESMCOUPLED)
    ! LANGMT       : La_t (W3_CESMCOUPLED)
    ! LAPROJ       : La_{Proj} (W3_CESMCOUPLED)
    ! LASL         : La_{SL} (W3_CESMCOUPLED)
    ! LASLPJ       : La_{SL,Proj} (W3_CESMCOUPLED)
    ! ALPHAL       : angle between wind and Langmuir cells (SL averaged) (W3_CESMCOUPLED)
    ! ALPHALS      : angle between wind and Langmuir cells (surface) (W3_CESMCOUPLED)
    ! UD           : wind direction (W3_CESMCOUPLED)
    ! LAMULT       : enhancement factor (W3_CESMCOUPLED)
    ! HML          : mixing layer depth (from coupler) (W3_CESMCOUPLED)
    USE W3ADATMD, ONLY: LAMULT, USSXH, USSYH, LANGMT, LAPROJ, ALPHAL, ALPHALS, LASL, UD, LASLPJ  ! W3_CESMCOUPLED
    USE W3IDATMD, ONLY: HML ! W3_CESMCOUPLED
    USE W3WDATMD, ONLY: ASF ! W3_CESMCOUPLED
    USE W3SERVMD, ONLY: STRACE ! W3_S
    !
    ...
    REAL       :: SWW !angle between wind and waves   ! W3_CESMCOUPLED
    REAL       :: HSL !surface layer depth (=0.2*HML) ! W3_CESMCOUPLED
    ! tmp variables for surface and SL averaged SD
    REAL       :: ETUSSX(NSEAL)                       ! W3_CESMCOUPLED
    REAL       :: ETUSSY(NSEAL)                       ! W3_CESMCOUPLED
    REAL       :: ETUSSXH(NSEAL)                      ! W3_CESMCOUPLED
    REAL       :: ETUSSYH(NSEAL)                      ! W3_CESMCOUPLED
    !/
    !/ ------------------------------------------------------------------- /
    !/
    if (w3_s_flag) then
    ...
    FP1    = UNDEF
    THP1   = UNDEF
    ETUSSX  = 0.    ! W3_CESMCOUPLED
    ETUSSY  = 0.    ! W3_CESMCOUPLED
    ETUSCX  = 0.    ! W3_CESMCOUPLED
    ETUSCY  = 0.    ! W3_CESMCOUPLED
    ETUSSXH  = 0.   ! W3_CESMCOUPLED
    ETUSSYH  = 0    ! W3_CESMCOUPLED
    LANGMT = UNDEF  ! W3_CESMCOUPLED
    LAPROJ = UNDEF  ! W3_CESMCOUPLED
    LASL   = UNDEF  ! W3_CESMCOUPLED
    LASLPJ = UNDEF  ! W3_CESMCOUPLED
    ALPHAL = UNDEF  ! W3_CESMCOUPLED
    ALPHALS = UNDEF ! W3_CESMCOUPLED
    USSX   = 0.     ! W3_CESMCOUPLED
    USSY   = 0.     ! W3_CESMCOUPLED
    USSXH  = 0.     ! W3_CESMCOUPLED
    USSYH  = 0.     ! W3_CESMCOUPLED
    LAMULT  = 1.    ! W3_CESMCOUPLED
    !
    ...
    ! Formats
    !
9050 FORMAT (' TEST W3OUTG : ISEA, IX, IY, HS, L, Tm, THm, THs',     & ! W3_T
         ', FP0, THP0, FP1, THP1')
...
9055 FORMAT (2X,I8,2I8,F6.2,F7.1,F6.2,2F6.1,2(F6.3,F6.0))
  ...
  SUBROUTINE W3IOGO ( INXOUT, NDSOG, IOTST, IMOD )
    ...
    USE W3ADATMD, ONLY: FP1, THP1, DTDYN, &
         ...
         USSP, TAUOCX, TAUOCY
    !/
    ! PROBLEM (MV) - do not alias a variable to WRITE
    USE W3ODATMD, ONLY: NOGRP, NGRPP, IDOUT, UNDEF, NDST, NDSE,     &
         FLOGRD, IPASS => IPASS1, write_flag => WRITE1,   &
    ...
    !
    if (w3_t_flag) then
       WRITE (NDST,9000) IPASS, INXOUT, write_flag, NDSOG, IGRD, FILEXT
    end if
    !
    !
    ! open file ---------------------------------------------------------- *
    ...
    !
    IF ( write_flag) CALL FLUSH ( NDSOG )
    !
...
1002 FORMAT (/' *** WAVEWATCH III ERROR IN W3IOGO : '/               &
         ...
         '     IOSTAT =',I5/)
    !
9000 FORMAT (' TEST W3IOGO : IPASS =',I4,' INXOUT = ',A,          & ! W3_T
         ' WRITE = ',L1,' UNIT =',I3/                         &
         '               IGRD =',I3,' FEXT = ',A)
