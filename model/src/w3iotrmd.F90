...
CONTAINS
  ...
  SUBROUTINE W3IOTR ( NDSINP, NDSOUT, A, IMOD )
    ...
    USE W3ODATMD, ONLY: NDST, NDSE, IAPROC, NAPROC, NAPTRK, NAPERR, &
                        ...
                        MASK2, TRCKID, FNMPRE
    USE W3ODATMD, ONLY: IT0TRK, NRQTR, IRQTR ! W3_MPI
    !/
    USE W3TIMEMD
    ...
    INTEGER                 :: NREAD, NTRACK, NSPECO, NLOCO ! W3_T
    INTEGER                 :: ISPT ! W3_T3
    INTEGER                 :: IT, IROOT, IFROM, IERR_MPI ! W3_MPI
    INTEGER, ALLOCATABLE    :: STATUS(:,:) ! W3_MPI
    ...
    CHARACTER(LEN=17), SAVE :: TSTLOC = '                 ' ! W3_T1
    CHARACTER(LEN=1)        :: MAPSTR(NX) ! W3_T2
    !
    EQUIVALENCE (IXX(1),IX1) , (IXX(2),IX2) , (IYY(1),IY1) , (IYY(3),IY2)
    !/
    !/ ------------------------------------------------------------------- /
    !/
    if (w3_s_flag) then
          ...
          IF ( MASK1(IY,IX) ) THEN
             ...
             IF ( IAPROC .EQ. NAPTRK ) THEN
                ...
                IF ( TSTSTR .EQ. 'SEA' ) THEN
                   ...
                   ELSE
                      if (w3_t3_flag) then
                         WRITE (NDST,9040) IX, IY, ISEA, ISPT, 'RECEIVING'
                      end if
#ifdef W3_MPI
                      CALL MPI_RECV (ASPTRK, NSPEC, MPI_REAL,&
                           IFROM, IT, MPI_COMM_WAVE, STATUS, IERR_MPI )
#endif
                      !
                      DO IK=1, NK
                         DO ITH=1, NTH
...
1011 FORMAT (/' *** WAVEWATCH III WARNING IN W3IOTR : '/             &
         ...
         '     (ADITIONAL) TRACK OUTPUT DISABLED '/)
    !
9000 FORMAT (' TEST W3IOTR : MODEL TIME       : ',I8.8,I7.6) ! W3_T
...
9015 FORMAT (' TEST W3IOTR : PREPARING MASKS')
    !
9020 FORMAT (' TEST W3IOTR : SHIFTING MASKS') ! W3_T
9021 FORMAT (' TEST W3IOTR : OUTPUT TIME FRAME: ',I8.8,I7.6/      &
         '                                  ',I8.8,I7.6)
9022 FORMAT (' TEST W3IOTR : ENDING TIME REACHED')
    !
9030 FORMAT (' TEST W3IOTR : POINT-BY-POINT STATUS') ! W3_T1
9031 FORMAT ('      ',I8.8,I7.6,2F9.2,1X,A,1X,4I4,2L3)
9033 FORMAT (' TEST W3IOTR : END OF INPUT FILE') ! W3_T
...
9090 FORMAT (' TEST W3IOTR : NUMBER OF TRACK P: ',I10,            & ! W3_T
         '  (OUT OF',I10,')'/                                 &
         '               NUMBER OF SPECTRA: ',I10,            &
         '  (OUT OF',I10,')')
    !
9080 FORMAT (' TEST W3IOTR : OUTPUT TYPE DISABLED.') ! W3_T
    !/
    !/ End of W3IOTR ----------------------------------------------------- /
    !/
