...
CONTAINS
  ...
  SUBROUTINE WMIOBS ( IMOD )
    ...
    INTEGER          :: ISPROC  ! W3_DIST
    INTEGER          :: IP, IT0, ITAG, IERR_MPI ! W3_MPI
    ...
    DO J=1, NRGRD
       ...
       IF ( IAPROC .EQ. 1 ) THEN
          ...
          DO IP=1, NMPROC
             IF ( ALLPRC(IP,J) .NE. 0 .AND.                 &
                ...
                NRQ = NRQ + 1
                CALL MPI_ISEND ( BPSTGE(J,IMOD)%STIME, 2, MPI_INTEGER, IP-1, ITAG, MPI_COMM_MWAVE, &
                     IRQ(NRQ), IERR_MPI )
                if (w3_mpit_flag) then
                   WRITE (MDST,9081) NRQ, IP, ITAG-MTAG0, IRQ(NRQ), IERR_MPI
                end if
             END IF
          END DO
       END IF
       ...
       DO I=1, NBI2G(J,IMOD)
          ...
          if (w3_mpi_flag) then
             IT0    = MTAG0 + NRGRD**2 + SUM(NBI2G(1:J-1,:)) + SUM(NBI2G(J,1:IMOD-1))
          end if
          !
          DO IS=1, NSPEC
          ...
          DO IP=1, NMPROC
             IF ( ALLPRC(IP,J) .NE. 0 .AND. &    
                ...
                IF ( ITAG .GT. MTAG1 ) THEN
                   ...
                   CALL EXTCDE (1001) 
                END IF
                CALL MPI_ISEND ( SBPI(1,I), NSPEC, MPI_REAL, IP-1, ITAG, MPI_COMM_MWAVE, IRQ(NRQ), IERR_MPI )
                if (w3_mpit_flag) then
                   WRITE (MDST,9082) NRQ, JSEA, IP, ITAG-MTAG0, IRQ(NRQ), IERR_MPI
                end if
             END IF
          END DO
#endif
    ...
    ! Formats
    !
1001 FORMAT (/ ' *** ERROR WMIOBS : REQUESTED MPI TAG EXCEEDS', & ! W3_MPI
               ' UPPER BOUND (MTAG1) ***')
9000 FORMAT (  ' TEST WMIOBS : STAGING DATA FROM GRID ',I3) ! W3_T
9001 FORMAT (  ' TEST WMIOBS : NR. OF SPECTRA PER GRID : '/        &
               '             ',25I4)
    !
9010 FORMAT (  ' TEST WMIOBS : STAGING',I4,' SPECTRA FROM GRID ',  & ! W3_T
         I3,   ' TO GRID ',I3/                                &
               '               STARTING WITH SPECTRUM ',I4,        &
               ', RESPEC =',L2)
    !
9030 FORMAT (  ' TEST WMIOBS : TIME :',I10.8,I7.6) ! W3_T
    !
9080 FORMAT (/ ' MPIT WMIOBS: COMMUNICATION CALLS            '/ & ! W3_MPIT
               ' +------+------+------+------+--------------+'/ &
...
9084 FORMAT (  ' MPIT WMIOBS: NRQBPT:',I10/)
  ...
  SUBROUTINE WMIOBG ( IMOD, DONE ) 
    ...
    INTEGER                 :: IERR_MPI, IT0, ITAG, IFROM ! W3_MPI
    INTEGER                 :: ISEA, JSEA, ISPROC
    INTEGER                 :: ICOUNT ! W3_MPIT
    INTEGER, SAVE           :: IENT = 0 ! W3_S
    INTEGER, POINTER        :: VTIME(:)
    INTEGER, POINTER        :: NRQ, IRQ(:) ! W3_MPI
    INTEGER, ALLOCATABLE    :: STATUS(:,:)
    REAL                    :: DTTST, DT1, DT2, W1, W2
    REAL, POINTER           :: SBPI(:,:)
    REAL, ALLOCATABLE       :: TSTORE(:,:) ! W3_MPI
    LOGICAL                 :: FLAGOK
    LOGICAL                 :: FLAG ! W3_MPIT
    !/
    if (w3_s_flag) then
    ...
    IF ( NBISTA(IMOD) .EQ. 0 ) THEN
       ...
       DO J=1, NRGRD
          ...
          IF ( BPSTGE(IMOD,J)%INIT ) THEN
             ...
             ELSE
                IF ( SIZE(BPSTGE(IMOD,J)%SBPI(:,1)) .NE. SGRDS(J)%NSPEC .OR. &
                     SIZE(BPSTGE(IMOD,J)%SBPI(1,:)) .NE. NBI2G(IMOD,J) ) THEN
          ...
          IF ( DTTST .LE. 0. ) THEN
             ...
             NRQ    = NRQ + 1
             CALL MPI_IRECV ( BPSTGE(IMOD,J)%VTIME, 2, MPI_INTEGER, IFROM, ITAG, MPI_COMM_MWAVE, IRQ(NRQ), &
                  IERR_MPI )
             if (w3_mpit_flag) then
             ...
             END IF
             !
             IT0 = MTAG0 + NRGRD**2 + SUM(NBI2G(1:IMOD-1,:)) + SUM(NBI2G(IMOD,1:J-1))
             !
    ...
    IF ( NBISTA(IMOD) .EQ. 1 ) THEN
       ...
       IF ( PRESENT(DONE) ) THEN
          !
          CALL MPI_TESTALL ( NRQ, IRQ, FLAGOK, STATUS, IERR_MPI )
          !
          if (w3_mpit_flag) then
             ICOUNT = 0
             DO I=1, NRQ
                CALL MPI_TEST ( IRQ(I), FLAG, STATUS(1,1), IERR_MPI )
...
1002 FORMAT (/ ' *** ERROR WMIOBG : INITIAL DATA NOT AT INITAL ',     &
               'MODEL TIME ***'/)
1003 FORMAT (/ ' *** ERROR WMIOBG : UNEXPECTED SIZE OF STAGING', & ! W3_MPI
               ' ARRAY ***')
    !
9000 FORMAT (  ' TEST WMIOBG : GATHERING DATA FOR GRID ',I3) ! W3_T
...
9004 FORMAT (  ' TEST WMIOBG : DATA UP TO DATE')
    !
9010 FORMAT (  ' TEST WMIOBG : TEST DATA AVAILABILITY') ! W3_T
9011 FORMAT (  ' MPIT WMIOBG : NBISTA =',I2) ! W3_MPIT
...
9100 FORMAT (  ' MPIT WMIOBG : CONVERTING SPECTRA FROM GRID',I3)
    !
9020 FORMAT (  ' TEST WMIOBG : FILLING ABPI0/N AND TIMES') ! W3_T
9021 FORMAT (  ' TEST WMIOBG : NEXT VALID TIME FOR ABPIN:',I9.8,I7.6)
9022 FORMAT (  ' TEST WMIOBG : GETTING',I4,' SPECTRA FROM GRID ',  &
         I3,   ' STORING AT ',I3/               &
               '               WEIGHTS : ',2F6.3)
    !
9030 FORMAT (  ' TEST WMIOBG : DUMP DATA TO FILE') ! W3_T
    !
9040 FORMAT (  ' TEST WMIOBG : FILLING BBPI0/N') ! W3_T
    !/
    !/ End of WMIOBG ----------------------------------------------------- /
    !/
  ...
  SUBROUTINE WMIOBF ( IMOD )
    ...
    DO J=1, NRGRD
       ...
       !
       NRQ    = 0
       DEALLOCATE ( BPSTGE(J,IMOD)%IRQBPS ,  BPSTGE(J,IMOD)%TSTORE )
#endif
       !
       if (w3_t_flag) then
          WRITE (MDST,9010) J
    ...
    ! Formats
    !
9000 FORMAT ( ' TEST WMIOBF : FINALIZE STAGING DATA FROM GRID ',I3) ! W3_T
9010 FORMAT ( ' TEST WMIOBF : FINISHED WITH TARGET ',I3)
  ...
  SUBROUTINE WMIOHS ( IMOD )
    ...
    INTEGER          :: ITAG, IP, IT0, IERR_MPI          ! W3_MPI
    INTEGER          :: I1, I2
    INTEGER          :: IENT = 0                         ! W3_S
    INTEGER, POINTER :: NRQ, IRQ(:), NRQOUT, OUTDAT(:,:) ! W3_MPI
    REAL             :: DTOUTP
#ifdef W3_SHRD
    REAL, POINTER    :: SHGH(:,:,:)
#endif
    ...
    DO J=1, NRGRD
       ...
       DO I=1, NR
          ...
          DO IS=1, NSPEC
#ifdef W3_SHRD
             SHGH(IS,I2,I1) = VA(IS,JSEA) * SIG2(IS) / CG(1+(IS-1)/NTH,ISEA)
#endif
#ifdef W3_MPI
             SHGH(  IS,I  ) = VA(IS,JSEA) * SIG2(IS) / CG(1+(IS-1)/NTH,ISEA)
#endif
          END DO
          !
#ifdef W3_MPI
          IF ( IP .NE. IMPROC ) THEN
             NRQ    = NRQ + 1
             CALL MPI_ISEND ( SHGH(1,I), NSPEC, MPI_REAL, IP-1, ITAG, MPI_COMM_MWAVE, IRQ(NRQ), IERR_MPI )
             if (w3_mpit_flag) then
                WRITE (MDST,9082) NRQ, JSEA, IP, ITAG-MTAG1, IRQ(NRQ), IERR_MPI
             end if
          ELSE
             NRQOUT = NRQOUT + 1
             OUTDAT(NRQOUT,1) = I
    ...
    ! Formats
    !
1001 FORMAT (/' *** ERROR WMIOHS : REQUESTED MPI TAG EXCEEDS', & ! W3_MPI
         ' UPPER BOUND (MTAG2) ***')
9000 FORMAT ( ' TEST WMIOHS : STAGING DATA FROM GRID ',I3,        & ! W3_T
         '   FLGHG1 = ',L1)
9001 FORMAT ( ' TEST WMIOHS : NR. OF SPECTRA PER GRID : '/        &
         '             ',15I6)
    !
9010 FORMAT ( ' TEST WMIOHS : POSTING DATA TO GRID ',I3,          & ! W3_T
         '   NR = ',I6)
9011 FORMAT ( ' TEST WMIOHS : POSTING DATA TO GRID ',I3,          &
         '   NR = ',I6,'   TIME GAP = ',2F8.1)
    !
9030 FORMAT ( ' TEST WMIOHS : TIME :',I10.8,I7.6) ! W3_T
    !
9080 FORMAT (/' MPIT WMIOHS: COMMUNICATION CALLS            '/ & ! W3_MPIT
         ' +------+------+------+------+--------------+'/ &
         ' |  IH  |  ID  | TARG |  TAG |   handle err |'/ &
         ' +------+------+------+------+--------------+')
9082 FORMAT ( ' |',I5,' |',I5,' |',2(I5,' |'),I9,I4,' |')
...
9084 FORMAT ( ' MPIT WMIOHS: NRQHGS:',I10/)
  ...
  SUBROUTINE WMIOHG ( IMOD, DONE ) 
    ...
    INTEGER                 :: ITAG, IT0, IFROM, ILOC, NLOC,   & ! W3_MPI
         ISPROC, IERR_MPI, ICOUNT,       &
         I0, I1, I2
    INTEGER, SAVE           :: IENT = 0 ! W3_S
    INTEGER, POINTER        :: VTIME(:)
    INTEGER, POINTER        :: NRQ, IRQ(:), STATUS(:,:) ! W3_MPI
    REAL                    :: DTTST, WGTH
    REAL, POINTER           :: SPEC1(:,:), SPEC2(:,:), SPEC(:,:)
    REAL, POINTER           :: SHGH(:,:,:) ! W3_MPI
    LOGICAL                 :: FLGALL
    LOGICAL                 :: FLAGOK ! W3_MPI
    LOGICAL                 :: FLAG ! W3_MPIT
    !/
    if (w3_s_flag) then
    ...
    ! Formats
    !
9000 FORMAT (  ' TEST WMIOHG : GATHERING DATA FOR GRID ',I3/       & ! W3_T
               '               DTOUTP, FLGALL :',F8.1,L4)
9001 FORMAT (  ' TEST WMIOHG : NR. OF SPECTRA PER SOURCE GRID : '/ &
               '             ',25I4)
9003 FORMAT (  ' TEST WMIOHG : NO DATA TO BE GATHERED')
    !
9010 FORMAT (  ' TEST WMIOHG : TEST DATA AVAILABILITY FOR',I9.8,I7.6) ! W3_T
9011 FORMAT (  ' MPIT WMIOHG : HGHSTA =',I2) ! W3_MPIT
9013 FORMAT (  '               VTIME, DTTST :',I9.8,I7.6,1X,F8.1)
...
9019 FORMAT (  ' MPIT WMIOHG : RECEIVES FINISHED :',F6.1,'%')
    !
9020 FORMAT (  ' TEST WMIOHG : PROCESSING DATA GRID BY GRID') ! W3_T
...
9024 FORMAT (  '               MOVE SPECTRA TO PERMANENT STORAGE')
  ...
  SUBROUTINE WMIOHF ( IMOD )
    ...
    ! Formats
    !
9000 FORMAT ( ' TEST WMIOHF : FINALIZE STAGING DATA FROM GRID ',I3) ! W3_T
9010 FORMAT ( ' TEST WMIOHF : FINISHED WITH TARGET ',I3)
  ...
  SUBROUTINE WMIOES ( IMOD )
    ...
    INTEGER          :: IT0, ITAG, IP, IERR_MPI          ! W3_MPI
    INTEGER          :: IENT = 0                         ! W3_S
    INTEGER, POINTER :: NRQ, IRQ(:), NRQOUT, OUTDAT(:,:) ! W3_MPI
#ifdef W3_SHRD
    REAL, POINTER    :: SEQL(:,:,:)
    ...
    DO J=1, NRGRD
       ...
       !
       ALLOCATE ( EQSTGE(J,IMOD)%IRQEQS(NR), EQSTGE(J,IMOD)%OUTDAT(NR,3) )
       ...
       DO I=1, NR
          ...
          if ( w3_smc_flag .and. ( GTYPE .EQ. SMCTYPE )) then
             SEQL(:, I) = VA(:, JSEA) 
          else
             DO IS=1, NSPEC
#ifdef W3_SHRD
                SEQL(IS,I1,I2) = VA(IS,JSEA) * SIG2(IS) / CG(1+(IS-1)/NTH,ISEA)
#endif
#ifdef W3_MPI
                SEQL(  IS,I  ) = VA(IS,JSEA) * SIG2(IS)/ CG(1+(IS-1)/NTH,ISEA)
#endif
             END DO
          end if
          !
          IF ( IP .NE. IMPROC ) THEN
             ...
             if (w3_mpit_flag) then
                WRITE (MDST,9082) NRQ, JSEA, IP, ITAG-MTAG2, IRQ(NRQ), IERR_MPI
             end if
          ELSE 
             NRQOUT = NRQOUT + 1
             OUTDAT(NRQOUT,1) = I
    ...
    ! Formats
    !
1001 FORMAT (/' *** ERROR WMIOES : REQUESTED MPI TAG EXCEEDS', & ! W3_MPI
         ' UPPER BOUND (MTAG_UB) ***')
9000 FORMAT ( ' TEST WMIOES : STAGING DATA FROM GRID ',I3) ! W3_T
9001 FORMAT ( ' TEST WMIOES : NR. OF SPECTRA PER GRID : '/        &
         '             ',15I6)
    !
9010 FORMAT ( ' TEST WMIOES : POSTING DATA TO GRID ',I3,          & ! W3_T
         '   NR = ',I6)
9011 FORMAT ( ' TEST WMIOES : POSTING DATA TO GRID ',I3,          &
         '   NR = ',I6,'   TIME GAP = ',F8.1)
    !
9030 FORMAT ( ' TEST WMIOES : TIME :',I10.8,I7.6) ! W3_T
    !/
9080 FORMAT (/' MPIT WMIOES: COMMUNICATION CALLS            '/ & ! W3_MPIT
         ' +------+------+------+------+--------------+'/ &
         ' |  IH  |  ID  | TARG |  TAG |   handle err |'/ &
         ' +------+------+------+------+--------------+')
9082 FORMAT ( ' |',I5,' |',I5,' |',2(I5,' |'),I9,I4,' |')
...
9084 FORMAT ( ' MPIT WMIOES: NRQEQS:',I10/)
  ...
  SUBROUTINE WMIOEG ( IMOD, DONE ) 
    ...
    INTEGER, SAVE           :: IENT = 0 ! W3_S
    INTEGER                 :: IT0, ITAG, IFROM, IERR_MPI,     & ! W3_MPI
         NA, IP, I1, I2
    INTEGER                 :: ICOUNT ! W3_MPIT
    INTEGER, POINTER        :: VTIME(:)
    INTEGER, POINTER        :: NRQ, IRQ(:), STATUS(:,:) ! W3_MPI
    ...
    IF ( EQLSTA(IMOD) .EQ. 0 ) THEN
       ...
       DO J=1, NRGRD
          IF ( J .EQ. IMOD ) CYCLE
          NRQ    = NRQ + EQSTGE(IMOD,J)%NREC * EQSTGE(IMOD,J)%NAVMAX
       END DO
       ALLOCATE ( IRQ(NRQ) )
       IRQ    = 0
       NRQ    = 0
                ...
                DO IA=1, NA
                   ...
                   IF ( IP .NE. IMPROC ) THEN
                      ...
                      CALL MPI_IRECV ( SEQL(1,I,IA),           &
                           ...
                           IRQ(NRQ), IERR_MPI )
                      if (w3_mpit_flag) then
                         WRITE (MDST,9016) NRQ, JSEA, IP, ITAG-MTAG2, IRQ(NRQ), IERR_MPI
                      end if
                   END IF
                END DO
             END DO
    ...
    DO J=1, NRGRD
       ...
       if (w3_smc_flag .and. ( GTYPE .EQ. SMCTYPE )) then
          ...
          DO I=1, EQSTGE(IMOD,J)%NREC
             ...
             if (w3_smc_flag .and. ( NGRPSMC .GT. 0 )) then
                VA(:,JSEA) = SPEC(:,I)
             else
                DO IS=1, NSPEC
                   VA(IS,JSEA) = VA(IS,JSEA) + WGHT * SPEC(IS,I) / SIG2(IS) * CG(1+(IS-1)/NTH,ISEA)
                END DO
             end if ! !! NGRPSMC .GT. 0
          END DO
          !
       ...
       ENDIF  !  if (w3_smc_flag .and. ( GTYPE .EQ. SMCTYPE ))
       !
       !!  End 2.b J grid loop.
    END DO
    !
    ...
    ! Formats
    !
9000 FORMAT ( ' TEST WMIOEG : GATHERING DATA FOR GRID ',I4) ! W3_T
9001 FORMAT ( ' TEST WMIOEG : ',A,' PER SOURCE GRID : '/13X,20I5)
9002 FORMAT ( ' TEST WMIOEG : NO DATA TO BE GATHERED')
    !
9010 FORMAT ( ' TEST WMIOEG : TEST DATA AVAILABILITY FOR',I9.8,I7.6) ! W3_T
9011 FORMAT ( ' MPIT WMIOEG : EQLSTA =',I2) ! W3_MPIT
9012 FORMAT ( '               STAGING ARRAY FROM',I4,1X,A)
9013 FORMAT ( '               VTIME, DTTST :',I9.8,I7.6,1X,F8.1)
...
9019 FORMAT ( ' MPIT WMIOEG : RECEIVES FINISHED :',F6.1,'%')
    !
9020 FORMAT ( ' TEST WMIOEG : PROCESSING DATA GRID BY GRID') ! W3_T
...
9024 FORMAT ( '                  CONVERTING SPECTRA')
  ...
  SUBROUTINE WMIOEF ( IMOD )
    ...
    ! Formats
    !
9000 FORMAT ( ' TEST WMIOEF : FINALIZE STAGING DATA FROM GRID ',I3) ! W3_T
