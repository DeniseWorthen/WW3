...
MODULE W3SNL5MD
  ...
  ! Parameter list
  INTEGER                        :: NSEL
  INTEGER, ALLOCATABLE           :: PSEA(:)
  CHARACTER(LEN=10), ALLOCATABLE :: PNMS(:)
  !
CONTAINS
  !/ ------------------------------------------------------------------- /
  SUBROUTINE W3SNL5(A, CG, WN, FMEAN, T1ABS, U10, UDIR, JSEA, S, D, KURT)
    !/
    !/                  +-----------------------------------+
    !/                  | WAVEWATCH III           NOAA/NCEP |
    !/                  |           O. Gramstad             |
    ...
    USE W3GDATMD,  ONLY: NK, NTH, NSPEC, SIG, TH, GTYPE, RLGTYPE, CLGTYPE
    ! PROBLEM(MV) the following uses should be in W3GDATMD NOT W3WDATMD - fixing below
    USE W3WDATMD,  ONLY: QI5TBEG, QR5TIM0, QR5CVK0, QC5INT0, QR5TMIX
    USE W3WDATMD,  ONLY: QR5TIM0, QR5CVK0
    USE W3GDATMD , ONLY: QR5DPT, QR5OML, QI5DIS, QI5KEV, QI5NNZ, QI5IPL, QI5PMX
    USE W3ODATMD,  ONLY: FLOUT, NOPTS, TOSNL5, TOLAST, IAPROC, NAPOUT, SCREEN
...
113 FORMAT ((10ES11.3))
  ...
  SUBROUTINE INSNL5
    ...
    ! PROBLEM(MV) the following uses should be in W3GDATMD NOT W3GKEMD - fixing below
    !USE W3GKEMD , ONLY: QR5DPT, QR5OML, QI5DIS, QI5KEV, QI5NNZ, QI5IPL, QI5PMX
    USE W3GDATMD , ONLY: QR5DPT, QR5OML, QI5DIS, QI5KEV, QI5NNZ, QI5IPL, QI5PMX
  ...
  FUNCTION CALC_WBTv2 (A, CG, WN, DPT, U10, UDIR)
    ...
    REAL            :: ESIG(NK) ! E(σ)
    REAL            :: FACTOR, ET, HS, ETP, HSP, SIGP, KP, CGP, WSTP, TWBT
    !/
    !/ ------------------------------------------------------------------- /
    !/
    if (w3_s_flag) then
  ...
  SUBROUTINE INPOUT
    ...
    USE W3GDATMD  , ONLY: NK, NTH, SIG, TH, QR5DPT, FLAGLL, XGRD, YGRD, MAPSTA, MAPFS
    USE W3ODATMD  , ONLY: NOPTS, PTNME, PTLOC, IPTINT, IAPROC, NAPOUT, SCREEN
