...
40    FORMAT ('PRE_VA_',i4.4,'.txt')
  ...
  SUBROUTINE PDLIB_EXPLICIT_BLOCK(IMOD, FACX, FACY, RD10, RD20, DTG, VGX, VGY, LCALC)
    ...
    !CG(IK,IS)
    !print *,'AAA0 ', lbound(cg,1),ubound(cg,1),lbound(cg,2),ubound(cg,2)
    ! 0:51,0:1878523
    !do ik = lbound(cg,1),ubound(cg,1)
    !  do ip = lbound(cg,2),ubound(cg,2)
    ! do ik = 1,nk
    !    do ip = 1,npa
    !       cgsig1 = cg(ik,ip)
    !       !call wavnu3 (sig(ik), dw(iplg(ip)), ksig1, cgsig1)
    !       !if(isnan(csig1))write(5000+iaproc,'(a,2i8,4g14.7)')'XXX ',ip,ik,sig(ik),dw(iplg(ip)),ksig1,cgsig1
    !       !if (isnan(cgsig1))write(*,'(a,L,3i8,4g14.7)')'XXX ',lcalc,iaproc,ip,ik,sig(ik),dw(iplg(ip)),ksig1,cgsig1
    !       write(*,'(a,L,3i8,3g14.7)')'XXX ',lcalc,iaproc,ip,ik,sig(ik),dw(iplg(ip)),cgsig1
    !    enddo
    ! end do
    ...
    DO IK = 1, NK
      ...
      do ip = 1,npa
        ...
        do ith = 1,nth
          isp = ith + (ik-1)*nth
          !if (.not. lcalc) cgsig(ip) = cg(isp,ip)
          tmp = cgsig(ip)
          if(isnan(tmp)) write(1000+iaproc,'(a,L,3i8,4g14.7)')'XXX ',lcalc,ik,ip,ith,va(isp,ip),&
                cgsig(ip),clats(iplg(ip)),dw(iplg(ip))
