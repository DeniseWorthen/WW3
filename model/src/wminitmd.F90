...
CONTAINS
  ...
  SUBROUTINE WMINIT ( IDSI, IDSO, IDSS, IDST, IDSE, IFNAME, MPI_COMM, PREAMB )
    ...
    USE W3GDATMD, ONLY: NCel, NUFc, NVFc, NRLv, NBSMC        ! W3_SMC
    USE W3GDATMD, ONLY: NARC, NBAC, NSPEC, SMCTYPE           ! W3_SMC
    USE W3GDATMD, ONLY: FLAGLL, ICLOSE, GSU, X0, Y0, SX, SY  ! W3_MPI
    USE W3GDATMD, ONLY: XGRD, YGRD, DXDP, DXDQ, DYDP, DYDQ   ! W3_MPI
    USE W3GDATMD, ONLY: HQFAC, HPFAC, MAPSTA, MAPST2         ! W3_MPI
    USE W3GDATMD, ONLY: GRIDSHIFT, NSEAL, NK, NTH, XFR, FR1  ! W3_MPI
    USE W3GDATMD, ONLY: TH, DTMAX, DTCFL
    !
    USE W3GSRUMD
    !
    USE W3IDATMD, ONLY: W3NINP, W3SETI, W3DIMI
    USE W3IDATMD, ONLY: INFLAGS1, INFLAGS2, INPUTS, IINIT, JFIRST
    !
    ...
    USE W3ODATMD, ONLY: FLBPO, NFBPO, NBI, NDS, IAPROC
    USE W3ODATMD, ONLY: NAPFLD, NAPPNT, NAPTRK, NAPBPT
    USE W3ODATMD, ONLY: NAPPRT, NAPROC, FNMPRE, OUTPTS, NDST, NDSE
    USE W3ODATMD, ONLY: NOPTS, IOSTYP, UNIPTS, UPPROC, DTOUT
    USE W3ODATMD, ONLY: TOLAST, NOTYPE
    ...
    !
    USE W3TIMEMD, ONLY: PRINIT, PRTIME                       ! W3_MPRF
    ...
    USE WMMDATMD, ONLY: IMPROC, NMPROC, NMPSCR, NMPERR
    USE WMMDATMD, ONLY: NMPLOG, NMPUPT, STIME, ETIME, NMV, NMVMAX
    USE WMMDATMD, ONLY: TMV, AMV, DMV, NRGRD, NRINP, NRGRP, GRANK
    USE WMMDATMD, ONLY: GRGRP, INGRP, GRDHGH, GRDEQL, GRDLOW
    USE WMMDATMD, ONLY: ALLPRC, MODMAP, TSYNC, TMAX, TOUTP, TDATA
    USE WMMDATMD, ONLY: GRSTAT, DTRES, BCDUMP, FLGHG1, FLGHG2
    USE WMMDATMD, ONLY: INPMAP, IDINP, NGRPSMC
    USE WMMDATMD, ONLY: CLKDT1, CLKDT2, CLKFIN
    USE WMMDATMD, ONLY: MPI_COMM_MWAVE, MPI_COMM_GRD        ! W3_MPI
    USE WMMDATMD, ONLY: MPI_COMM_BCT, CROOT, FBCAST         ! W3_MPI
    ...
    CHARACTER(len=*) , INTENT(IN), OPTIONAL :: PREAMB
    !/
    !/ ------------------------------------------------------------------- /
    !/ Local parameters
    !/
    ...
    INTEGER                        :: IERR_MPI, BGROUP, LGROUP, IROOT ! W3_MPI
    INTEGER                        :: IENT = 0 ! W3_S
    ...
    REAL, ALLOCATABLE              :: X(:), Y(:), AMOVE(:), DMOVE(:), RP1(:), RPN(:)
    ...
    CHARACTER(LEN=18)              :: TFILE ! W3_SHRD, W3_DIST
    CHARACTER(LEN=18)              :: PFILE ! W3_MPRF
    ...
    ! 1.c Input file
    !
    IF ( MDSS.NE.MDSO .AND. NMPSCR.EQ.IMPROC ) WRITE (MDSS,910)  IFNAME, MDSI
    !
    OPEN (MDSI,FILE=TRIM(FNMPRE)//IFNAME,STATUS='OLD',ERR=2000, IOSTAT=IERR)
    REWIND (MDSI)
    READ (MDSI,'(A)',END=2001,ERR=2002) COMSTR
    IF (COMSTR.EQ.' ') COMSTR = '$'
    CALL WMUSET ( MDSS, MDSS, MDSI, .TRUE., 'INP', TRIM(FNMPRE)//IFNAME, 'Model control input file')
    !
    ...
    IF ( IMPROC .EQ. NMPLOG ) THEN
       ...
       IF ( MDSS.NE.MDSO .AND. NMPSCR.EQ.IMPROC )  WRITE (MDSS,911)  LFILE, MDSO
       CALL WMUSET ( MDSS, MDSS, MDSO, .TRUE., 'OUT', TRIM(FNMPRE)//LFILE, 'Log file')
    ELSE
       CALL WMUSET ( MDSS, MDSS, MDSO, .TRUE., 'XXX', 'Log file on other processors')
    END IF
    !
    IF ( MDST.NE.MDSO .AND. MDST.NE.MDSS .AND. TSTOUT ) THEN
       IFT    = LEN_TRIM(TFILE)
       OPEN (MDST,FILE=TRIM(FNMPRE)//TFILE(:IFT),ERR=2011,IOSTAT=IERR)
       CALL WMUSET ( MDSS, MDST, MDST, .TRUE., 'OUT', TRIM(FNMPRE)//TFILE(:IFT), 'Test output file')
    END IF
    !
    if (w3_mprf_flag) then
       IFT    = LEN_TRIM(PFILE)
       CALL WMUGET ( MDSS, MDST, MDSP, 'OUT' )
       CALL WMUSET ( MDSS, MDST, MDSP, .TRUE., 'OUT', TRIM(FNMPRE)//PFILE(:IFT), 'Profiling file')
    ...
    READ (MDSI,*,END=2001,ERR=2002) NRGRD, NRINP, UNIPTS, IOSTYP, UPPROC, PSHARE
    ...
    CALL WMUSET ( MDSE, MDST, NDSREC, .TRUE., 'I/O', NAME='...', DESC='Recyclable I/O (mod_def etc.)' )
    CALL WMUGET ( MDSE, MDST, SCRATCH, 'SCR' )
    CALL WMUSET ( MDSE, MDST, SCRATCH, .TRUE., DESC='Scratch file',  NAME=TRIM(FNMPRE)//'ww3_multi.scratch' )
    !
    ...
    DO
       DO J=NRGRD+1, 2*NRGRD
          IF ( TMPRNK(J) .EQ. RNKTMP ) THEN
             ...
             BCDUMP(I)  = BCDTMP(J)
             if (w3_t_flag) then
                WRITE (MDST,9031) I, MNAMES(I), INFLAGS1, TMPRNK(I), TMPGRP(I), RP1(I), RPN(I)
             end if
          END IF
       END DO
       IF ( I .EQ. NRGRD ) EXIT
    ...
    DO  ! unlabeled do loop
       ...
       ! ... End of loop in 5.j
       !
    END DO  ! DO  for unlabeled do loop
    !
    if (w3_t_flag) then
       DO I=1, NRGRD
          WRITE (MDST,9050) I
    ...
    !     *******************************************************
    !
    ALLOCATE ( ALLPRC(NMPROC,NRGRD) , MODMAP(NMPROC,NRGRP), LOADMP(NMPROC,NRGRP) )
    !
    ALLPRC = 0
    MODMAP = 0
    LOADMP = 0
    ...
    END IF
    !
    IF ( UPPROC .AND. MDSS.NE.MDSO .AND. NMPSCR.EQ.IMPROC ) WRITE (MDSS,972) NMPUPT
    !
    ! 7.b.2 Other output
    !
    ...
    DO I=1, NRGRD
       ...
       if (w3_mpi_flag) then
          ...
          FBCAST = NAPLOC .NE. NCPROC .OR. ( IOSTYP.GT.1 .AND. .NOT.PSHARE )
       end if
       DO J=IP1, IPN
          TMPRNK(1+J-IP1) = J - 1
       END DO
       ...
       !
#ifdef W3_MPI
       CALL MPI_GROUP_INCL ( BGROUP, NAPADD, TMPRNK, LGROUP, IERR_MPI )
       CALL MPI_COMM_CREATE ( MPI_COMM_MWAVE, LGROUP, MPI_COMM_GRD, IERR_MPI )
       CALL MPI_GROUP_FREE ( LGROUP, IERR_MPI )
       if (MPI_COMM_GRD .NE. MPI_COMM_NULL) then
          mpi_comm_grd_not_null = .true.
       else
       ...
       ELSE
          ...
          ELSE IF ( IOSTYP .EQ. 3 ) THEN
             ...
             IF ( ODAT(23,I) .GT. 0 ) OUTSTR(5) = STOUT
             !
          END IF ! IF ( IOSTYP .EQ. 2 )
          !
       END IF ! IF ( IOSTYP .LE. 1 )
       !
       IF ( MDSS.NE.MDSO .AND. NMPSCR.EQ.IMPROC ) WRITE (MDSS,975) MNAMES(I), IP1, IPN, OUTSTR
       IF ( NMPLOG .EQ. IMPROC ) WRITE (MDSO,1975)MNAMES(I), IP1, IPN, OUTSTR
       !
#ifdef W3_MPI
       IF ( FBCAST ) THEN
          TMPRNK(1) = IP1 - 1
          ...
          DO J=1, NMPROC
             ...
             END IF
          END DO
          CALL MPI_GROUP_INCL ( BGROUP, NAPBCT, TMPRNK, LGROUP, IERR_MPI )
          CALL MPI_COMM_CREATE ( MPI_COMM_MWAVE, LGROUP, MPI_COMM_BCT, IERR_MPI )
          CALL MPI_GROUP_FREE ( LGROUP, IERR_MPI )
       END IF
#endif
       !
    ...
    DO I=1, NRGRD
       ...
       END DO
       IF ( MDSS.NE.MDSO .AND. NMPSC2.EQ.IMPROC )  WRITE (MDSS,981) I, MNAMES(I)(1:J)
       !
       if (w3_mpi_flag) then
          CALL MPI_BARRIER (MPI_COMM_MWAVE,IERR_MPI)
       end if
       ...
       if (w3_smc_flag) then
          ...
          IF ( IMPROC .EQ. NMPERR ) WRITE(MDSE,*) "GRID IMPROC GTYPE", I, IMPROC, GRIDS(I)%GTYPE
       end if
       !
       if (w3_t_flag) then
    ...
    DO I=1, NRGRD
       ...
       IF ( FBCAST .AND. MPI_COMM_BCT.NE.MPI_COMM_NULL ) THEN
          ...
          CALL MPI_BCAST ( TOUTP(1,I), 2, MPI_INTEGER, 0, MPI_COMM_BCT, IERR_MPI )
          CALL MPI_BCAST ( TSYNC(1,I), 2, MPI_INTEGER, 0, MPI_COMM_BCT, IERR_MPI )
          CALL MPI_BCAST ( GRSTAT(I) , 1, MPI_INTEGER, 0, MPI_COMM_BCT, IERR_MPI )
          !
          ! 8.a.4 Grid sizes etc. for processors that are not used.
          !
          CALL MPI_BCAST ( FLAGLL,1   , MPI_LOGICAL,   0, MPI_COMM_BCT, IERR_MPI )
          CALL MPI_BCAST ( GTYPE, 1   , MPI_INTEGER,   0, MPI_COMM_BCT, IERR_MPI )
          CALL MPI_BCAST ( ICLOSE,1   , MPI_INTEGER,   0, MPI_COMM_BCT, IERR_MPI )
          CALL MPI_BCAST ( NX   , 1   , MPI_INTEGER,   0, MPI_COMM_BCT, IERR_MPI )
          CALL MPI_BCAST ( NY   , 1   , MPI_INTEGER,   0, MPI_COMM_BCT, IERR_MPI )
          CALL MPI_BCAST ( X0   , 1   , MPI_REAL   ,   0, MPI_COMM_BCT, IERR_MPI )
          CALL MPI_BCAST ( SX   , 1   , MPI_REAL   ,   0, MPI_COMM_BCT, IERR_MPI )
          CALL MPI_BCAST ( Y0   , 1   , MPI_REAL   ,   0, MPI_COMM_BCT, IERR_MPI )
          CALL MPI_BCAST ( SY   , 1   , MPI_REAL   ,   0, MPI_COMM_BCT, IERR_MPI )
          CALL MPI_BCAST ( NSEA , 1   , MPI_INTEGER,   0, MPI_COMM_BCT, IERR_MPI )
          CALL MPI_BCAST ( NSEAL, 1   , MPI_INTEGER,   0, MPI_COMM_BCT, IERR_MPI )
          CALL MPI_BCAST ( DTMAX, 1   , MPI_REAL,      0, MPI_COMM_BCT, IERR_MPI )
          CALL MPI_BCAST ( DTCFL, 1   , MPI_REAL,      0, MPI_COMM_BCT, IERR_MPI )
          CALL MPI_BCAST ( FILEXT, 10 , MPI_CHARACTER, 0, MPI_COMM_BCT, IERR_MPI )
          IF ( mpi_comm_grd_not_null) then
             if (w3_smc_flag) then
                ...
                ! PROBLEM (MV) all optional variables should be with keyword/value pairs
                CALL W3DIMX  ( I, NX, NY, NSEA, MDSE, MDST, &
                     MCel=1, MUFc=1, MVFc=1, MRLv=1, MBSMC=1, MARC=1, MBAC=1, MSPEC=1)
             else
                CALL W3DIMX  ( I, NX, NY, NSEA, MDSE, MDST)
             end if
          end IF
          CALL MPI_BCAST ( HQFAC , NX*NY, MPI_REAL,             0, MPI_COMM_BCT, IERR_MPI )
          CALL MPI_BCAST ( HPFAC , NX*NY, MPI_REAL,             0, MPI_COMM_BCT, IERR_MPI )
          CALL MPI_BCAST ( XGRD  , NX*NY, MPI_DOUBLE_PRECISION, 0, MPI_COMM_BCT, IERR_MPI )
          CALL MPI_BCAST ( YGRD  , NX*NY, MPI_DOUBLE_PRECISION, 0, MPI_COMM_BCT, IERR_MPI )
          IF ( mpi_comm_grd_not_null ) then
             GSU = W3GSUC( .FALSE., FLAGLL, ICLOSE, XGRD, YGRD )
          end IF
          CALL MPI_BCAST ( DXDP      , NX*NY , MPI_REAL,             0, MPI_COMM_BCT, IERR_MPI )
          CALL MPI_BCAST ( DXDQ      , NX*NY , MPI_REAL,             0, MPI_COMM_BCT, IERR_MPI )
          CALL MPI_BCAST ( DYDP      , NX*NY , MPI_REAL,             0, MPI_COMM_BCT, IERR_MPI )
          CALL MPI_BCAST ( DYDQ      , NX*NY , MPI_REAL,             0, MPI_COMM_BCT, IERR_MPI )
          CALL MPI_BCAST ( MAPSTA    , NX*NY , MPI_INTEGER,          0, MPI_COMM_BCT, IERR_MPI )
          CALL MPI_BCAST ( MAPST2    , NX*NY , MPI_INTEGER,          0, MPI_COMM_BCT, IERR_MPI )
          CALL MPI_BCAST ( GRIDSHIFT , 1     , MPI_DOUBLE_PRECISION, 0, MPI_COMM_BCT, IERR_MPI )
          !
          CALL MPI_BCAST ( NK   , 1, MPI_INTEGER, 0, MPI_COMM_BCT, IERR_MPI )
          CALL MPI_BCAST ( NTH  , 1, MPI_INTEGER, 0, MPI_COMM_BCT, IERR_MPI )
          CALL MPI_BCAST ( XFR  , 1, MPI_REAL   , 0, MPI_COMM_BCT, IERR_MPI )
          CALL MPI_BCAST ( FR1  , 1, MPI_REAL   , 0, MPI_COMM_BCT, IERR_MPI )
          IF ( mpi_comm_grd_not_null) then
             CALL W3DIMS ( I, NK, NTH, MDSE, MDST )
          end IF
          CALL MPI_BCAST ( TH, NTH, MPI_REAL  , 0, MPI_COMM_BCT, IERR_MPI )
          !
          CALL MPI_BCAST ( NAPROC , 1, MPI_INTEGER , 0, MPI_COMM_BCT, IERR_MPI )
          CALL MPI_BCAST ( NAPPNT , 1, MPI_INTEGER , 0, MPI_COMM_BCT, IERR_MPI )
          CALL MPI_BCAST ( NBI    , 1, MPI_INTEGER , 0, MPI_COMM_BCT, IERR_MPI )
          CALL MPI_BCAST ( FLOUT  , 8, MPI_LOGICAL , 0, MPI_COMM_BCT, IERR_MPI )
          CALL MPI_BCAST ( DTOUT  , 8, MPI_REAL    , 0, MPI_COMM_BCT, IERR_MPI )
          CALL MPI_BCAST ( TONEXT ,16, MPI_INTEGER , 0, MPI_COMM_BCT, IERR_MPI )
          CALL MPI_BCAST ( TOLAST ,16, MPI_INTEGER , 0, MPI_COMM_BCT, IERR_MPI )
          !
       END IF ! IF ( FBCAST .AND. MPI_COMM_BCT.NE.MPI_COMM_NULL )
    END DO ! DO I=1, NRGRD
    CALL MPI_BARRIER (MPI_COMM_MWAVE,IERR_MPI)
#endif
    !
    ...
    DO I=1, NRGRD
       ...
       END IF
       !
       IF ( (w3_shrd_flag .and. (.NOT. FLRBPI(I) .AND. FLBPI )) .or. &
            (w3_mpi_flag  .and. (.NOT. FLRBPI(I) .AND. FLBPI .AND. mpi_comm_grd_not_null))) then
    ...
    DO I=1, NRGRD
       if (w3_mpi_flag) then
          CALL WMSETM ( I, MDSE, MDST )
          IF ( mpi_comm_grd_not_null ) CALL WMIOBS ( I )
       end if
       if (w3_shrd_flag) then
          CALL WMIOBS ( I )
       end if
    ...
    if (w3_mpi_flag) then
       DO I=1, NRGRD
          CALL WMSETM ( I, MDSE, MDST )
          IF ( mpi_comm_grd_not_null ) CALL WMIOBF ( I )
       END DO
    end if
    !
    ...
    IF ( UNIPTS ) THEN
       ...
       IF ( DTTST .LT. 0. ) THEN
          UNIPTS = .FALSE.
       ELSE
          CALL WMIOPP ( OT2(0)%NPTS, OT2(0)%X, OT2(0)%Y, OT2(0)%PNAMES )
       END IF
       !
#ifdef W3_MPI
       DO I=1, NRGRD
          ...
          CALL W3SETO ( I, MDSE, MDST )
          IF ( FBCAST .AND. MPI_COMM_BCT.NE.MPI_COMM_NULL ) THEN
             CALL MPI_BCAST ( NOPTS, 1, MPI_INTEGER, 0, MPI_COMM_BCT, IERR_MPI )
          END IF
       END DO
#endif
    END IF ! IF  (UNIPTS )
...
988 FORMAT ( '       Input data grid',I3,' [',A,']')
    !
990 FORMAT (1X,3F12.3,' WMINIT',1X,A) ! W3_MPRF
    !
...
1082 FORMAT ( '  No boundary data dump for grid',I3/)
    !
9000 FORMAT ( ' TEST WMINIT : UNIT NUMBERS    : ',5I6/            & ! W3_T
         '               INPUT FILE NAME : ',A)
    !
9020 FORMAT ( ' TEST WMINIT : UNIT NUMBERS FOR GRIDS (',A,')'/    & ! W3_T
...
9037 FORMAT ( '             ',2I3,':',20I3)
    !
9050 FORMAT ( ' TEST WMINIT : GRID NUMBER',I3,' =================') ! W3_T
...
9052 FORMAT ( ' TEST WMINIT : FLGRD  : ',5(5L2,1X)/24X,5(5L2,1X))
    !
9060 FORMAT ( ' TEST WMINIT : GRID MOVEMENT DATA') ! W3_T
9061 FORMAT ( '             ',I8.8,I7,1X,2F8.2)
    !
9070 FORMAT ( ' TEST WMINIT : ALLPRC ') ! W3_T
9071 FORMAT ( ' ',I3,'  : ',250I3)
8042 FORMAT ( ' TEST WMINIT : MODMAP ')
8043 FORMAT ( ' TEST WMINIT : LOADMP ')
8044 FORMAT ( '        ',I3,'  : ',250I2)
    !
9080 FORMAT ( ' TEST WMINIT : MODEL INITIALIZATION') ! W3_T
...
9084 FORMAT ( '               ',I4,17(2X,A3))
  ...
  SUBROUTINE WMINITNML ( IDSI, IDSO, IDSS, IDST, IDSE, IFNAME,       &
    ...
    USE W3FLDSMD, ONLY: W3FLDO
    USE W3IDATMD, ONLY: W3NINP, W3SETI, W3DIMI
    USE W3IDATMD, ONLY: INFLAGS1, INPUTS, IINIT, JFIRST, INFLAGS2
    USE W3INITMD, ONLY: W3INIT
    USE W3INITMD, ONLY: WWVER
    ...
    INTEGER                        :: IFT, ILOOP
    !
    INTEGER                        :: TTIME(2), TOUT(2), STMPT(2), ETMPT(2), TLST(2)
    INTEGER                        :: IERR_MPI, BGROUP, LGROUP, IROOT ! W3_MPI
    INTEGER                        :: IENT = 0 ! W3_S
    INTEGER,           ALLOCATABLE :: MDS(:,:), NTRACE(:,:), ODAT(:,:)
    INTEGER,           ALLOCATABLE :: TMPRNK(:), TMPGRP(:), NINGRP(:)
    ...
    if (w3_t_flag) then
       DO I=1, NRGRD
          WRITE (MDST,9050) I
          ! PROBLEM (MV) - format 9053 is not defined - commenting this out
...
988 FORMAT ( '       Input data grid',I3,' [',A,']')
    !
990 FORMAT (1X,3F12.3,' WMINITNML',1X,A) ! W3_MPRF
    !
998 FORMAT ( '  Running the model :'/                               &
         ' --------------------------------------------------'/)
...
1082 FORMAT ( '  No boundary data dump for grid',I3/)
    !
9000 FORMAT ( ' TEST WMINITNML : UNIT NUMBERS    : ',5I6/          & ! W3_T
         '               INPUT FILE NAME : ',A)
    !
9020 FORMAT ( ' TEST WMINITNML : UNIT NUMBERS FOR GRIDS (',A,')'/  & ! W3_T
         15X,'GRID MDS(1-13)',43X,'NTRACE')
9021 FORMAT (14X,16I4)
...
9037 FORMAT ( '             ',2I3,':',20I3)
    !
9050 FORMAT ( ' TEST WMINITNML : GRID NUMBER',I3,' =================') ! W3_T
9051 FORMAT ( ' TEST WMINITNML : ODAT   : ',I9.8,I7.6,I7,I9.8,I7.6,  &
         5(/24X,I9.8,I7.6,I7,I9.8,I7.6) )
9052 FORMAT ( ' TEST WMINITNML : FLGRD  : ',5(5L2,1X)/24X,5(5L2,1X))
    !
9060 FORMAT ( ' TEST WMINITNML : GRID MOVEMENT DATA') ! W3_T
9061 FORMAT ( '             ',I8.8,I7,1X,2F8.2)
    !
9070 FORMAT ( ' TEST WMINITNML : ALLPRC ') ! W3_T
9071 FORMAT ( ' ',I3,'  : ',250I3)
8042 FORMAT ( ' TEST WMINITNML : MODMAP ')
8043 FORMAT ( ' TEST WMINITNML : LOADMP ')
8044 FORMAT ( '        ',I3,'  : ',250I2)
    !
9080 FORMAT ( ' TEST WMINITNML : MODEL INITIALIZATION') ! W3_T
9081 FORMAT ( '               MODEL AND TIME   :',I4,I10.8,I8.6)
9082 FORMAT ( '               STATUS AND TIMES :',I4,3(I10.8,I8.6))
