...
MODULE W3BULLMD
  ...
  REAL                 :: HST(NPTAB,2), TPT(NPTAB,2), DMT(NPTAB,2)
  CHARACTER(LEN=129)   :: ASCBLINE 
  CHARACTER(LEN=664)   :: CSVBLINE 
  CHARACTER(LEN=67)    :: CASCBLINE  ! W3_NCO
  LOGICAL              :: IYY(NPMAX)
  !/    
...
CONTAINS
  ...
  SUBROUTINE W3BULL ( NPART, XPART, DIMXP, UABS, UD, IPNT, IOUT, TIMEV ) 
    ...
    CHARACTER(LEN=67)  :: CBLANK, CTAIL !, CASCBLINE ! W3_NCO
    CHARACTER(LEN=15)  :: PART
    CHARACTER(LEN=9)   :: CPART         ! W3_NCO
    CHARACTER(LEN=664) :: BLANK2        !,CSVBLINE
    CHARACTER          :: STIME*8,FORM*20,FORM1*2
    CHARACTER(LEN=16)  :: PART2
    integer, PARAMETER :: DHSMAX = 1.50
    integer, PARAMETER :: DTPMAX = 1.50
    integer, PARAMETER :: DDMMAX = 15.   
    integer, PARAMETER :: DDWMAX = 30.   
    integer, PARAMETER :: AGEMIN = 0.8  
    !/
    !/ ------------------------------------------------------------------- /
    !
    ! 1.  Initializations
    ...
    WRITE (CSVBLINE(11:12),'(I2)') INT(TIME(1)/100)-100*INT(TIME(1)/10000)
