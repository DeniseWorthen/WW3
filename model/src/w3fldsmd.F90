...
CONTAINS
  ...
  SUBROUTINE W3FLDO ( INXOUT, IDFLD, NDS, NDST, NDSE, NX, NY,     &
    ...
    !
    USE W3ODATMD, only : IAPROC ! W3_DEBUGFLS
...
1010 FORMAT (/' *** WAVEWATCH III ERROR IN W3FLDO : '/          &
    ...
    !
9000 FORMAT (' TEST W3FLDO : INXOUT : ',A/                     & ! W3_T
         '               IDFLD  : ',A/                     &
         '               NDS    : ',I2/                    &
         '               NDST   : ',I2/                    &
         '               NDSE   : ',I2/                    &
...
1006 FORMAT (/' *** WAVEWATCH III ERROR IN W3FLDTIDE2 : '/         &
         '     PREMATURE END OF ',A,' FILE'/)
1007 FORMAT (/' *** WAVEWATCH III ERROR IN W3FLDTIDE2 : '/         & ! W3_TIDE
  ...
  SUBROUTINE W3FLDG (INXOUT, IDFLD, NDS, NDST, NDSE, MX, MY, &
    ...
    DO
       ...
       IF ( WRITE ) THEN
          ...
          IF ( .NOT. FL2D ) THEN
             J = 1
             WRITE (NDS,ERR=804,IOSTAT=ISTAT) ((FA0(IX,IY),IX=1,NX),IY=1,NY)
          ELSE
             J = 1
             WRITE (NDS,ERR=804,IOSTAT=ISTAT) ((FX0(IX,IY),IX=1,NX),IY=1,NY)
             J = 2
             WRITE (NDS,ERR=804,IOSTAT=ISTAT) ((FY0(IX,IY),IX=1,NX),IY=1,NY)
             J = 3
             IF ( FLST ) WRITE (NDS,ERR=804,IOSTAT=ISTAT) ((FA0(IX,IY),IX=1,NX),IY=1,NY)
          END IF
          !
          EXIT
          !
       ...
       ELSE
          ...
          IF (FLCOUPL) THEN 
             ...
             IF ( (ID_OASIS_TIME.EQ.0 .AND. ( FLFRST .OR. CPLT0 )) .OR. (ID_OASIS_TIME.GT.0)) THEN
                ...
                CALL RCV_FIELDS_FROM_ATMOS(COUPL_COMM, IDFLD, FXN, FYN, FAN)
#endif
#ifdef W3_OASOCM
                ! Getting UCUR (CX), VCUR (CY), WLV from ocean model
                CALL RCV_FIELDS_FROM_OCEAN(COUPL_COMM, IDFLD, FXN, FYN, FAN)
#endif
#ifdef W3_OASICM
                ! Getting ICEF from ice model
                CALL RCV_FIELDS_FROM_ICE(COUPL_COMM, IDFLD, FXN, FYN, FAN)
#endif
          ...
          ELSE 
             ...
             IF ( .NOT. FL2D ) THEN
                ...
                J = 1
                READ (NDS,END=806,ERR=807,IOSTAT=ISTAT) ((FAN(IX,IY),IX=1,NX),IY=1,NY)
             ELSE
                J = 1
                READ (NDS,END=806,ERR=807,IOSTAT=ISTAT) ((FXN(IX,IY),IX=1,NX),IY=1,NY)
                J = 2
                READ (NDS,END=806,ERR=807,IOSTAT=ISTAT) ((FYN(IX,IY),IX=1,NX),IY=1,NY)
                ...
                J = 3
                IF ( FLST ) READ (NDS,END=806,ERR=807,IOSTAT=ISTAT) ((FAN(IX,IY),IX=1,NX),IY=1,NY)
             END IF
#ifdef W3_OASIS
          END IF
#endif
...
1007 FORMAT (/' *** WAVEWATCH III ERROR IN W3FLDG : '/               &
    ...
    !
9000 FORMAT (' TEST W3FLDG : INXOUT     : ',A/                    & ! W3_T
         '               IDFLD      : ',A/                    &
         '               NDS(T/E)   :',3I4/                   &
         '               MX, MY     :',2I8/                   &
         '               NX, NY     :',2I8/                   &
...
9001 FORMAT (' TEST W3FLDG : WRITE      :',L4/                    &
         ...
         '               FIRST      :',L4)
    !
9020 FORMAT (' TEST W3FLDG : FIELD SHIFTED') ! W3_T
9021 FORMAT ('               NO FIELD TO SHIFT')
    !
9030 FORMAT (' TEST W3FLDG : WRITE TIME : ',I8,I7.6) ! W3_T
9031 FORMAT (' TEST W3FLDG : NEW TIME   : ',I8,I7.6)
9032 FORMAT (' TEST W3FLDG : NEW TIME   : ',I8,I7.6,              &
         ' EOF (IERR =',I3,')')
    !
9040 FORMAT (' TEST W3FLDG : FILLING IN FIRST FIELD') ! W3_T
...
1007 FORMAT (/' *** WAVEWATCH III ERROR IN W3FLDD : '/               &
    ...
    !
9000 FORMAT (' TEST W3FLDD : INXOUT     : ',A/                    & ! W3_T
         '               IDFLD      : ',A/                    &
         '               NDS(T/E)   :',3I4/                   &
         '               NR, ND     :',2I4/                   &
         '               TIME       :',I8,I7.6/               &
...
9001 FORMAT (' TEST W3FLDD : WRITE      :',L4/                    &
         '               SIZE       :',L4)
    !
9020 FORMAT (' TEST W3FLDD : WRITE TIME : ',I8,I7.6/              & ! W3_T
         '               RECORDS    : ',I6)
9021 FORMAT (' TEST W3FLDD : NEW TIME   : ',I8,I7.6/              &
         '               RECORDS    : ',I6)
    !
9030 FORMAT (' TEST W3FLDD : FINAL TIME : ',I8,I7.6) ! W3_T
    !/
    !/ End of W3FLDD ----------------------------------------------------- /
    !/
...
910 FORMAT ( ' *** WARNING W3FLDP : SEA POINT ON LAND MASK ', &
    ...
    !
9000 FORMAT ( ' TEST W3FLDP : NDSM/T/E : ',3I8/                & ! W3_T
         '               MX, MY   : ',2I8/                &
         '               NX, NY   : ',2I8/                &
         '               ILAND    : ',I8/                 &
         '               MXI, MYI : ',2I8/                &
...
9001 FORMAT ( ' TEST W3FLDP : GRID SEARCH INFO -- OUTPUT FROM W3GSUP')
    !
9010 FORMAT ( ' TEST W3FLDP : IX =',I4,' IY =',I4,             & ! W3_T1
         '  LONGITUDE =',F8.2, '  LATITUDE =',F8.2,       &
         '  ================================')
    !
9020 FORMAT ( ' TEST W3FLDP : IX =',I4,' IY =',I4,             & ! W3_T1
         '  LONGITUDE =',F8.2, '  LATITUDE =',F8.2,       &
         '   (',A,')')
9021 FORMAT ( '                   ***** OUT OF RANGE *****')
    !
9031 FORMAT ( ' TEST W3FLDP : FINAL INTERPOLATION DATA (IX,IY,R)', & ! W3_T
         4(/'                                ',2I4,f7.3))
...
1001 FORMAT (/' *** WAVEWATCH III ERROR IN W3FLDH : '/               &
    ...
    !
9000 FORMAT (' TEST W3FLDH : J, NDST/E  : ',3I4/                  & ! W3_T
    '               DIMENSIONS : ',4I4/                  &
         '               T0         : ',I8,I7.6/              &
         '               TN         : ',I8,I7.6/              &
         '               NH(M)      : ',2I4/                  &
...
9001 FORMAT (' TEST W3FLDH : FIRST FIELD : ',L2)
    !
9020 FORMAT (' TEST W3FLDH : FIELD SHIFTED') ! W3_T
9021 FORMAT ('               NO FIELD TO SHIFT')
    !
9050 FORMAT (' TEST W3FLDH : NEW VALUE(S) : ',3F8.2) ! W3_T
...
1001 FORMAT (/' *** WAVEWATCH III ERROR IN W3FLDM : '/               &
         '     ILLEGAL FIELD ID NR : ',I4/)
    !
9000 FORMAT (' TEST W3FLDM : J, NDST/E  : ',3I4/                  & ! W3_T
    '               T0         : ',I8,I7.6/              &
         '               TN         : ',I8,I7.6/              &
         '               NH(M)      : ',2I4/                  &
         '               TF0        : ',I8,I7.6/              &
...
9001 FORMAT (' TEST W3FLDM : FIRST FIELD : ',L2)
    !
9020 FORMAT (' TEST W3FLDM : FIELD SHIFTED') ! W3_T
9021 FORMAT ('               NO FIELD TO SHIFT')
    !
9050 FORMAT (' TEST W3FLDM : NEW VALUE(S) : ',2F8.2) ! W3_T
