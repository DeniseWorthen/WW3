...
CONTAINS
  ...
  SUBROUTINE W3WAVE ( IMOD, ODAT, TEND, STAMP, NO_OUT)
    ...
    !
    !/ ------------------------------------------------------------------- /
    USE CONSTANTS   , only : UNDEF, RADIUS, DERA, DAIR
    ...
    USE W3WDATMD    , only : VAOLD, VSTOT, VDTOT, SHAVETOT ! W3_PDLIB
    USE W3WDATMD    , only : TIC5                          ! W3_IS2
    USE W3WDATMD    , only : TIME00, TIMEEND               ! W3_OASIS
    ...
    USE W3IDATMD    , only : INFLAGS1, FLLEV, FLCUR, FLWIND, FLICE, FLTAUA, FLRHOA, FLCUR
    ...
    USE W3IDATMD    , only : TI5                        ! W3_IS2
    USE W3IDATMD    , only : ICEP1, ICEP2, ICEP3, ICEP4 ! W3_IC3
    USE W3IDATMD    , only : W3SETI
    !
    USE W3ODATMD    , only : FLOUT, FLOGRD, FLOGR2, FLBPI, NOGE, NTPROC
    ...
    USE W3GDATMD    , only : IC3PARS                                               ! W3_IC3
    USE W3GDATMD    , only : IOBP_LOC, IOBPD_LOC, IOBPA_LOC, IOBDP_LOC             ! W3_PDLIB
    USE W3GDATMD    , only : FSSOURCE, FSTOTALEXP, FSTOTALIMP                      ! W3_PDLIB
    USE W3GDATMD    , only : FSN, FSPSI, FSFCT, FSNIMP, FLCTH                      ! W3_PDLIB
    USE W3GDATMD    , only : FSREFRACTION, FLCK, FSFREQSHIFT, FLSOU                ! W3_PDLIB
    ...
    USE W3UPDTMD    , only : W3UIC5 ! W3_IS2
    ...
    USE W3SERVMD    , only : W3ACTURN ! W3_SMC
    ! PROBLEM (MV): the following two uses are needed - compilation would not have occurred otherwise
    ...
    USE W3PARALL    , only : LSLOC                     ! W3_PDLIB
    USE W3PARALL    , only : PDLIB_NSEAL, PDLIB_NSEALM ! W3_PDLIB
    ...
    DO
       ...
       ELSE
          IT0    = 1
       END IF
       call print_memcheck(IAPROC+40000, 'memcheck_____:'//' WW3_WAVE')
       !
       if (w3_t_flag) then
       ...
       DO IT = IT0, NT
          ...
          call print_memcheck(IAPROC+40000, 'memcheck_____:'//' WW3_WAVE TIME LOOP 0')
          !
          ITIME  = ITIME + 1
          !
          ...
          call print_memcheck(IAPROC+40000, 'memcheck_____:'//' WW3_WAVE TIME LOOP 1')
          !
          ...
          END IF
          call print_memcheck(IAPROC+40000, 'memcheck_____:'//' WW3_WAVE TIME LOOP 2')
          ...
          call print_memcheck(IAPROC+40000, 'memcheck_____:'//' WW3_WAVE TIME LOOP 3a')
          ...
          IF ( FLCUR  ) THEN
             ...
             call print_memcheck(IAPROC+40000, 'memcheck_____:'//' WW3_WAVE TIME LOOP 3b')
          ...
          call print_memcheck(IAPROC+40000, 'memcheck_____:'//' WW3_WAVE TIME LOOP 5')
          !
          IF ( FLWIND ) THEN
             IF ( FLFRST ) ASF = 1.
          ...
          !        STOP
          !      ENDIF
          call print_memcheck(IAPROC+40000, 'memcheck_____:'//' WW3_WAVE TIME LOOP 6')
          if (w3_timings_flag) then
          ...
          call print_memcheck(IAPROC+40000, 'memcheck_____:'//' WW3_WAVE TIME LOOP 7')
          !
          ...
          call print_memcheck(IAPROC+40000, 'memcheck_____:'//' WW3_WAVE TIME LOOP 8')
          !
...
#endif
          call print_memcheck(IAPROC+40000, 'memcheck_____:'//' WW3_WAVE TIME LOOP 9')
          !
          ...
          IF ( FLIC1 .AND. DTI10.NE.0. ) THEN
             ...
             !
          END IF
          call print_memcheck(IAPROC+40000, 'memcheck_____:'//' WW3_WAVE TIME LOOP 10')
          !
          ...
          call print_memcheck(IAPROC+40000, 'memcheck_____:'//' WW3_WAVE TIME LOOP 11a')
          !
          ...
          call print_memcheck(IAPROC+40000, 'memcheck_____:'//' WW3_WAVE TIME LOOP 11b')
          !
          ...
          IF ( FLDDIR ) THEN
             ...
             FLDDIR = .FALSE.
          END IF
          call print_memcheck(IAPROC+40000, 'memcheck_____:'//' WW3_WAVE TIME LOOP 12')
          if (w3_debugrun_flag) then
          ...
          ELSE
             DTGpre = DTG
          END IF
          call print_memcheck(IAPROC+40000, 'memcheck_____:'//' WW3_WAVE TIME LOOP 13')
          !
          IF ( FLSOU .and. LPDLIB .and. FSSOURCE) THEN
             ...
             DO JSEA = 1, NP
                ...
                END IF
             END DO  ! DO JSEA=1,NP
          END IF ! IF ( FLSOU .and. LPDLIB .and. FSSOURCE)
          ...
          call print_memcheck(IAPROC+40000, 'memcheck_____:'//' WW3_WAVE TIME LOOP 14')
          ...
          call print_memcheck(IAPROC+40000, 'memcheck_____:'//' WW3_WAVE TIME LOOP 15')
          !
          if (w3_debugrun_flag) then
             DO JSEA = 1, NSEAL
                DO IS = 1, NSPEC
          ...
          if (w3_sec1_flag) then
             DTGTEMP=DTG
             DTG=DTG/NITERSEC1
             loop_count = nitersec1
          else
             loop_count = 1
          end if

          DO ISEC1=1,loop_count
             ...
             call print_memcheck(IAPROC+40000, 'memcheck_____:'//' WW3_WAVE TIME LOOP 17')
             ...
             if (LPDLIB) then
                if (w3_debugcoh_flag) then
                   ...
                   WRITE(740+IAPROC,*) 'FLCX         =', FLCX
                   WRITE(740+IAPROC,*) 'FLCY         =', FLCY
                end if
             end if
             !
             IF (GTYPE .EQ. UNGTYPE) THEN
             ...
             ELSE
                ...
                !
             END IF ! IF ((GTYPE .EQ. UNGTYPE) .and. LPDLIB)
...
370          CONTINUE
             IF ( FLSOU ) THEN
                ...
                DO JSEA=1, NSEAL
                   ...
                   ELSE ! IF ( MAPSTA(IY,IX) .EQ. 1 .AND. FLAGST(ISEA))
                      UST   (ISEA) = UNDEF
                      USTDIR(ISEA) = UNDEF
                      DTDYN (JSEA) = UNDEF
                      FCUT  (JSEA) = UNDEF
...
380       CONTINUE
       ...
       call print_memcheck(IAPROC+40000, 'memcheck_____:'//' WW3_WAVE END TIME LOOP')
       !
...
400    CONTINUE
       ...
       IF ( DTTST.LE.0. .AND. DTTST1.NE.0. .AND. FLAG_O ) THEN
          ...
          !
          if (w3_debugrun_flag) then
             WRITE(740+IAPROC,*) 'After DO_OUTPUT_EXCHANGES, step 6'
          end if
          IF ( NRQMAX .NE. 0 ) ALLOCATE( STATIO(MPI_STATUS_SIZE,NRQMAX) )
#endif
          call print_memcheck(IAPROC+40000, 'memcheck_____:'//' WW3_WAVE AFTER TIME LOOP2')
          !
          ...
          !
          call print_memcheck(IAPROC+40000, 'memcheck_____:'//' WW3_WAVE AFTER TIME LOOP3')
          !
          ...
          IF ( FLOUT(J) ) THEN
             ...
             IF ( DTTST .EQ. 0. ) THEN
                ...
                IF ( FLOUT(J) ) THEN
                   OUTID(2*J-1:2*J-1) = 'X'
                   if (w3_oasis_flag) then
                      IF ( (DTOUT(7).NE.0) .AND.           &
                           (DSEC21(TIME,TIME00).EQ.0 .OR. DSEC21(TIME,TIMEEND).EQ.0) ) then
                         OUTID(13:13) = ' '
                      end IF
                   end if
                ELSE
...
!
          ...
          !
       END IF !  IF ( DTTST.LE.0. .AND. DTTST1.NE.0. .AND. FLAG_O )

       if (w3_timings_flag) then
          CALL PRINT_MY_TIME("Before update log file")
       end if
       call print_memcheck(IAPROC+40000, 'memcheck_____:'//' WW3_WAVE AFTER TIME LOOP4')
       !
       ! 5.  Update log file ------------------------------------------------ /
    ...
    !
    call print_memcheck(IAPROC+40000, 'memcheck_____:'//' WW3_WAVE END W3WAVE')
    !
    RETURN
    !
    ! Formats
...
9044 FORMAT (' TEST W3WAVE : END OF OUTPUT')
  ...
  SUBROUTINE W3GATH ( ISPEC, FIELD )
    ...
    IF ( BSTAT(IBFLOC) .EQ. 0 ) THEN
       ...
       IF ( NRQSG2 .GT. 0 ) then
          CALL MPI_STARTALL ( NRQSG2, IRQSG2(IOFF,1), IERR_MPI )
       end IF
       if (w3_mpit_flag) then
          STRT(10:10) = 'g'
       end if
    ...
    IOFF =  1 + (BISPL(IBFLOC)-1)*NRQSG2
    IF ( NRQSG2 .GT. 0 ) then
       CALL MPI_WAITALL ( NRQSG2, IRQSG2(IOFF,1), STATUS, IERR_MPI )
    end IF
    !
    if (w3_mpit_flag) then
       STRT(11:11) = 'G'
...
9010 FORMAT ( ' |',I4,' |',8A15)
  ...
  SUBROUTINE W3SCAT ( ISPEC, MAPSTA, FIELD )
    ...
    IOFF   = 1 + (ISPLOC-1)*NRQSG2
    IF ( NRQSG2 .GT. 0 ) then
       CALL  MPI_STARTALL ( NRQSG2, IRQSG2(IOFF,2), IERR_MPI )
    end IF
    BSTAT(IBFLOC) = 2
    if (w3_mpit_flag) then
       STRT(12:12) = 's'
