...
CONTAINS
  ...
  SUBROUTINE W3WAVE ( IMOD, ODAT, TEND, STAMP, NO_OUT, ID_LCOMM, TIMEN)
    ...
    USE CONSTANTS       , only : TPIINV, DEBUG_NODE
    ...
    USE W3GDATMD        , only : XGRD, YGRD, ZB, FSN, FSPSI, FSNIMP, FSSOURCE, FSTOTALEXP, FSFCT
...
! W3_PDLIB start
          ...
          IF ( FLSOU .and. LPDLIB .and. FSSOURCE) THEN
             ...
             DO JSEA = 1, NP
...
#else
                   ! Note that there is no TAUA and TAUADIR arguments below
                   CALL W3SRCE(srce_imp_pre, IT, ISEA, JSEA, IX, IY, IMOD, &
                        VAold(:,JSEA), VA(:,JSEA),                         &
                        VSioDummy, VDioDummy, SHAVETOT(JSEA),              &
                        ALPHA(1:NK,JSEA), WN(1:NK,ISEA),                   &
                ...
                END IF
             END DO ! DO JSEA = 1, NP
          END IF  ! IF ( FLSOU .and. LPDLIB .and. FSSOURCE) THEN
...
!$OMP END PARALLEL
             ...
             IF ((GTYPE .EQ. UNGTYPE) .and. LPDLIB) THEN
                ...
                END IF  ! IF ((FSTOTALIMP .eqv. .FALSE.).and.(FLCX .or. FLCY)) THEN
...
!$OMP DO SCHEDULE (DYNAMIC,1)
                   ...
                   DO JSEA = 1, NSEAL
                      ...
                      IF ( MAPSTA(IY,IX) .EQ. 1 ) THEN
                         ...
                         END IF  !! GTYPE
                         !
                      END IF  ! IF ( MAPSTA(IY,IX) .EQ. 1 ) THEN
                   END DO  !  DO JSEA = 1, NSEAL
                   !
#ifdef W3_OMPG
!$OMP END DO
...
! W3_PDLIB end
...
#ifdef W3_PDLIB
                      call_srce_direct = .not. fssource
#else
                      call_srce_direct = .true.
#endif
                      if (call_srce_direct) then
#ifdef W3_FLX5
                   ...
                   ELSE
                      ...
                      FCUT  (JSEA) = UNDEF
                      !                    VA(:,JSEA)  = 0.
                   END IF  ! IF ( MAPSTA(IY,IX) .EQ. 1 .AND. FLAGST(ISEA)) THEN
                   if (w3_debugrun_flag) then
       ...
       IF ( DTTST.LE.0. .AND. DTTST1.NE.0. .AND. FLAG_O ) THEN
          ...
          DO J=1, NOTYPE
             ...
             IF ( FLOUT(J) ) THEN
                ...
                !
                if (w3_nl5_flag) then
                   if (do_point_output) TOSNL5  = TONEXT(:, 2)
                end if
                TOUT(:) = TONEXT(:,J)
                ...
                IF ( DTTST .EQ. 0. ) THEN
                   ...
                   if (do_gridded_output) then
                      ...
                      if (user_gridncout) then
                         ...
                         ! frequencies, otherwise native (inp) frequencies will be used
                         write_now = .false.
                         if (user_histalarm) then
          ...
          !
       END IF  ! IF ( DTTST.LE.0. .AND. DTTST1.NE.0. .AND. FLAG_O ) THEN
#ifdef W3_TIMINGS
...
9010 FORMAT ( ' |',I4,' |',8A15)
  ...
  SUBROUTINE W3SCAT ( ISPEC, MAPSTA, FIELD )
    ...
    ! Formats
    !
9000 FORMAT ( ' |     |',8A15) ! only for W3_MPIT
9010 FORMAT ( ' +-----+',8A15)
    !/
    !/ End of W3SCAT ----------------------------------------------------- /
    !/
  ...
  SUBROUTINE W3NMIN ( MAPSTA, FLAG0 )
    ...
    ! Formats
    !
9000 FORMAT ( ' TEST W3NMIN : IPROC =',I3,'  NLOC =',I5) ! only for W3_T
