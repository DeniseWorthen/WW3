...
950 FORMAT ( '        Past last ',A)
    !
9000 FORMAT ( ' TEST WMUPDT : INPUT : ',I4,I10.8,I7.6,            & ! W3_T
         '  <============================')
...
9073 FORMAT ( '               ',I2,1X,A3,L3,17X,2(I10.8,I7.6))
  ...
  SUBROUTINE WMUPD1 ( IMOD, IDSTR, J, IERR )
    ...
    USE W3GDATMD, ONLY: FSWND, NSEA  ! W3_SMC
    USE W3WDATMD, ONLY: TIME
    ...
    ! Formats
    !
9000 FORMAT ( ' TEST WMUPD1 : INPUT : ',2I4) ! W3_T
    !/
    !/ End of WMUPD1 ----------------------------------------------------- /
    !/
  ...
  SUBROUTINE WMUPD2 ( IMOD, J, JMOD, IERR )
    ...
    !
    IERR = 0
    if (w3_crx0_flag) then
       ICONSC = 0
    end if
    if (w3_crx1_flag) then
       ICONSC = 1
    end if
    if (w3_crx2_flag) then
       ICONSC = 2
    end if
    if (w3_wnx0_flag) then
       ICONSW = 0
    end if
    if (w3_wnx1_flag) then
       ICONSW = 1
    end if
    if (w3_wnx2_flag) then
       ICONSW = 2
    end if
    if (w3_wnx0_flag) then
       ICONSU = 0
...
1999 FORMAT (/ ' *** ERROR WMUPD2: OPTION NOT YET IMPLEMENTED ***'/)
    !
9000 FORMAT (  ' TEST WMUPD2 : INPUT : ',3I4) ! W3_T
...
9030 FORMAT (  ' TEST WMUPD2 : INITIAL FIELD FOR ',I1,             &
  ...
  SUBROUTINE WMUPDV ( IMOD, VX, VY, JMOD, VXI, VYI, UNDEF, CONSTP )
    ...
    LOGICAL                   :: MAP1(NX,NY), MAP2(NX,NY), MAP3(NX,NY), FLAGUP
                                                                 !
    ...
    ELSE
       ...
       if (w3_t1_flag) then
          DO, IX=1, NX
             IF ( NXA(IX,0) .GT. 0 ) WRITE (MDST,FORMAT1) IX, NXA(IX,1:MXA), RXA(IX,1:MXA), SUM(RXA(IX,1:MXA))
          END DO
       end if
       !
    ...
    if (w3_t1_flag) then
       DO, IY=1, NY
          IF ( NYA(IY,0) .GT. 0 ) WRITE (MDST,FORMAT1) IY, NYA(IY,1:MYA), RYA(IY,1:MYA), SUM(RYA(IY,1:MYA))
       END DO
    end if
    !
    ! 2.c Process grid
    ...
    ! Formats
    !
9000 FORMAT ( ' TEST WMUPDV : GRID INFORMATION : '/               & ! W3_T
         '             ',3I5,4E11.3/                         &
         '             ',3I5,4E11.3/                         &
         '               UNDEFINED = ',E10.3)
...
9021 FORMAT ( ' TEST WMUPDV : ARAY DIMENSIONED AS : ',2I6) ! W3_T1
9022 FORMAT ( ' TEST WMUPDV : RECONCILING MASKS') ! W3_T
9023 FORMAT ( '               SWEEP NR ',I4)
  ...
  SUBROUTINE WMUPDS ( IMOD, FD, JMOD, FDI, UNDEF )
    ...
    REAL                      :: XSR, YFL, YFR, YSL, YSR
    REAL                      :: FDL, WTOT, WL
    REAL                      :: LONC, LATC,  SXYC, XDI, DTOLER, VALUEINTER 
    REAL, ALLOCATABLE         :: RXA(:,:), RYA(:,:)
    LOGICAL                   :: MAP1(NX,NY), MAP2(NX,NY), MAP3(NX,NY), FLAGUP
    INTEGER,          POINTER :: NXI, NYI, MAP(:,:), MAPI(:,:)
    DOUBLE PRECISION, POINTER :: XGRDI(:,:), YGRDI(:,:), XGRDC(:,:), YGRDC(:,:)
    ...
    INTEGER,          POINTER :: ICLOSE
    CHARACTER(LEN=17)         :: FORMAT1                  ! W3_T1
    !/
    !/ ------------------------------------------------------------------- /
    ! 0.  Initialization
    ! 0.a Subroutine tracing and test output
    ...
    ! Formats
    !
9000 FORMAT ( ' TEST WMUPDS : GRID INFORMATION : '/               & ! W3_T
              '             ',3I5,4E11.3/                         &
              '             ',3I5,4E11.3/                         &
              '               UNDEFINED = ',E10.3)
...
9021 FORMAT ( ' TEST WMUPDS : ARAY DIMENSIONED AS : ',2I6) ! W3_T1
9022 FORMAT ( ' TEST WMUPDS : RECONCILING MASKS') ! W3_T
9023 FORMAT ( '               SWEEP NR ',I4)
    !/
    !/ End of WMUPDS ----------------------------------------------------- /
    !/
