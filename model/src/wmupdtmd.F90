...
CONTAINS
  ...
  SUBROUTINE WMUPDT ( IMOD ,TDATA )
    ...
    USE W3IDATMD , ONLY : TUN, TR0, TRN, TIN, T0N, T1N, T2N, TG0
    USE W3IDATMD , ONLY : TGN, TFN, TDN, TTN, TVN, TZN, TI1, TI2
    USE W3IDATMD , ONLY : TI3, TI4, TI5, JFIRST

    USE WMMDATMD , ONLY : IMPROC, MDSO, MDSS, MDST, MDSE, NMPSCR
    USE WMMDATMD , ONLY : NMPERR, ETIME, FLLSTL, FLLSTR, FLLSTI
    USE WMMDATMD , ONLY : INPMAP, IDINP, IFLSTI, IFLSTL, IFLSTR
    !/
    IMPLICIT NONE
    !/
    !/ ------------------------------------------------------------------- /
...
9073 FORMAT ( '               ',I2,1X,A3,L3,17X,2(I10.8,I7.6))
  ...
  SUBROUTINE WMUPD1 ( IMOD, IDSTR, J, IERR )
    ...
    USE W3IDATMD , ONLY : TW0, TWN, TU0, TUN, TR0, TRN, WX0, WXN
    USE W3IDATMD , ONLY : WY0, WYN, DT0, DTN, TIN, TRN, ICEI, UX0
    USE W3IDATMD , ONLY : UXN, UY0, UYN, RH0, RHN, T0N, T1N, T2N
    USE W3IDATMD , ONLY : TDN, INFLAGS1, TG0, TGN, GA0, GD0, GAN
    USE W3IDATMD , ONLY : GDN, BERGI, TTN, MUDT, TVN, MUDV, TZN
    USE W3IDATMD , ONLY : MUDD, TI1, TI2, TI3, TI4, TI5, ICEP1
    USE W3IDATMD , ONLY : ICEP2, ICEP3, ICEP4, ICEP5
    USE WMMDATMD , ONLY : IMPROC, NMPERR, MDST, MDSE, MDSF, ETIME
    USE WMMDATMD , ONLY : FLLSTL, FLLSTI, FLLSTR, RCLD, NDT, DATA0
    USE WMMDATMD , ONLY : DATA1, DATA2, NMV, NMVMAX, TMV, AMV, DMV
    !/
    IMPLICIT NONE
    !/
    !/ ------------------------------------------------------------------- /
...
9000 FORMAT ( ' TEST WMUPD1 : INPUT : ',2I4)
  ...
  SUBROUTINE WMUPD2 ( IMOD, J, JMOD, IERR )
    ...
    USE WMMDATMD  , ONLY : MDSO, ETIME, IDINP
...
9030 FORMAT ( ' TEST WMUPD2 : INITIAL FIELD FOR ',I1,             &
  ...
  SUBROUTINE WMUPDV ( IMOD, VX, VY, JMOD, VXI, VYI, UNDEF, CONSTP )
    ...
    USE W3GDATMD , ONLY : GTYPE, RLGTYPE, CLGTYPE, UNGTYPE
    USE W3GDATMD , ONLY : ICLOSE_NONE, ICLOSE_SMPL, ICLOSE_TRPL
    USE W3GDATMD , ONLY : HPFAC, HQFAC, XGRD, YGRD
    USE WMMDATMD , ONLY : IMPROC, NMPERR, NMPSCR, MDST, MDSE, MDSO
    USE WMMDATMD , ONLY : MDSS
    !/
    IMPLICIT NONE
    !/
    !/ ------------------------------------------------------------------- /
