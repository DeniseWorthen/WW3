module MallocInfo_m
  ...
  !/ Local PARAMETERs
  !/
  ! Note that must define flags explicitly at the module leve since cannot use
  ! wav_shr_flags - otherwise get a circular dependency
...
contains
  ...
  subroutine getMallocInfo(malinfo)
    ...
    !/ Parameter list
    !/
    !/ PROBEM(MV) this appeared in the executable section - which caused a 
    !/ compile problem 
  ...
  subroutine printMallInfo(ihdnl,malinfo)
    ...
    !/ Parameter list
    !/
    !/ PROBLEM (MV) - these specification statements appeared in the executable section
