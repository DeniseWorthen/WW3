...
CONTAINS
  ...
  SUBROUTINE W3IOGR ( INXOUT, NDSM, IMOD, FEXT )
    ...
    USE CONSTANTS
    USE W3GDATMD
    USE W3ADATMD, ONLY: MPI_COMM_WAVE ! W3_MPI
    USE W3ODATMD
    ...
    USE W3SRC4MD, ONLY: DELU, DELTAUW, DELUST
    USE W3SRC4MD, ONLY: DELALP, DELTAIL
    USE W3SRC4MD, ONLY: DIKCUMUL 
#endif
#ifdef W3_NL1
    ...
    INTEGER              :: IK, ITH, IK2, ITH2 ! W3_ST4
    INTEGER, ALLOCATABLE :: MAPTMP(:,:)
    INTEGER              :: IERR_MPI, IP       ! W3_MPI
    INTEGER, SAVE        :: IENT = 0           ! W3_S
    INTEGER              :: K                  ! W3_T
    ! PROBLEM (MV) do not use WRITE as a variable - use write_flag
    LOGICAL              :: write_flag, FLTEST = .FALSE., TESTLL, FLSNL2 = .FALSE.
    LOGICAL, SAVE        :: FLINP = .FALSE. , FLDISP = .FALSE., FLIS  = .FALSE. 
    ...
    call print_memcheck(740+IAPROC, 'memcheck_____:'//'WIOGR SECTION 1')
    ...
    END IF
    !
    IF (INXOUT.NE.'READ' .AND. INXOUT.NE.'WRITE' .AND. INXOUT.NE.'GRID') THEN
    ...
    write_flag  = INXOUT .EQ. 'WRITE'
    !
    if (w3_t_flag) then
       WRITE (NDST,9000) INXOUT, write_flag, NDSM, IGRD, TEMPXT
    end if
    !
    ...
    call print_memcheck(740+IAPROC, 'memcheck_____:'//'WIOGR SECTION 2')
    !
    ! open file ---------------------------------------------------------- *
    !
    I      = LEN_TRIM(FILEXT)
    ...
    REWIND ( NDSM )
    if (w3_debugiogr_flag) then
       WRITE(740+IAPROC,*) 'W3IOGR, step 5, WRITE=', write_flag
    end if
    !
    ...
    IF ( write_flag ) THEN
       ...
       !
       WRITE (NDSM) (NBO(I),I=0,NFBPO), (NBO2(I),I=0,NFBPO)
    ...
    ELSE
       ...
       !
       if (w3_smc_flag) then
          READ (NDSM,END=801,ERR=802,IOSTAT=IERR) NCel, NUFc, NVFc, NRLv, MRFct
          READ (NDSM,END=801,ERR=802,IOSTAT=IERR) NGLO, NARC, NBGL, NBAC, NBSMC
       end if
       !
       NK     = MK
       NTH    = MTH
       ...
       END IF
       !
       READ (NDSM,END=801,ERR=802,IOSTAT=IERR) (NBO(I),I=0,NFBPO), (NBO2(I),I=0,NFBPO)
       if (w3_t_flag) then
          WRITE (NDST,9002) (NBO(I),I=0,NFBPO)
          WRITE (NDST,9003) (NBO2(I),I=0,NFBPO)
       end if
    ...
    call print_memcheck(740+IAPROC, 'memcheck_____:'//'WIOGR SECTION 3')
    !
    ...
    ELSE
       ...
       call print_memcheck(740+IAPROC, 'memcheck_____:'//'WIOGR SECTION 4')

       READ (NDSM,END=801,ERR=802,IOSTAT=IERR) GTYPE, FLAGLL, ICLOSE
       if (w3_debugiogr_flag) then
       ...
       CASE (UNGTYPE) 
          ...
          END IF
          call print_memcheck(740+IAPROC, 'memcheck_____:'//'WIOGR SECTION 5')
          if (w3_debugiogr_flag) then
          ...
          call print_memcheck(740+IAPROC, 'memcheck_____:'//'WIOGR SECTION 6')
       ...
       if (w3_smc_flag) then
          IF( GTYPE .EQ. SMCTYPE ) THEN
             READ (NDSM,END=801,ERR=802,IOSTAT=IERR)NLvCel, NLvUFc, NLvVFc
             READ (NDSM,END=801,ERR=802,IOSTAT=IERR)IJKCel, IJKUFc, IJKVFc, ISMCBP 
             READ (NDSM,END=801,ERR=802,IOSTAT=IERR)ICLBAC 
             READ (NDSM,END=801,ERR=802,IOSTAT=IERR)ANGARC 
             READ (NDSM,END=801,ERR=802,IOSTAT=IERR)CTRNX,  CTRNY,  CLATF
          ENDIF
       end if
       !
       if (w3_debugiogr_flag) then
    ...
    call print_memcheck(740+IAPROC, 'memcheck_____:'//'WIOGR SECTION 7')
    if (w3_debugiogr_flag) then
    ...
    !
    IF ( write_flag ) THEN
       WRITE (NDSM) FACP, XREL, XFLT, FXFM, FXPM, XFT, XFC, FACSD, FHMAX, &
       ...
       if (w3_ref1_flag) then
          WRITE (NDSM) RREF, REFPARS, REFLC, REFLD
       end if
       if (w3_ig1_flag) then
          WRITE (NDSM) IGPARS(1:12)
       end if
       if (w3_ic2_flag) then
          WRITE (NDSM) IC2PARS(1:8)
       end if
       if (w3_ic3_flag) then
          WRITE (NDSM) IC3PARS
       end if
       if (w3_ic4_flag) then
          WRITE (NDSM) IC4PARS,IC4_KI,IC4_FC
       end if
       if (w3_ic5_flag) then
          WRITE (NDSM) IC5PARS
       end if
    ELSE
       ...
       if (w3_ref1_flag) then
          READ (NDSM) RREF, REFPARS, REFLC, REFLD
       end if
       if (w3_ig1_flag) then
          READ (NDSM) IGPARS(1:12)
       end if
       if (w3_ic2_flag) then
          READ (NDSM) IC2PARS(1:8)
       end if
       if (w3_ic3_flag) then
          READ (NDSM) IC3PARS
       end if
       if (w3_ic4_flag) then
          READ (NDSM) IC4PARS,IC4_KI,IC4_FC
       end if
       if (w3_ic5_flag) then
          READ (NDSM) IC5PARS
       end if
    ...
    !
    if (w3_t_flag) then
       WRITE (NDST,9040) FACP, XREL, XFLT, FXFM, FXPM, XFT, XFC, FACSD, FHMAX
    end if
    !
    ...
    if (w3_flx2_flag) then
       ...
       END IF
       IF ( FLTEST ) WRITE (NDST,9047) NITTIN, CINXSI
    end if
    !
    if (w3_flx3_flag) then
       IF ( write_flag ) THEN
          WRITE (NDSM)                            NITTIN, CINXSI, CD_MAX, CAP_ID
       ELSE
          READ (NDSM,END=801,ERR=802,IOSTAT=IERR) NITTIN, CINXSI, CD_MAX, CAP_ID
       END IF
    ...
    if (w3_st1_flag) then
       ...
       END IF
       IF ( FLTEST ) WRITE (NDST,90501) SINC1, SDSC1
    end if
    !
    if (w3_st2_flag) then
       IF ( write_flag ) THEN
    ...
    if (w3_nl1_flag) then
       ...
       ELSE
          READ (NDSM,END=801,ERR=802,IOSTAT=IERR)  &
               SNLC1, LAM, KDCON, KDMN, SNLS1, SNLS2, SNLS3
       END IF
       IF ( FLTEST ) WRITE (NDST,90511) SNLC1, LAM, &
    ...
    if (w3_nl2_flag) then
       IF ( write_flag ) THEN
          ...
          WRITE (NDSM) DPTHNL
       ELSE
          READ (NDSM,END=801,ERR=802,IOSTAT=IERR) IQTPE, NLTAIL, NDPTHS
          ALLOCATE ( MPARS(IGRD)%SNLPS%DPTHNL(NDPTHS) )
          DPTHNL => MPARS(IGRD)%SNLPS%DPTHNL
          PINIT  = .TRUE.
          READ (NDSM,END=801,ERR=802,IOSTAT=IERR) DPTHNL
       END IF
       IF ( FLTEST ) WRITE (NDST,90512) IQTPE, NLTAIL, NDPTHS
       IF ( FLTEST ) WRITE (NDST,91512) DPTHNL
    end if
    !
    if (w3_debugiogr_flag) then
    ...
    if (w3_nl3_flag) then
       IF ( FLTEST ) then
          WRITE (NDST,90513) SNLNQ, SNLMSC, SNLNSC, SNLSFD, SNLSFS
          DO I=1, SNLNQ
             WRITE (NDST,91513) SNLL(I), SNLM(I), SNLT(I), SNLCD(I), SNLCS(I)
          END DO
       END IF
    end if
    !
    if (w3_nl4_flag) then
       ...
       ELSE
          READ (NDSM,END=801,ERR=802,IOSTAT=IERR) ITSA, IALT
       END IF
       IF ( FLTEST ) WRITE (NDST,90514) ITSA, IALT
    end if
    ...
    if (w3_nl5_flag) then
       IF (write_flag) THEN
          ...
          CALL INSNL5
#endif
          WRITE (NDSM) QR5DPT, QR5OML, QI5DIS, QI5KEV,  QI5NNZ, QI5IPL, QI5PMX
       ELSE
          READ (NDSM,END=801,ERR=802,IOSTAT=IERR)  &
               QR5DPT, QR5OML, QI5DIS, QI5KEV, QI5NNZ, QI5IPL, QI5PMX
       END IF
       IF ( FLTEST ) WRITE (NDST,90515) QR5DPT, QR5OML, QI5DIS,    &
            QI5KEV, QI5NNZ, QI5IPL, QI5PMX
    end if
    !
    if (w3_nls_flag) then
       IF ( write_flag ) THEN
       ...
       ELSE
          READ (NDSM,END=801,ERR=802,IOSTAT=IERR)                &
               CNLSA, CNLSC, CNLSFM, CNLSC1, CNLSC2, CNLSC3
       END IF
       IF ( FLTEST ) WRITE (NDST,9251) CNLSA, CNLSC, CNLSFM, CNLSC1, CNLSC2, CNLSC3
    end if
    !
#ifdef W3_NL1
    IF ( .NOT. write_flag ) CALL INSNL1 ( IGRD )
#endif
    !
#ifdef W3_NL3
    IF ( .NOT. write_flag ) CALL INSNL3
#endif
#ifdef W3_NLS
    IF ( .NOT. write_flag ) CALL INSNLS
#endif
    !
...
#ifdef W3_MPI
    IF ( FLSNL2 .AND. .NOT. write_flag ) THEN
       DO IP=1, IAPROC-1
    ...
    if (w3_bt4_flag) then
       IF ( write_flag ) THEN
          WRITE (NDSM) SBTCX, SED_D50, SED_PSIC
       ELSE
          READ (NDSM,END=801,ERR=802,IOSTAT=IERR) SBTCX, SED_D50, SED_PSIC
       END IF
    end if
    !
    ...
    call print_memcheck(740+IAPROC, 'memcheck_____:'//'WIOGR SECTION 8')

    if (w3_db1_flag) then
       IF ( write_flag ) THEN
          WRITE (NDSM) SDBC1, SDBC2, FDONLY
       ELSE
          READ (NDSM,END=801,ERR=802,IOSTAT=IERR) SDBC1, SDBC2, FDONLY
       END IF
    ...
    if (w3_uost_flag) then
       IF ( write_flag ) THEN
          WRITE (NDSM) UOSTFILELOCAL, UOSTFILESHADOW, UOSTFACTORLOCAL, UOSTFACTORSHADOW
       ELSE
          READ (NDSM,END=801,ERR=802,IOSTAT=IERR) UOSTFILELOCAL, UOSTFILESHADOW, UOSTFACTORLOCAL, UOSTFACTORSHADOW
#ifdef W3_UOST
          CALL UOST_INITGRID(IGRD, UOSTFILELOCAL, UOSTFILESHADOW, UOSTFACTORLOCAL, UOSTFACTORSHADOW)
#endif
       END IF
    end if
    !
    ...
    if (w3_pr2_flag) then
       ...
       ELSE
          READ (NDSM,END=801,ERR=802,IOSTAT=IERR) DTME, CLATMN
       END IF
       if (w3_pr2_flag) then
          IF ( FLTEST ) WRITE (NDST,90602) DTME, CLATMN
       end if
    end if
    !
    if (w3_pr3_flag) then
       ...
       ELSE
          READ (NDSM,END=801,ERR=802,IOSTAT=IERR) WDCG, WDTH
       END IF
       IF ( FLTEST ) WRITE (NDST,90603) WDCG, WDTH
    end if
    !
    if (w3_smc_flag) then
       IF ( write_flag ) THEN
       ...
       ELSE
          READ (NDSM,END=801,ERR=802,IOSTAT=IERR) DTMS, Refran, FUNO3, FVERG, FSWND, ARCTC
       END IF
    ...
    IF ( .NOT. write_flag .AND. .NOT.FLDISP ) THEN
       if (w3_t_flag) then
          WRITE (NDST,9070)
       end if
       CALL DISTAB
    ...
    call print_memcheck(740+IAPROC, 'memcheck_____:'//'WIOGR SECTION 9')
    !
    RETURN
    !
...
1002 FORMAT (/' *** WAVEWATCH III ERROR IN W3IOGR : '/,              &
              ...
              5(A,/) /)
    !
9000 FORMAT (' TEST W3IOGR : INXOUT = ',A,', WRITE = ',L1,        & ! W3_T
...
9003 FORMAT ('      NBO2   : ',10I5)
    !
9010 FORMAT (' TEST W3IOGR : MODULE W3GDATMD GRID'/               & ! W3_T
             '      GTYPE  : ',I9/                                &
             '      FLAGLL : ',L9/                                &
             '      ICLOSE : ',I9/                                &
             '      SX, SY : ',2E10.3/                            &
...
9015 FORMAT ('      STEPS  : ',4F8.1/                             &
             ...
             '      STEDU  : ',F9.2)
    !
9016 FORMAT ('      FLAGS  : ',8L2) ! W3_T
9017 FORMAT ('      CLATS   : ',3F8.3,' ...'/                      &
             '      CLATIS  : ',3F8.3,' ...'/                      &
             '      CTHG0S  : ',3E11.3,' ...')
    !
9020 FORMAT (' TEST W3IOGR : MODULE W3ODATMD OUT5') ! W3_T
...
9025 FORMAT (' TEST W3IOGR : MODULE W3ODATMD OUT6'/               &
    ...
    !
9030 FORMAT (' TEST W3IOGR : MODULE W3GDATMD SGRD'/               & ! W3_T
             '      MAPWN  : ',8I4,' ...'/                        &
             '      MAPTH  : ',8I4,' ...'/                        &
             '      DTH    : ',F6.1/                              &
             '      TH     : ',4F6.1,' ...'/                      &
             ...
             '               ',3E10.3)
    !
9040 FORMAT (' TEST W3IOGR : MODULE W3GDATMD NPAR'/               & ! W3_T
             '      FACs   : ',5E10.3/                            &
             '               ',4E10.3)
    !
9047 FORMAT (' TEST W3IOGR : MODULE W3GDATMD SFLP'/            &
             '      FLUXES : ',I5,3X,E10.3)
...
9049 FORMAT (' TEST W3IOGR : MODULE W3GDATMD SLNP'/             &
             '      INPUT  : ',3E10.3)
    !
90501 FORMAT (' TEST W3IOGR : MODULE W3GDATMD SRCP'/             &
              '      INPUT  : ',E10.3/                           &
              '      DISSIP : ',E10.3)
90502 FORMAT (' TEST W3IOGR : MODULE W3GDATMD SRCP'/             &
              '      INPUT  : ',2E10.3/                          &
              '      DISSIP : ',4E10.3/                          &
              '               ',5E10.3/                          &
              '               ',3E10.3/                          &
              '      STAB2  : ',6E10.3)
90511 FORMAT (' TEST W3IOGR : MODULE W3GDATMD SNLP'/             &
              '      DATA   : ',2E10.3/                          &
              '               ',5E10.3)
!
90512 FORMAT (' TEST W3IOGR : MODULE W3GDATMD SNLP'/             &
             '      DATA   : ',I4,F5.1,I4)
91512 FORMAT ('               ',5F7.1)

    !
90513 FORMAT (' TEST W3IOGR : MODULE W3GDATMD SNLP'/             &
             '      DATA   : ',I4,4F8.3)
91513 FORMAT ('               ',2F8.3,F6.1,2E12.4)
    !
90514 FORMAT (' TEST W3IOGR : MODULE W3GDATMD SNLP'/             &
             '      DATA   : ',I4,I4)
    !
90515 FORMAT (' TEST W3IOGR : MODULE W3GDATMD SNLP'/             & ! W3_NL5
...
9053 FORMAT (' TEST W3IOGR : MODULE W3GDATMD SDBP'/             &
             '      DATA   : ',2E10.3,L4)
    !
90602 FORMAT (' TEST W3IOGR : MODULE W3GDATMD PROP'/             &
              '      DATA   : ',2E10.3)
    !
90603 FORMAT (' TEST W3IOGR : MODULE W3GDATMD PROP'/             &
              '      DATA   : ',2F6.2)
    !
9260 FORMAT (' TEST W3IOGR : MODULE W3GDATMD SMCG'/             &
             '      DATA   : ',3E10.3)
    !
9070 FORMAT (' TEST W3IOGR : DISPERSION INTEPOLATION TABLES') ! W3_T
    !/
    !/ End of W3IOGR ----------------------------------------------------- /
    !/
